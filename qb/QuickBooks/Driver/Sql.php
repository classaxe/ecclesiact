...
Version History:
  1.0.1 (2012-02-17)
    1) Many changes to operate with tables in Ecclesiact format:
       Having ID as primary key
       Maintaining systemID field
       Maintaining history_created_by, history_created_date and history_created_IP
       Maintaining history_modified_by, history_modified_date and history_modified_IP
    2) TODO:
       Use random IDs not autonumbers to avoid collisions with multiple sites in
       a single datbase
       Have SELECT statements check for correct systemID in filter clause
  1.0.0 (?)
...
abstract class QuickBooks_Driver_Sql extends QuickBooks_Driver
{
				...
				cfgkey = '" . $this->_escape($key) . "' ", $errnum, $errmsg)))
		{
			$this->_query("
				...
				SET
					...
					mod_datetime = NOW(),
                    history_modified_date = '".date('Y-m-d H:i:s')."',
                    history_modified_by = 0,
                    history_modified_IP = '".$_SERVER['REMOTE_ADDR']."'
				WHERE
					ID = " . $arr['ID'], $errnum, $errmsg);
		}
		else
		{
			return $this->_query("
			...
			return $this->_query("
				UPDATE
				 ...
				 SET 
                    ...
                    history_modified_IP = '".$_SERVER['REMOTE_ADDR']."'
				WHERE
					ID = " . $ticket_id, $db_errnum, $db_errmsg);
		}
	...
	protected function _errorLast($ticket)
	{
		...
		if ($ticket_id = $this->_ticketResolve($ticket))
		{
			if ($arr = $this->_fetch($this->_query("SELECT lasterror_num, lasterror_msg FROM " . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_TICKETTABLE) . " WHERE ID = '" . $ticket_id . "' ", $errnum, $errmsg, 0, 1)))
			{
	...
	protected function _recurDequeue($user, $by_priority = false)
	{
		...
		if ($arr = $this->_fetch($this->_query($sql . ' ', $errnum, $errmsg, 0, 1)))
		{
			...
			$errmsg = '';
			$this->_query(
              "UPDATE " . $this->_mapTableName(QUICKBOOKS_DRIVER_SQL_RECURTABLE) . "
              SET
                recur_lasttime = " . time() . ",
                history_modified_date = '".date('Y-m-d H:i:s')."',
                history_modified_by = 0,
                history_modified_IP = '".$_SERVER['REMOTE_ADDR']."'
              WHERE ID = " . $arr['ID'], $errnum, $errmsg);
			...
			if ($new_status == QUICKBOOKS_STATUS_SUCCESS)
			{
				$this->_query("
					...
					SET 
                        ...
                        history_modified_IP = '".$_SERVER['REMOTE_ADDR']."'
					WHERE
						ID = " . (int) $ticket_id . " ", $errnum, $errmsg);
			}
	...
	protected function _truncate($table, $max_history)
	{
		switch ($table)
		{
			case QUICKBOOKS_DRIVER_SQL_LOGTABLE:
				$sql = "SELECT ID FROM " . $this->_mapTableName($table) . " ORDER BY ID ASC LIMIT ";
				$field = 'ID';
				break;
	...
	protected function _generateCreateTable($name, $arr, $primary = array(), $keys = array())
	{
		...
		return array(
			'CREATE TABLE ' . $name . ' ( ' . "\n" . substr($sql, 0, -3) . ' ) ENGINE=MYISAM DEFAULT CHARSET=utf8; ',
			);
	}
	...
	protected function _initialize($init_options = array())
	{
		...
		$def = array(
			'ID' => array( QUICKBOOKS_DRIVER_SQL_SERIAL ),
