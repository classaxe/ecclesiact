...
if (QuickBooks_Utilities::initialized($dsn)){
    ...
    $primary_key_of_your_order = 123455;
    ...
    if (mysql_num_rows($result)>0){
      ...
      $Queue->enqueue(QUICKBOOKS_ADD_CUSTOMER, $record['ID'], 10);
    }
...
//	$Queue->enqueue(QUICKBOOKS_ADD_SALESRECEIPT, $primary_key_of_your_order, 1);
}
...
function _quickbooks_customer_add_response($requestID, $user, $action, $ID, $extra, &$err, $last_action_time, $last_actionident_time, $xml, $idents)
{
  ...
  $sql = "UPDATE `person` SET `qb_ident`='".$idents['ListID']."' WHERE `ID`=".$ID;
  mysql_query($sql);

//$fp = fopen('/www/logs/qb.txt', 'a+');
//fwrite($fp, $idents['ListID']."\r\n");
//fclose($fp);
		...
		<QBXML>
			<QBXMLMsgsRq onError="stopOnError">
				<SalesReceiptAddRq requestID="' . $requestID . '">
					<SalesReceiptAdd>
						<CustomerRef>
							<FullName>ConsoliBYTEs, LLC (12345)</FullName>
						</CustomerRef>
