...
class Event extends Posting {
  static $cache_event_registrant_count =      array();
  ...
  function __construct($ID="",$systemID=SYS_ID) {
    ...
    $this->_cp_vars_detail = array(
      'block_layout' =>             array('match' => '',                'default' => 'Event',           'hint' => 'Name of Block Layout to use'),
      ...
      'item_footer_component' =>    array('match' => '',                'default' => '',                'hint' => 'Name of component rendered below displayed Event'),
      'map_height' =>               array('match' => 'range|1,n',       'default' => '450',             'hint' => 'Height of Map if a location is given and found'),
      'map_width' =>                array('match' => 'range|1,n',       'default' => '450',            'hint' => 'Width of Map if a location is given and found'),
      'map_zoom' =>                 array('match' => 'range|1,18',      'default' => '16',              'hint' => 'Zoom for Map if a location is given and found'),
      ...
      'products_msg_signin' =>      array('match' => '',                'default' => '<p>There are registration options that cannot be seen unless you sign in.<br />If you have an account you may sign in to access to member-rate pricing.</p>',       'hint'=>'Message shown if there are products that public member cannot see'),
      ...
      'registration' =>             array('match' => 'enum|0,1',        'default' => '1',               'hint' => '0|1'),
    ...
    $this->_cp_vars_listings = array(
      ...
      'block_layout' =>             array('match' => '',                'default' => 'Event',           'hint' => 'Name of Block Layout to use'),
      'block_layout_for_associated' => array('match' => '',             'default' => 'Event for Associated', 'hint' => 'Name of Block Layout to use when this item is seen as associated with something else'),
      'box' =>                      array('match' => 'enum|0,1,2',      'default' => '0',               'hint' => '0|1|2'),
      'box_footer' =>               array('match' => '',                'default' => '',                'hint' => 'Text below displayed Events'),
      'box_header' =>               array('match' => '',                'default' => '',                'hint' => 'Text above displayed Events'),
      'box_rss_link' =>             array('match' => 'enum|0,1',        'default' => '0',               'hint' => '0|1'),
      'box_title' =>                array('match' => '',                'default' => 'Events',          'hint' => 'text'),
      'box_title_link' =>           array('match' => 'enum|0,1',        'default' => '0',               'hint' => '0|1'),
      'box_title_link_page' =>      array('match' => '',                'default' => 'all_events',      'hint' => 'page'),
      ...
      'filter_date_duration' =>     array('match' => '',                'default' => '',                'hint' => 'If filter_what is future or past, this further limits to given date range'),
      'filter_date_units' =>        array('match' => 'enum|,day,week,month,quarter,year',  'default' => '',     'hint' => ' |day|week|month|quarter|year - If filter_what is future or past, this provides units for given date range'),
      ...
      'filter_range_address' =>     array('match' => '',                'default' => '',                'hint' => 'Address to search for to obtain lat and lon'),
      'filter_range_distance' =>    array('match' => 'range|0,n',       'default' => '',                'hint' => 'Limits results to those events occuring within this distance of given location'),
      'filter_range_lat' =>         array('match' => '',                'default' => '',                'hint' => 'Latitude of search point'),
      'filter_range_lon' =>         array('match' => '',                'default' => '',                'hint' => 'Longitude of search point'),
      'filter_range_units' =>       array('match' => 'enum|km,mile',    'default' => 'km',              'hint' => 'Units of measurement to search point'),
      'filter_what' =>              array('match' => 'enum|all,future,month,past,year','default' => 'month', 'hint' => 'all|future|month|past'),
      'item_footer_component' =>    array('match' => '',                'default' => '',                'hint' => 'Name of component rendered below each displayed Event'),
  ...
  protected function BL_event_times($timeFormatCode=false){
    ...
    if ($result=='(All day)' && $this->record['effective_date_start']!=$this->record['effective_date_end']){
      return '';
    }
    return $result;
  }
  ...
  function draw_map($extra=true,$map_width=570,$map_height=500,$map_zoom=13) {
    global $system_vars;
    if (!$this->_get_ID()){
      if (!$ID = get_var('ID')){
    ...
    $map_html =
       "<strong>".$date_txt."<br />"
      .$record['title']."</strong><br />"
      .($record['thumbnail_small'] ? "<img style='float:left;border:1px solid #888;margin:0 5px 0 0;' src='".BASE_PATH."img/resize".$record['thumbnail_small']."?width=100&height=100' alt='' />" : "")
      .$record['location']
      ;
  ...
  function draw_registrants() {
    ...
    if (!$row = $this->get_record_for_sql($sql)){
      return "<h3 class='admin_heading'>Error</h3><p>The requested event doen't exist - perhaps it was deleted?</p>";
    }
    $out=
  ...
  protected function _draw_object_map_html_get_data(){
    ...
    $event_times =  $this->format_times($this->record['effective_time_start'],$this->record['effective_time_end']);
    $map_name =
    ...
    $map_location =
       ...
       ."<a href='#' onclick=\\\"popWin('".BASE_PATH."event/".$this->record['ID']."?print=1','event_".$this->record['ID']."','location=1,status=1,scrollbars=1,resizable=1',600,600,1);return false\\\">"
       ...
       .($this->_current_user_rights['canEdit'] ?
         ...
         ." onclick=\\\"details('".$this->_form."','".$this->record['ID']."','".$this->_popup['h']."','".$this->_popup['w']."');return false;\\\">"
         ."[Edit]</a>"
        : ""
        )
       ."\n"
  ...
  function export_icalendar(){
    ...
    $all_day =  (($record['effective_time_start']=='' || $record['effective_time_start']=='00:00') && ($record['effective_time_end']=='' || $record['effective_time_end']=='23:59'));
    $_start =   $record['effective_date_start']." ".($record['effective_time_start'] ? $record['effective_time_start'] : "00:00").":00";
    $_end =     $record['effective_date_end']  ." ".($record['effective_time_end']   ? $record['effective_time_end']   : "23:59").":00";
    $_stamp =   $record['history_modified_date']!='0000-00-00 00:00:00' ? $record['history_modified_date'] : $record['history_created_date'];
...
//      ."X-WR-RELCALID:".$system_vars['ID']."\r\n"
  ...
  static function form_field_end_date_and_time($width, $bulk_update, $row){
    $_width =         ((int)$width/2)-($bulk_update ? 21 : 0);
    $_div_open =      "<div style='float:left;height:21px;'>";
    $field_ede =      'effective_date_end';
    $value_ede =      (isset($row[$field_ede]) ? $row[$field_ede] : (isset($_REQUEST[$field_ede]) ? $_REQUEST[$field_ede] : ''));
    $field_ete =      'effective_time_end';
    $value_ete =      (isset($row[$field_ete]) ? $row[$field_ete] : '');
    return
       ($bulk_update ? "<input id=\"".$field_ede."_apply\" name=\"".$field_ede."_apply\" title=\"Apply changes to this field\" type='checkbox' value=\"1\" class=\"fl formField\" style=\"background-color: #60a060;  margin: 0 2px 0 0px;\">" : "")
      .$_div_open.draw_form_field($field_ede, $value_ede, "date", $_width)
      ."&nbsp; "
      ."</div>"
      .($bulk_update ? "<input id=\"".$field_ete."_apply\" name=\"".$field_ete."_apply\" title=\"Apply changes to this field\" type='checkbox' value=\"1\" class=\"fl formField\" style=\"background-color: #60a060;  margin: 0 2px 0 2px;\">" : "")
      .$_div_open.draw_form_field($field_ete,   $value_ete,   "hh:mm", $_width)."</div>"
      ."<div class='clear'>&nbsp;</div>";
  }

  static function form_field_start_date_and_time($width, $bulk_update, $row){
    $_width =         ((int)$width/2)-($bulk_update ? 21 : 0);
    $_div_open =      "<div style='float:left;height:20px;'>";
    $field_eds =      'effective_date_start';
    $value_eds =      (isset($row[$field_eds]) ? $row[$field_eds] : (isset($_REQUEST[$field_eds]) ? $_REQUEST[$field_eds] : ''));
    $field_ets =      'effective_time_start';
    $value_ets =      (isset($row[$field_ets]) ? $row[$field_ets] : '');
    $field_ete =      'effective_time_end';
    $value_ete =      (isset($row[$field_ete]) ? $row[$field_ete] : '');
    $field_ade =      'all_day_event';
    $value_ade =      (isset($row[$field_ets]) && ($row[$field_ets]=='' || $row[$field_ets]=='00:00') && isset($row[$field_ete]) && ($row[$field_ete]=='' || $row[$field_ete]=='23:59') ? 1 : 0);
    Page::push_content('javascript_onload', "  onclick_alldayevent(".$value_ade.");\n");
    return
       ($bulk_update ? "<input id=\"".$field_eds."_apply\" name=\"".$field_eds."_apply\" title=\"Apply changes to this field\" type='checkbox' value=\"1\" class=\"fl formField\" style=\"background-color: #60a060;  margin: 0 2px 0 0px;\">" : "")
      .$_div_open.draw_form_field($field_eds, $value_eds, "date", $_width)
      ."&nbsp; "
      ."</div>"
      .($bulk_update ? "<input id=\"".$field_ets."_apply\" name=\"".$field_ets."_apply\" title=\"Apply changes to this field\" type='checkbox' value=\"1\" class=\"fl formField\" style=\"background-color: #60a060;  margin: 0 2px 0 2px;\">" : "")
      .$_div_open.draw_form_field($field_ets,   $value_ets,   "hh:mm", $_width)
      ."&nbsp; "
      ."</div>"
      .$_div_open.draw_form_field($field_ade,   $value_ade,   "bool", $_width, "", "", "onclick='onclick_alldayevent(this.checked)'", 0, 0)
      ." <label for='".$field_ade."'>All day event</label>"
      ."</div>"
      ."<div class='clear'>&nbsp;</div>";
  }

  static function format_times($start, $end, $timeFormatCode=false){
    global $system_vars;
  ...
  function get_calendar_dates($MM, $YYYY, $memberID='',$category='*') {
    ...
    sscanf(
      ...
      "%4s-%2s-%2s %1s",
      $last_month_YYYY, $last_month_MM, $last_month_DD, $last_month_w
    );
    sscanf(
      adodb_date('Y-m-d',$this_month),
      "%4s-%2s-%2s",
      $this_month_YYYY, $this_month_MM, $this_month_DD
    );
    sscanf(
      adodb_date('Y-m-d',$next_month),
      "%4s-%2s",
      $next_month_YYYY, $next_month_MM
    );
    $arr_cal =	            array();
    ...
    for($i=0; $i<42; $i++){
      ...
      foreach ($records as $record) {
        ...
        if ($record['effective_date_end']!='0000-00-00' && $slot_date > $start_date && $slot_date == $end_date){
          $show = true;
        ...
        if($show){
          $events[] = array(
        	...
        	"enabled" =>              (isset($record['enabled']) ? $record['enabled'] : ""),
        	"icon" =>                 (isset($record['thumbnail_small']) ? $record['thumbnail_small'] : $record['icon']),
        	"important" =>            (isset($record['important']) ? $record['important'] : ""),
            "memberID" =>             (isset($record['memberID']) ? $record['memberID'] : ""),
            "member_shortform" =>     (isset($record['member_shortform']) ? $record['member_shortform'] : ""),
            "member_title" =>         (isset($record['member_title']) ? $record['member_title'] : ""),
            "member_url" =>           (isset($record['member_url']) ? $record['member_url'] : ""),
        	"permSHARED" =>           (isset($record['permSHARED']) ? $record['permSHARED'] : ""),
        	"systemID" =>             $record['systemID'],
  ...
  function get_events_for_date($YYYYMMDD,$memberID=''){
    ...
    foreach ($records as $record){
      ...
      $out[] = array(
        ...
        'path' =>                   (isset($record['path']) ? trim($record['systemURL'],'/').'/'.trim($record['path'],'/') : $record['URL']),
...
//    y($this->_get_records_args);die;
    ...
    switch($this->_get_records_args['what']){
      ...
      break;
      case 'calendar':
        $date_last_month =  adodb_mktime(0, 0, 0, $this->_get_records_args['MM'], 0, $this->_get_records_args['YYYY']);
        ...
        $date_next_month =	adodb_mktime(0, 0, 0, $this->_get_records_args['MM'], 42-$last_month_w, $this->_get_records_args['YYYY']);
        ...
        return
          "  (\n"
         ."    (\n"
         ."      `postings`.`effective_date_start` >= '".$last_month_YYYY."-".$last_month_MM."-".($last_month_DD-$last_month_w)."' AND\n"
    	 ."      `postings`.`effective_date_start` <  '".$next_month_YYYY."-".$next_month_MM."-".(lead_zero($next_month_DD,2))."'\n"
         ."    ) OR\n"
         ."    (\n"
         ."      `postings`.`effective_date_end` >= '".$last_month_YYYY."-".$last_month_MM."-".($last_month_DD-$last_month_w)."' AND\n"
         ."      `postings`.`effective_date_end` <= '".$next_month_YYYY."-".$next_month_MM."-".(lead_zero($next_month_DD,2))."'\n"
         ."    )\n"
         ."  ) AND\n";
      break;
      case 'date':
        return
          "  (\n"
         ."    `postings`.`effective_date_start` = '".$this->_get_records_args['YYYY']."-".$this->_get_records_args['MM']."-".$this->_get_records_args['DD']."' OR\n"
         ."    (\n"
         ."      `postings`.`effective_date_start` < '".$this->_get_records_args['YYYY']."-".$this->_get_records_args['MM']."-".$this->_get_records_args['DD']."' AND\n"
         ."      `postings`.`effective_date_end` >= '".$this->_get_records_args['YYYY']."-".$this->_get_records_args['MM']."-".$this->_get_records_args['DD']."'\n"
         ."    )\n"
         ."  ) AND\n";
      break;
      case 'future':
        return
          "  (\n"
         ."    (\n"
         ."      (`postings`.`effective_date_start` = '".$now_YYYY."-".$now_MM."-".$now_DD."' AND `postings`.`effective_time_start` > '".$now_hh.":".$now_mm.":".$now_ss."') OR\n"
         ."      (`postings`.`effective_date_start` > '".$now_YYYY."-".$now_MM."-".$now_DD."')\n"
         ."    )"
         .($this->_get_records_args['filter_date_duration'] ?
              " AND\n"
             ."    (`postings`.`effective_date_start` <  DATE_ADD('".$now_YYYY."-".$now_MM."-".$now_DD."', INTERVAL ".$this->_get_records_args['filter_date_duration']." ".$this->_get_records_args['filter_date_units']."))\n"
          :
              "\n"
          )
         ."  ) AND\n";
      ...
      case 'month':
        return
          "  (\n"
         ."    `postings`.`effective_date_start` >= '".$this->_get_records_args['YYYY']."-".$this->_get_records_args['MM']."-01' AND\n"
    	 ."    `postings`.`effective_date_start` <= DATE_ADD('".$this->_get_records_args['YYYY']."-".$this->_get_records_args['MM']."-01',INTERVAL 1 MONTH)\n"
         ."  ) AND\n";
      break;
      case 'past':
        return
         ...
         .($this->_get_records_args['filter_date_duration'] ?
              "AND\n"
             ."    `postings`.`effective_date_start` >=  DATE_SUB('".$now_YYYY."-".$now_MM."-".$now_DD."', INTERVAL ".$this->_get_records_args['filter_date_duration']." ".$this->_get_records_args['filter_date_units'].")\n"
          :
              "\n"
          )
         ."  ) AND\n";
  ...
  protected function get_related_products(){
    ...
    $sql =
       "SELECT\n"
      ."  (SELECT `product_grouping`.`name` FROM `product_grouping` WHERE `product_grouping`.`ID`=`product`.`groupingID`) `product_grouping_name`,\n"
      ."  `product`.*\n"
      ."FROM\n"
      ."  `postings`\n"
  ...
  function handle_report_delete(&$msg) {
    if ($registrations = $this->count_registrations()){
      $msg =
        status_message(
          2,true,'Event','',
          'has '.$registrations.' registrant'.($registrations==1 ? '' : 's').' registered. Deletion has been cancelled.',
          $this->_get_ID()
        );
      return false;
    }
    if ($children = $this->count_children()){
      ...
      if ($selected==1){
        ...
        $msg =
          status_message(
            1,true,'selected Event','',
            'is a Master Event with '.$children.' linked recurrence'.($children==1 ? '' : 's').'. Further clarification is needed.',
            $this->_get_ID()
          );
        return false;
      }
      if($selected == $parents){
        $msg =
          status_message(
            2,true,'selected Event','',
            'are all Master Events having a total of '.$children.' linked recurrence'.($children==1 ? '' : 's').'. Deletion has been cancelled.',
            $this->_get_ID()
          );
        return false;
      }
      $msg =
        status_message(
          2,true,'selected Event','',
          'include '.$parents." Master Event".($parents==1 ? '' : 's').' having a total of '.$children.' linked recurrence'.($children==1 ? '' : 's').'. Deletion has been cancelled.',
          $this->_get_ID()
        );
        return false;
    }
  ...
  function isRegisterable($YYYYMMDD=false,$hhmm=false){
  ...
  public function manage_registrants(){
    ...
    if (!$selectID = get_var('selectID')) {
      return
        ...
        ."<p style='margin:0.25em'>No registrants - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    $sql =
       "SELECT\n"
      ."  `postings`.`ID`,\n"
  ...
  function on_action_warn_if_non_registerable(){
    $msg =  "";
    ...
    foreach ($ID_arr as $ID){
      ...
      if ($this->record['canRegister'] && !$this->isRegisterable()) {
        $msg.="<li>".$this->record['effective_date_start']." - '".$this->record['title']."' isn't registerable - the date has already passed.</li>";
      }
    }
    return $msg;
  }

  function userIsRegistered($PUsername){
    $sql =
       "SELECT\n"
      ."  COUNT(*)\n"
      ."FROM\n"
  ...
  function _try_delete_item_dialog_has_children($child_count,$targetID){
    $msg =
     ...
     .($child_count==1 ? "(promotes the Recurrence to a stand-alone Event)" : "(promotes the first Recurrence to 'Master')")
     .",  or the whole series?"
     ...
     ." onclick=\\\"geid_set('command','');geid_set('submode','');geid_set('targetID','');geid('form').submit();\\\""
     ." /> "
  ...
  protected function _try_delete_item_dialog_has_registrants($registrant_count){
    ...
    Page::push_content(
      'javascript_onload',
      "  popup_msg=\"".$msg."\";popup_dialog('Event Delete',\"<div style='padding:4px;'>\"+popup_msg+\"</div>\",'300',120,'OK','','');\n"
    );
  }
  ...
  protected function _try_delete_item_msg_deleted_series(){
    ...
    Page::push_content(
      'javascript_onload',
      "  popup_msg=\"".$msg."\";popup_dialog('Event Delete',\"<div style='padding:4px'>\"+popup_msg+\"</div>\",'320',120,'Done','','');\n"
    );
  }
