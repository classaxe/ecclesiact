...
class Portal extends Base{
  ...
  private static $_path_type_prefixed_types =
    array(
      'Article', 'Event', 'Gallery_Album', 'Gallery_Image', 'Job_Posting', 'News_Item', 'Page', 'Podcast', 'Podcast_Album', 'Product', 'Survey'
    );
  ...
  static function parse_request() {
    ...
    if (Portal::_parse_request_mode_prefix($request, $mode, $submode, $ID, $page, $report_name, $targetID, $search_categories, $search_keywords, $search_offset, $search_type )){
      return true;
    }
  ...
  static function _parse_request_mode_prefix(
    $request, &$mode, &$submode, &$ID, &$page, &$report_name, &$targetID,
    &$search_categories, &$search_keywords, &$search_offset, &$search_type
  ){
...
*/
  ...
  static function _parse_request_page(&$page, $mode){
    if ($page=="" && $mode=="") {
  ...
  static function _parse_request_posting($request, &$mode, &$ID) {
    if ($request=='') {
      return false;
    }
  ...
  static function _parse_request_product($request, &$mode, &$ID) {
    if ($request=='') {
      return false;
    }
  ...
  static function _parse_request_search_range(
    $request, &$page, &$search_date_start, &$search_date_end, &$search_type
  ){
    if ($request==''){
      return false;
    }
    ...
    else if (strlen($search_date_end)==7) {
  ...
  static function _parse_request_special($request){
    $request_arr =  explode('/',$request);
  ...
  static function _parse_request_type_prefix(
    $request, &$mode, &$ID, &$page, &$search_type
  ){
    $request_arr =  explode("/",$request);
