<?php
define ("VERSION_POSTING","1.0.118");
/*
Version History:
  1.0.118 (2014-10-26)
...
// Current Getter and Setter functions:
  public function _get_default_enclosure_base_folder()      { return $this->_default_enclosure_base_folder; }
  public function _get_subtype()                            { return $this->subtype; }

  public function _set_default_enclosure_base_folder($value){ $this->_default_enclosure_base_folder = $value; }
  ...
  static function check_posting_prefix($path,$posting_prefix){
    ...
    switch ($posting_prefix) {
      ...
      case "YYYY/MM/DD":
        ...
        if (!sanitize('range',$YYYY,1990,2200,false) || !sanitize('range',$MM,1,12,false) || !sanitize('range',$DD,1,$_DD,false)){
          return false;
        }
      break;
    }
  ...
  function delete_for_parentID($parentID,$systemID=false){
    $sql =
       "DELETE FROM\n"
      ."  `".$this->_get_db_name()."`.`".$this->_get_table_name()."`\n"
      ."WHERE\n"
  ...
  function draw_product_catalogue(){
    $out = "";
    ...
    foreach($records as $record){
      if (Product::is_visible($record)){
        $record['quantity'] = Cart::item_get_quantity($record['ID'],$this->_get_object_type(),$this->_get_ID());
        $filtered[] = $record;
      }
    }
    ...
    $out.=
      ...
      .(Cart::has_items() || $shop ?
          "<p class='txt_c'>"
         .($shop ? "<input type='button' style='width:150px' value='Continue Shopping' onclick=\"document.location='".$shop."'\"/> " : "")
         .(Cart::has_items() ?
             "<input type='button' style='width:100px' value='Checkout' onclick=\"document.location='".BASE_PATH."checkout'\"/> "
            ."<input type='button' style='width:100px' value='Empty Cart' onclick=\"if (confirm('Empty your cart? \\nThis will remove ALL items')) { geid('command').value='empty_cart';geid('form').submit();}\"/> "
           : ""
          )
         ."</p>"
        : ""
  ...
  function draw_search_results($result) {
    $out = "";
    ...
    if ($found) {
      ...
      foreach ($result['results'] as $row) {
        ...
        if (substr($date,0,4)=="0000") { // repeating item
          $now =	time();
          $YYYY =   date('Y',$now);
          $MM =	    substr($date,6,2);
        }
  ...
  function get_community_path($communityID){
    if ($communityID==0){
      return '/';
    }
  ...
  function get_community_member_path($communityID){
    if ($communityID==0){
      return '/';
    }
    $sql =
  ...
  function get_coords($address=false) {
    if (!$address){
...
//    z($sql);
  ...
  function get_ID_csv_for_name_csv($name_csv,$systemID=false){
    $sql =
       "SELECT\n"
      ."  GROUP_CONCAT(`ID`)\n"
      ."FROM\n"
...
//    z($sql);
  ...
  function get_n_per_category($args) {
...
//    z($sql);die;
    ...
    $categories = $Obj_Category->get_labels_for_values("'".implode("','",array_keys($categories))."'","'".get_class($this)." category'");
//     y($categories);
    foreach($records as &$record) {
  ...
  function get_random_record($category_csv="",$systemID=SYS_ID){
    $sql =
       "SELECT\n"
      ."  *\n"
      ."FROM\n"
...
//    y($_args);die;
  ...
  protected function _get_records_apply_offset_and_limits(){
    if ($this->_get_records_args['limit']==0 && $this->_get_records_args['offset']==0) {
      return;
    }
...
//      y(System::$cache_ID_by_URL_array);
    foreach ($provider_arr as $url) {
      if (trim($url)!='') {
        ...
        switch($this->_get_type()) {
          case 'article':
            $results = $Obj_Remote->get_items('articles','','','',0,$category,0,$category_master,$memberID,$personID,$DD);
          break;
          case 'event':
            $results = $Obj_Remote->get_items('events',$what,$YYYY,$MM,0,$category,0,$category_master,$memberID,$personID,$DD);
          break;
          case 'gallery-image':
            $results = $Obj_Remote->get_items('gallery-images',  '','','',0,$category,0,$category_master,$memberID,$personID,$DD,$container_path,$container_subs);
          break;
          case 'job':
            $results = $Obj_Remote->get_items('jobs',  '','','',0,$category,0,$category_master,$memberID,$personID,$DD);
          break;
          case 'news':
            $results = $Obj_Remote->get_items('news',  '','','',0,$category,0,$category_master,$memberID,$personID,$DD);
          break;
          case 'podcast':
            $results = $Obj_Remote->get_items('podcasts','','','',0,$category,0,$category_master,$memberID,$personID,$DD,$container_path,$container_subs);
          break;
        }
  ...
  protected function _get_records_get_sql(){
    $sql =
      ...
      .($this->_get_subtype() ? "  `postings`.`subtype` = '".$this->_get_subtype()."' AND\n" : "")
      .$this->_get_records_get_sql_filter_publish_date()
      .$this->_get_records_get_sql_filter_date()
      .$this->_get_records_get_sql_filter_range()
      .($this->_get_records_args['byRemote']==true ?   "  `permSHARED` = 1 AND\n" : "")
      .($this->_get_records_args['category']!=='*' && $this->_get_records_args['category']!==''?   "  `postings`.`category` REGEXP \"".implode("|",explode(',',$this->_get_records_args['category']))."\" AND\n": "")
      .($this->_get_records_args['category_master'] ?  "  `postings`.`category` REGEXP \"".implode("|",explode(',',$this->_get_records_args['category_master']))."\" AND\n": "")
      .($this->_get_records_args['container_path'] ?
         ($this->_get_records_args['container_subs'] ?
            "  `postings`.`container_path` LIKE \"//".trim($this->_get_records_args['container_path'],'/')."%\" AND\n"
          :
            "  `postings`.`container_path` = \"//".trim($this->_get_records_args['container_path'],'/')."\" AND\n"
          )
        :
          ""
       )
      .($this->_get_records_args['important']!=='' ?   "  `postings`.`important`=".$this->_get_records_args['important']." AND\n" : "")
      .($this->_get_records_args['communityID']!=='' ? "  `postings`.`communityID` IN(".$this->_get_records_args['communityID'].") AND\n" : "")
      .($this->_get_records_args['memberID']!=='' ?    "  `postings`.`memberID` IN(".$this->_get_records_args['memberID'].") AND\n" : "")
      .($this->_get_records_args['isShared']==true ?   "  `permSHARED` = 1 AND\n" : "")
      .($this->_get_records_args['personID'] ?         "  `postings`.`personID` IN(".$this->_get_records_args['personID'].") AND\n" : "")
...
//    z($sql);
  ...
  protected function _get_records_sort_records(){
    $this->_get_records_sort_records_using_filter_order_by();
  }

  protected function _get_records_sort_records_using_filter_order_by(){
    switch($this->_get_records_args['order_by']){
      case "date":
  ...
  function get_root_ID($ID, $depth=0) {
    $max_depth = 3;
    $sql =
       "SELECT"
      ."  `postings`.`parentID`\n"
  ...
  function get_search_results($args, $cp) {
    ...
    $search_types = array();
    if ($cp['search_articles']){        $search_types[] = "'article'"; }
    if ($cp['search_events']){          $search_types[] = "'event'"; }
    if ($cp['search_gallery_images']){  $search_types[] = "'gallery-image'"; }
    if ($cp['search_jobs']){            $search_types[] = "'job-posting'"; }
    if ($cp['search_news']){            $search_types[] = "'news-item'"; }
    if ($cp['search_podcasts']){        $search_types[] = "'podcast'"; }
    ...
    $sql =
      ...
      .($search_communityID!=0 ? "  (`p`.`communityID`=".$search_communityID." OR `p`.`memberID` IN(SELECT `memberID` FROM `community_membership` WHERE `communityID`=".$search_communityID.")) AND\n" : "")
      .($search_memberID!=0 ? "  `p`.`memberID` IN(".$search_memberID.") AND\n" : "")
      .($userIsAdmin ? "" : "  IF(`p`.`type`='event',`effective_date_start`,`date`)<= NOW() AND\n")
      .($search_date_start!="" ? "  IF(`p`.`type`='event',`effective_date_start`,`date`) >= '".$search_date_start."' AND\n" : "")
      .($search_date_end!="" ? "  IF(`p`.`type`='event',`effective_date_start`,`date`) < CONVERT(DATE_ADD('".$search_date_end."',INTERVAL 1 DAY) USING utf8) AND\n" : "")
      .($search_keywordIDs!="" ? "  `k`.`keywordID` IN(".$search_keywordIDs.") AND\n" : "")
      .($search_type=='' || $search_type=='*' ? "" : "  `p`.`type` = '".$search_type."' AND\n")
      .($search_text ?
           "(\n"
      ...
      .($search_categories!="" ? "  `category` REGEXP \"".implode("|",explode(', ',$search_categories))."\" AND\n": "")
...
//          $visible = $this->is_visible($row);
        ...
        if ($visible) {
          switch($row['type']) {
            case 'article':
              if ($out['article']['count']>=$search_offset && ($limit==0 || count($out['article']['results'])<$limit)) {
                $out['article']['results'][] = $row;
              }
              $out['article']['count']++;
            break;
            case 'event':
              if ($out['event']['count']>=$search_offset && ($limit==0 || count($out['event']['results'])<$limit)) {
                $out['event']['results'][] = $row;
              }
              $out['event']['count']++;
            break;
            case 'gallery-image':
              if ($out['gallery-image']['count']>=$search_offset && ($limit==0 || count($out['gallery-image']['results'])<$limit)) {
                $out['gallery-image']['results'][] = $row;
              }
              $out['gallery-image']['count']++;
            break;
            case 'job-posting':
              if ($out['job-posting']['count']>=$search_offset && ($limit==0 || count($out['job-posting']['results'])<$limit)) {
                $out['job-posting']['results'][] = $row;
              }
              $out['job-posting']['count']++;
            break;
            case 'news-item':
              if ($out['news-item']['count']>=$search_offset && ($limit==0 || count($out['news-item']['results'])<$limit)) {
                $out['news-item']['results'][] = $row;
              }
              $out['news-item']['count']++;
            break;
            case 'podcast':
              if ($out['podcast']['count']>=$search_offset && ($limit==0 || count($out['podcast']['results'])<$limit)) {
  ...
  function get_unique_name($name,$container_path='',$systemID=SYS_ID) {
...
//    y($query_arr);y($video);y($img);
  ...
  public static function is_in_active_date_range($record) {
    ...
    return (
      ...
      ( $record['date_end']=='0000-00-00' ||   $now < $record['date_end'])
      ? 1 : 0
    );
  }
  ...
  public function manage_product_relationships(){
    ...
    $out = "<h3 style='margin:0.25em'>Product Relationships for ".$this->_get_object_name()."</h3>";
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No Product Relationships - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
      $out.= draw_auto_report($report,1);
    }
    return $out;
  }
  ...
  public function manage_products(){
    ...
    $out = "<h3 style='margin:0.25em'>Products associated with ".$this->_get_object_name()."</h3>";
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No associated products - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
      $out.= draw_auto_report($params['report_related_products'],1);
    }
    return $out;
  }

  function on_action_set_path(){
  ...
  function on_action_handle_video(){
    global $action_parameters;
    $type =     $action_parameters['triggerObject'];
    $ID =       $action_parameters['triggerID'];
    $ID_arr =   explode(',',$ID);
  ...
  function set_path($reveal_modification=false){
    $this->load();
    $this->record['path'] =   ""; // reset this so we can calculate from scratch
    if ($this->record['systemID']!=SYS_ID) {
      ...
      $this->record['systemID'] = SYS_ID;  // This is a total con-trick to prevent system URL from appearing in path
    }
    $prefix = "/";
