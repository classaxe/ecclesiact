...
class Posting extends Displayable_Item
{
  ...
  //    z($sql);
    ...
    protected function _get_records_from_partners()
    {
        ...
        $category =         $this->_get_records_args['filter_category_list'];
  ...
  //      y(System::$cache_ID_by_URL_array);
    ...
    {
        $containerPathsCsv =    explode(',', $this->_get_records_args['filter_container_path']);
        $containerPathsArr =    array();
        foreach ($containerPathsCsv as $path) {
            $containerPathsArr[] = trim($path, '/ ');
        }
        $sql =
             "SELECT\n"
            ."  `postings`.*,\n"
            ...
            .(!in_array($this->_get_records_args['filter_category_list'], array('','*')) ?
                 "  `postings`.`category` REGEXP \""
                .implode("|", explode(',', $this->_get_records_args['filter_category_list']))
                ."\" AND\n"
              :
                ""
             )
            .($this->_get_records_args['filter_text_list']!==''?
                 "  (\n"
                ."    (`postings`.`content_text` REGEXP \""
                .implode("|", explode(',', $this->_get_records_args['filter_text_list']))
                ."\") OR\n"
                ."    (`postings`.`title` REGEXP \""
                .implode("|", explode(',', $this->_get_records_args['filter_text_list']))
                ."\")\n"
                ."  ) AND\n"
              :
            ...
            .($this->_get_records_args['filter_container_path'] ?
                ($this->_get_records_args['filter_container_subs'] ?
                    ...
                    .implode("%\" OR   `postings`.`container_path` LIKE \"//",$containerPathsArr)
                    ."%\""
                    .") AND\n"
                 :
                     "(  `postings`.`container_path` = \"//"
                    .implode("\" OR   `postings`.`container_path` = \"//",$containerPathsArr)
                    ."\""
                    .")AND\n"
                )
                :
            ...
            .($this->_get_records_args['filter_has_video']!=='' ?
                "  `postings`.`video`"
                .($this->_get_records_args['filter_has_video']===true ? "!=''" : "=''")
                ." AND\n"
              :
                ""
             )
