...
class Report_Config extends Report {
    ...
    public function _draw_config_for_type($reports){
        $out = "";
        $strings = array(
            'ID' =>               '',
            'actions' =>          '',
    ...
    public function get_overview_global(){
        ...
        $sql =
            ...
            . "              CONCAT_WS(\n"
            . "                '|',\n"
            . "                `ID`, `report_columns_csv`, `report_filters_csv`\n"
            . "              )\n"
            . "            )\n"
            . "            ORDER BY \n"
            . "              `ID`\n"
            ...
            . "      `report_settings`\n"
            . "    WHERE\n"
            . "      `report_settings`.`systemID` = 1 AND\n"
            . "      `report_settings`.`destinationType` = 'global' AND\n"
            . "      `report_settings`.`reportID`=`report`.`ID`\n"
            . "  ) `settings`,\n"
            ...
            . "              CONCAT_WS(\n"
            . "                '|',\n"
            . "                `rcf`.`ID`, `rcf`.`filterID`, `rcf`.`filter_criterion`, `rcf`.`filter_matchmodeID`, `rcf`.`filter_seq`, `rcf`.`filter_value`\n"
            . "              )\n"
            . "            )\n"
            . "            ORDER BY \n"
            . "              `rcf`.`ID`\n"
            ...
            . "    )\n"
            . "    FROM\n"
            . "      `report_filter_criteria` `rcf`\n"
            . "    LEFT JOIN `report_filter` ON\n"
            . "       `report_filter`.`ID` = `rcf`.`filterID`\n"
            . "    WHERE\n"
            . "      `report_filter`.`systemID` = 1 AND\n"
            . "      `report_filter`.`destinationType` = 'global' AND\n"
            . "      `report_filter`.`reportID`=`report`.`ID`\n"
        ...
        foreach ($records as &$record){
            ...
            $record['settings'] =   strToLower($record['settings']);
            $record['filters'] =    strToLower($record['filters']);
            $record['criteria'] =   strToLower($record['criteria']);
            $combined = $record['report']
                . '|' . $record['actions']
                . '|' . $record['columns']
                . '|' . $record['settings']
                . '|' . $record['filters']
                . '|' . $record['criteria'];
            $crc32 =  dechex(crc32($combined));
            $record['crc32'] =    $crc32;
        }
        return $records;
    }
