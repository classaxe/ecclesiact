...
class CRM_Case extends Record
{
    ...
    public function cases_for_person()
    {
    ...
    {
        $header =
            "Selected ".$this->_get_object_name().$this->plural($targetID)." with related Tasks";
        $extra_delete =
             "DELETE FROM `case_tasks`             WHERE `destinationType`='cases' AND `destinationID` IN ("
            .$targetID
            .");\n";
        $Obj = new Backup;
        $extra_select = $Obj->db_export_sql_query(
            "`case_tasks`            ",
            "SELECT * FROM `case_tasks` WHERE `destinationType`='cases' AND `destinationID` IN ("
            .$targetID
    ...
    public function on_update()
    {
        ...
        $bulk_update =
            (isset($action_parameters['data']['bulk_update']) && $action_parameters['data']['bulk_update']==1 ?
                true
            :
                false
            );
        foreach ($ID_arr as $ID) {
            ...
            switch ($bulk_update) {
                ...
                default:
                    if (isset($action_parameters['data']['ID'])) {
                        ...
                        $changed_closed =
                            (isset($action_parameters['data']['closed']) ?
                                $action_parameters['data']['closed']!=$old_closed
                             :
                                false
                            );
                    }
                    $changed_assigned_personID = (
                        isset($action_parameters['data']['assigned_personID']) &&
                        (int)$action_parameters['data']['assigned_personID']
                    ) != $old_assigned_personID;
                    $changed_related_personID = (
                        isset($action_parameters['data']['related_personID']) &&
                        (int)$action_parameters['data']['related_personID']
                    ) !=  $old_related_personID;
                    $changed_history_due_date = (
                        isset($action_parameters['data']['history_due_date']) &&
                        $action_parameters['data']['history_due_date']
                    ) !=  $old_history_due_date;
                    $changed_category = (
                        isset($action_parameters['data']['category']) &&
                        $action_parameters['data']['category']
                    ) !=  $old_category;
                    $changed_subject = (
                        isset($action_parameters['data']['subject']) &&
                        $action_parameters['data']['subject']
                    ) !=  $old_subject;
                    break;
            }
            if ($changed_closed &&
                (!isset($action_parameters['data']['closed']) || $action_parameters['data']['closed']==0)
            ) {
            ...
            if ($changed_closed &&
                isset($action_parameters['data']['closed']) &&
                $action_parameters['data']['closed']==1
            ) {
    ...
    public function manage_tasks()
    {
        ...
        if (!get_var('selectID')) {
            $out.=
                 "<p style='margin:0.25em'>There are no tasks - this "
