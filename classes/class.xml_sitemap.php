<?php
define ("VERSION_XML_SITEMAP","1.0.2");
/*
Version History:
  1.0.2 (2014-12-29)
...
class XML_Sitemap extends Record {
    ...
    if ($buttons){
      foreach ($buttons as $button) {
        if ($button['sitemap_priority'] && $button['visible']) {
          ...
          else if(substr($bURL,0,2)=='./'){
            $bURL = BASE_PATH.substr($bURL,2);
          }
          $out[] = $button['sitemap_priority']."|".$button['sitemap_frequency']."|".trim($this->_system_vars['URL'],'/')."/".trim($bURL,'/');
          if ($childID){
  ...
  private function _get_xml_head(){
    $this->_xml.=
      ...
      ."  Generated by ".System::get_item_version('system_family')." ".System::get_item_version('codebase').".".$this->_system_vars['db_version']
      ." -->\n";
  }
