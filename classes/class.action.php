...
class Action extends Record
{
    ...
    public function count_for_source($destinationOperation, $destinationID, $sourceID, $sourceTrigger, $sourceType)
    {
        $sql =
             "SELECT\n"
            ."  COUNT(*) AS `count`\n"
    ...
    public function draw_combo_action_operation(
        ...
        if ($isMASTERADMIN) {
            $sql_components =
                ...
                ."  CONCAT(\n"
                ."    '[C] [',\n"
                ."    IF(`systemID`=1, '*', `system`.`textEnglish`),\n"
                ."    '] ',\n"
                ."    `component`.`name`\n"
                ."  ) AS `text`,\n"
        ...
        $html =
            ...
            .Report_Column::draw_label(
                'Operation',
                'Determines the kind of activity this action performs.'
            )
            ."</td>\n"
            ."    <td>"
            .draw_form_field(
                ...
                $form_op_sql,
                0,
                "onchange=\"set_action_operation_options(geid('destinationOperation'),geid('destinationValue'));\""
            )
            .draw_form_field(
                'temp_id',
                $destinationID,
            ...
            .Report_Column::draw_label(
                'Target',
                'Determines which item relating to the operation will be used.'
            )
            ."</td>\n"
            ."    <td>"
            ."<select id='destinationID' name='destinationID' class='formField' style='width:"
            .($formContentWidth+4)
            ."px;'>\n"
            ...
            .Report_Column::draw_label(
                'Value',
                'Provides additional parameters to a component for execution or permissions for group membership.'
            )
            ."</td>\n"
            ."    <td>"
            .draw_form_field(
        ...
        if (count($type_arr)==0 || in_array('component_execute', $type_arr)) {
            ...
            $js.=
                ...
                ."  destID.component_execute[0] =\n"
                ."    ['COMPONENTS (Please Select - standard entries are prefixed with *) ',''];\n";
        ...
        if (count($type_arr)==0 || in_array('mailtemplate_send_email', $type_arr)) {
            $js.=
                ...
                ."  destID.mailtemplate_send_email[0] =\n"
                ."    ['EMAIL TEMPLATES: (Please Select - standard entries are prefixed with *)',''];\n";
        ...
        foreach ($records as $row) {
          ...
          //          print_r($action_parameters); die;
                ...
                case "event_register":
                    ...
                    $inviterID =  $_personID;
                    $this->execute_event_register($eventID, $multiplier, $inviterID, $triggerType, $triggerID);
                    break;
                case "group_membership_set":
                    ...
                    } elseif (array_key_exists('new_personID', $data)) {
                ...
                case "mailtemplate_send_email":
                    ...
                    do_log(
                        ...
                        __CLASS__.'::'.__FUNCTION__.'()',
                        'mailtemplate_send_email',
                        'Sending Email for '.component_result('PUsername').' using template '.$ID
                    );
                    break;
                default:
    ...
    private function execute_event_register($eventID, $multiplier, $inviterID, $triggerType, $triggerID)
    {
    ...
    public function exists_for_source($destinationOperation, $destinationID, $sourceID, $sourceTrigger, $sourceType)
    {
        return
            $this->count_for_source($destinationOperation, $destinationID, $sourceID, $sourceTrigger, $sourceType)>0;
    }
    ...
    public function get_records_matching($sourceType, $sourceID, $sourceTrigger)
    {
        $sql =
         "SELECT\n"
        ."  *\n"
