...
class Scheduled_Task extends Record{
    const VERSION = '1.0.5';
    const FIELDS = 'ID, archive, archiveID, deleted, enabled, systemID, description, componentID, at_YYYY, at_MM, at_DD, at_hour, at_min, at_DOW, at_WOM, date_last_run, last_result, repeats_remaining, run_now, running, history_created_by, history_created_date, history_created_IP, history_modified_by, history_modified_date, history_modified_IP';
    ...
    public function _get_current_task()
    {
        return $this->_current_task;
    }

    public function _get_tasks()
    {
        return $this->_tasks;
    }

    public function _set_current_task($value)
    {
        $this->_current_task = $value;
    }

    public function _set_tasks($value)
    {
        $this->_tasks = $value;
    }

    public function all_tasks_load()
    {
        sscanf(
    ...
    private function _current_task_run()
    {
        ...
        if (!$Obj_Component->exists()) {
            do_log(
                ...
                'component_execute',
                'No such Component as '.$task['componentID']
            );
            return false;
        }
