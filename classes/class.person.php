    ...
    protected function _draw_listings_load_records()
    {
        ...
        $results = $this->getFilteredSortedAndPagedRecords(
            array(
                ...
                'filter_groups_list' =>
                    (isset($this->_cp['filter_groups_list']) ? $this->_cp['filter_groups_list'] : ''),
                'results_limit' =>
                    $this->_cp['results_limit'],
                'results_offset' =>
                    $this->_filter_offset,
                'results_order' =>
                    (isset($this->_cp['results_order']) ? $this->_cp['results_order'] : 'NLast')
            )
        );
  ...
  // find any additional permissions from session:
    ...
    public function getFilteredSortedAndPagedRecords()
    {
        ...
        $sql =
            ...
            ."WHERE\n"
            .$this->get_records_where_modifier()
            ."  `person`.`systemID` = ".$this->_get_systemID()." AND\n"
            .($vars['filter_category_list']!="*" ?
                "  `person`.`category` REGEXP \"".implode("|", explode(',', $vars['filter_category_list']))."\" AND\n"
              :
                ""
             )
    ...
    public function scrubPiiData()
    {
        ...
        foreach ($IDs as $ID) {
            $this->_set_ID($ID);
            $this->scrubPIIDataForPerson();
        }
    }
    
    public function scrubPIIDataForPerson()
    {
