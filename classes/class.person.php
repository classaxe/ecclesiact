...
class Person extends Displayable_Item {
  ...
  public function __construct($ID=""){
    ...
    $this->_cp_vars_listings = array(
      ...
      'box_title' =>                array('match' => '',                    'default' => 'People',          'hint' => 'text'),
      'box_title_link' =>           array('match' => 'enum|0,1',            'default' => '0',               'hint' => '0|1'),
      'box_title_link_page' =>      array('match' => '',                    'default' => '',                'hint' => 'page'),
      ...
      'filter_groups_list' =>       array('match' => '',                    'default' => '',                'hint' => 'Optionally INSIST on this category'),
      ...
      'results_limit' =>            array('match' => 'range|0,n',           'default' => '10',              'hint' => '0..n'),
      'results_order' =>            array('match' => 'enum|NFirst,NLast,PUsername', 'default' =>'NFirst',   'hint'=>'firstname|lastname|username'),
      'results_paging' =>           array('match' => 'enum|0,1,2',          'default' => '0',               'hint' => '0|1|2 - 1 for buttons, 2 for links'),
      'show_about' =>               array('match' => 'enum|0,1',            'default' => '1',               'hint' => '0|1'),
      'show_address' =>             array('match' => 'enum|0,1',            'default' => '1',               'hint' => '0|1'),
      'show_address_map_link' =>    array('match' => 'enum|0,1',            'default' => '1',               'hint' => '0|1'),
      'show_cellphone' =>           array('match' => 'enum|0,1',            'default' => '1',               'hint' => '0|1'),
  ...
  protected function BL_context_selection_start(){
    ...
    return
     ...
     .($this->_current_user_rights['isSYSADMIN'] && isset($this->_block_layout['systemID']) && $this->_block_layout['systemID']==SYS_ID || ($this->_current_user_rights['isMASTERADMIN'] && isset($this->_block_layout['ID'])) ?
        "_CM_ID[3]='"
      ...
      : '')
     ."}\" "
     .($this->record['systemID']!=SYS_ID ? " title='This ".$this->record['type']." belongs to another site'" : '')
  ...
  protected function BL_distance(){
    if (isset($this->record['AMap_range']) && $this->record['AMap_range']!=100000){
      return $this->record['AMap_range'].' '.($this->_filter_radius_units=="km" ? "km" : "mile").($this->record['AMap_range']==1 ? '' : 's');
    }
    if (isset($this->record['WMap_range']) && $this->record['WMap_range']!=100000){
      return $this->record['WMap_range'].' '.($this->_filter_radius_units=="km" ? "km" : "mile").($this->record['WMap_range']==1 ? '' : 's');
    }
  }
  ...
  protected function BL_map(){
    ...
    foreach ($this->_records as $r){
      $this->_draw_item_map_point($r,'home',(isset($this->_cp['marker_home']) ? $this->_cp['marker_home'] : ''));
      $this->_draw_item_map_point($r,'work',(isset($this->_cp['marker_work']) ? $this->_cp['marker_work'] : ''));
    }
    ...
    if(isset($this->_search_lat) && $this->_search_lat && $this->_search_lon){
      ...
      if ($has_start_ring || $has_range_ring){
        ...
        if ($has_range_ring){
          $range_radius =   $this->_filter_radius_distance*($this->_filter_radius_units=='km' ? 1000 : 1609.34);
        }
        if ($has_start_ring && !$has_range_ring){
          $rings = array('start');
        }
        else if ($has_range_ring && !$has_start_ring){
          $rings = array('range');
        }
        else {
    ...
    if ($range){
      ...
      $this->_Obj_Map->map_centre($this->_map_points[0]['map_lat'],$this->_map_points[0]['map_lon'],$this->_cp['map_default_zoom']);
    }
    $args = array(
  ...
  protected function _draw_item_map_point($r, $type, $icon=''){
    ...
    $description =
       "<a href='#item_".$r['ID']."'"
      .($this->_current_user_rights['canEdit'] ? " onclick=\\\"details('".$r['type']."',".$r['ID'].",".$this->_popup[$r['type']]['h'].",".$this->_popup[$r['type']]['w'].",'','');\\\"" : "")
      .">"
  ...
  protected function _draw_listings_load_records(){
    ...
    $results = $this->get_records(
      array(
        ...
        'filter_category_master' => (isset($this->_cp['filter_category_master']) ?  $this->_cp['filter_category_master'] : false),
        'filter_groups_list' =>     (isset($this->_cp['filter_groups_list']) ?      $this->_cp['filter_groups_list'] : ''),
        'results_limit' =>          $this->_cp['results_limit'],
        'results_offset' =>         $this->_filter_offset,
        'results_order' =>          (isset($this->_cp['results_order']) ?           $this->_cp['results_order'] : 'NLast'),
      )
    );
  ...
  protected function _draw_object_map_html_setup(){
    ...
    $this->_show_home =        sanitize('range',get_var('show_home'),0,1,System::has_feature('show-home-address'));
    $this->_show_home_phone =  sanitize('range',get_var('show_home_phone'),0,1,System::has_feature('show-home-address'));
    $this->_show_work =        sanitize('range',get_var('show_work'),0,1,System::has_feature('show-work-address'));
    $this->_show_work_phone =  sanitize('range',get_var('show_work_phone'),0,1,System::has_feature('show-home-address'));
    if ($this->_current_user_rights['canEdit']){
    ...
    for($i=0; $i<count($lat_field_arr); $i++){
      ...
      if (
        ...
        if (trim($map_location=='')){
          $map_location =     $this->record['NFirst'].' '.$this->record['NLast']."\n".$this->record[$loc_field];
        }
        ...
        $map_location =
           ...
           .($this->_current_user_rights['canEdit'] ?
              "<a class='edit' href='".BASE_PATH."details/".$this->_form[$this->record['type']][0]."/".$this->record['ID']."'"
             ." onclick=\\\"details('".$this->_form[$this->record['type']][0]."','".$this->record['ID']."','".$this->_form[$this->record['type']][1]['h']."','".$this->_form[$this->record['type']][1]['w']."');return false;\\\">"
             ."[Edit]</a>"
            : ""
            )
           ."\n"
  ...
  public function do_email_signup($template_name='user_signup') {
    ...
    if (!$mailtemplateID){
      do_log(3,__CLASS__."::".__FUNCTION__.'()','(none)','Invalid template specified - \''.$template_name.'\'');
      return false;
    }
    $Obj_MT->_set_ID($mailtemplateID);
    $result = $Obj_MT->send_email($this->_get_ID());
    do_log((substr($result,0,11)=='Message-ID:' ? 0 : 2),__CLASS__."::".__FUNCTION__.'()','(none)','Signup for '.$this->_get_ID().' using template \''.$template_name.'\' (ID:'.$this->_get_ID().') - result: '.$result);
    return $result;
  }
  ...
  public function do_group_unsubscribe() {
    ...
    $parameter_spec = array(
      'groups' =>           array('default'=>'*',                   'hint'=>'group1,group2,group3|*'),
      'shadow' =>           array('default'=>'0',                   'hint'=>'0|1'),
      'steps' =>            array('default'=>'1',                   'hint'=>'1|2 (1:instant, 2:email confirm)'),
      'template_confirm' => array('default'=>'unsubscribe_confirm', 'hint'=>'template name - only for 2-step process'),
      'template_done' =>    array('default'=>'unsubscribe_done',    'hint'=>'template name')
    );
      ...
      case "unsubscribe":
        if ($targetID!='') {
          ...
          if (!$Obj->exists()) {
            $out.=
              status_message(
              2,true,'group','','There is no such group',$targetID);
          }
          else {
            $perms_arr =
              array(
    ...
    $out.=
      ...
      .($cp['steps']==1 ?
          ""
        : "<p>This is a two-step process - a confirmation email will be sent to allow you to confirm your choice.</p>"
       );
    if ($email=='') {
    ...
    $content =
      ...
      ."<div class='clr'>&nbsp;</div>\n"
      ."<div class='controls'>\n"
      ."<input type='submit' onclick=\"if (geid_val('email').indexOf('@')==-1) { alert('Please enter your email address');return false; };geid('btnSubmit').value='Please Wait';this.disabled=1;geid('command').value='new_password';geid('form').submit();\" value='Submit' style='formButton'/>"
      ."</div>\n";
    $out.=
    ...
    if ($email!='') {
      ...
      if (count($records)) {
        $out.=
           "<table cellpadding='2' cellspacing='0' border='1' bordercolor='#808080' style='border-collapse: collapse;'>\n"
          ."  <tr>\n"
  ...
  public function draw_forgotten_password($initialText='',$failureText='',$successText='',$shadow=0){
    ...
    $msg = "";
    if ($initialText=='') {
      $initialText = "<p>Use this form if you have forgotten your password. A new password will be created and emailed to you.</p>";
    }
    if ($failureText=='') {
      $failureText = "Sorry - we were unable to deliver your password to the address you gave.";
    }
    if ($successText=='') {
      $successText = "Success: your new password has been emailed to your address.";
    }
    switch ($command) {
      case "new_password":
        $Obj = new Captcha();
    ...
    $content =
      ...
      ."  <img class='formField' style='border:1px solid #7F9DB9;padding:2px;' src='./?command=captcha_img' alt='Verification Image' />\n"
      ."  <div class='clr'>&nbsp;</div>\n"
      ."  <label for='captcha_key' style='width:10em'>Verification Code</label>\n"
      ."  <input type='text' id='captcha_key' name='captcha_key' size='20' style='width: 180px;display:block;' value=\"\"/>\n"
      ."  <div class='clr'>&nbsp;</div>\n"
      ."  <div class='controls'>\n"
      ."    <input type='submit' value='Submit' id='btnSubmit' name='btnSubmit' style='formButton' "
      ."onclick=\"if (geid_val('email').indexOf('@')==-1) { alert('Please enter your email address');return false; };geid('btnSubmit').value='Please Wait';this.disabled=1;geid('command').value='new_password';geid('form').submit();\"/>"
      ."  </div>\n";
    Page::push_content('javascript_onload',"  geid('email').focus();\n");
    return
       ($msg ? "<span style='background-color:#ffe0e0; color:#ff0000; border: solid 1px #ff0000;padding-left:0.25em;padding-right:0.25em'>".$msg."</span>" : "")
      .$initialText
      .HTML::draw_form_box('Send New Password', $content, '_help_user_signin_existing_forgotten',$shadow);
  }
  ...
  public function draw_signup(
    ...
    $emailTo=0,$report_name='signup',$mail_template='user_signup',$shadow=false
  ){
    deprecated();
  ...
  public function export_sql($targetID,$show_fields){
    $header =       "Selected ".$this->_get_object_name().$this->plural($targetID)." with Group Membership records and Keywords";
    $extra_delete =
       "DELETE FROM `group_members`          WHERE `personID` IN (".$targetID.");\n"
      ."DELETE FROM `postings`               WHERE `type`='note' AND `personID` IN (".$targetID.");\n";
    $Obj = new Backup;
    $extra_select =
       $Obj->db_export_sql_query("`group_members`         ","SELECT * FROM `group_members` WHERE `personID` IN(".$targetID.");",$show_fields)
      .$Obj->db_export_sql_query("`postings`              ","SELECT * FROM `postings` WHERE `type`='note' AND `personID` IN(".$targetID.");",$show_fields);
...
//    y($result);
  ...
  public function get_notification_summary($datetime,$systemID,$base_url){
    ...
    foreach ($records as $record){
      ...
      $out.=
        ...
        ."    <td>".($record['PEmail'] ? "<a href=\"mailto:".$record['PEmail']."\">".$record['PEmail']."</a>" : "")."</td>\n"
  ...
  public static function get_permission($permission,$group_list=""){
    ...
    $EDITOR =       false;
    $VIEWER =       false;
    if ($group_list!="" && isset($_SESSION['person']['permissions'])) {  // find any additional permissions from session:
  ...
  public function get_person_to_session($username,$password_enc){
    ...
    if (preg_match('/^Mozilla\/4\.0 \(compatible; MSIE 6/', $_SERVER['HTTP_USER_AGENT']) && !preg_match('/\bopera/i', $_SERVER['HTTP_USER_AGENT'])){
...
//    y($vars);die;
    ...
    $sql =
      ...
      .($vars['filter_category']!="*" ?         "  `person`.`category` REGEXP \"".implode("|",explode(',',$vars['filter_category']))."\" AND\n": "")
      .($vars['filter_category_master'] ?       "  `person`.`category` REGEXP \"".implode("|",explode(',',$vars['filter_category_master']))."\" AND\n": "")
      .($vars['filter_groups_list']!="" ?       "  `person`.`ID` IN(SELECT `personID` FROM `group_members` `gm` WHERE `gm`.`groupID` IN(".implode($group_IDs).")) AND\n" : "")
      ."  1\n";
    $records = $this->get_records_for_sql($sql);
    if ($records === false) {
      return;
  ...
  public function get_records_pending_map_update($limit=false){
    $sql =
       "SELECT\n"
      ."  `ID`,\n"
      ."  `ID`,\n"
  ...
  public static function get_user_status() {
    ...
    $out =    array();
    $out[] =	"SYSLOGON";
    if ($_SESSION['person']['permMASTERADMIN'])   $out[] = "MASTERADMIN";
    if ($_SESSION['person']['permUSERADMIN'])     $out[] = "USERADMIN";
    if ($_SESSION['person']['permSYSADMIN'])      $out[] = "SYSADMIN";
    if ($_SESSION['person']['permSYSAPPROVER'])   $out[] = "SYSAPPROVER";
    if ($_SESSION['person']['permSYSEDITOR'])     $out[] = "SYSEDITOR";
    if ($_SESSION['person']['permSYSMEMBER'])     $out[] = "SYSMEMBER";
    if ($_SESSION['person']['permSYSLOGON'])      $out[] = "SYSLOGON";
    if ($_SESSION['person']['permGROUPEDITOR'])   $out[] = "GROUPEDITOR";
    if ($_SESSION['person']['permGROUPVIEWER'])   $out[] = "GROUPVIEWER";
    return implode($out,"|");
  }
  ...
  public static function get_user_status_text() {
    ...
    $out = array();
    if ($_SESSION['person']['permMASTERADMIN'])   $out[] = "Master Administrator";
    if ($_SESSION['person']['permUSERADMIN'])     $out[] = "User Administrator";
    if ($_SESSION['person']['permSYSADMIN'])      $out[] = "Site Administrator";
    if ($_SESSION['person']['permSYSAPPROVER'])   $out[] = "Site Approver";
    if ($_SESSION['person']['permSYSEDITOR'])     $out[] = "Site Editor";
    if ($_SESSION['person']['permSYSMEMBER'])     $out[] = "Site Member";
    if ($_SESSION['person']['permSYSLOGON'])      $out[] = "Site User";
    if ($_SESSION['person']['permGROUPEDITOR'])   $out[] = "Group Editor";
    if ($_SESSION['person']['permGROUPVIEWER'])   $out[] = "Group Viewer";
    return implode($out,", ");
  }
  ...
  public function handle_report_delete(&$msg) {
    ...
    $msg =
      status_message(
        2,true,"selected ".$this->_get_object_name(),''," include ".$num_locked." whose ".($num_locked==1 ? 'profile has' : 'profiles have')." been locked - your deletion has therefore been cancelled.",$targetID
      );
    return false;
  }
  ...
  public function manage_emails(){
    ...
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No Email history - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
  ...
  public function manage_groups(){
    ...
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No member Groups - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
  ...
  public function manage_notes(){
    ...
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em 0'>No Notes - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
      ...
      $out.=
        ...
        ."<img class='fl' src='".BASE_PATH."img/sysimg/icon_toolbar_end_left.gif' style='height:16px;width:6px;padding-left:2px;' alt='|' />"
        ."<div class='toolbar_text' style='float:left;margin-right:0.25em'><b>Add new</b></a></div>";
      foreach($subtypes as $subtype){
        $out.=
           "<a href=\"#\" onclick=\"details('person_notes','','560','960','',".get_var('selectID').",0,'subtype=".$subtype['value']."')\">"
           .$subtype['content']
           ."</a>";
      }
      $out.=
  ...
  public function manage_orders(){
    ...
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No associated Orders - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
        ...
        $data['AMap_description'] =
          trim(
            str_replace(
              ...
              .($r['ACountryID']=='CAN' ? '' : (isset($countries[$r['ACountryID']]) ? "\r\n".$countries[$r['ACountryID']] : ''))
            )
          );
      }
        ...
        $data['AMap_location'] =
          trim(
            str_replace(
              ...
              ." ".$r['ASpID']." ".$r['APostal']
              .($r['ASpID']=='PR' ? '' : (isset($countries[$r['ACountryID']]) ? ' '.$countries[$r['ACountryID']] : ''))
            )," "
          );
        ...
        $data['WMap_description'] =
          trim(
            str_replace(
              ...
              .($r['WCountryID']=='CAN' ? '' : (isset($countries[$r['WCountryID']]) ? "\r\n".$countries[$r['WCountryID']] : ''))
            )
          );
      }
        ...
        $data['WMap_location'] =
          trim(
            str_replace(
              ...
              ." ".$r['WSpID']." ".$r['WPostal']
              .($r['WSpID']=='PR' ? '' : (isset($countries[$r['WCountryID']]) ? ' '.$countries[$r['WCountryID']] : ''))
            )," "
          );
        $WMap_location_changed = true;
      }
  ...
  public static function send_email_for_new_user(){
    ...
    if (!$templateID){
      do_log(3,__CLASS__."::".__FUNCTION__.'()','(none)','Invalid template specified in component - '.$template_name);
      return false;
    }
    $Obj_Mail_Template->_set_ID($templateID);
    $result = $Obj_Mail_Template->send_email($personID);
    do_log(1,__CLASS__."::".__FUNCTION__.'()','(none)','Signup for '.$personID.' using template '.$template_name.' - result: '.$result);
