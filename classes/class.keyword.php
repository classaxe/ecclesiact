...
class Keyword extends Record
{
    ...
    public function export_sql($targetID, $show_fields)
    {
        ...
        $extra_select = $Obj->db_export_sql_query(
            "`keyword_assign`        ",
            "SELECT * FROM `keyword_assign` WHERE `keywordID` IN(".$targetID.") ORDER BY `keywordID`;",
            $show_fields
        )."\n";
        return  $this->sql_export($targetID, $show_fields, $header, '', $extra_delete, $extra_select);
    }

    public static function get_keywordIDs_list_by_keywords_list($keywords)
    {
    ...
    public static function get_keywords_list_by_IDs_list($keywordIDs)
    {
        $sql =
             "SELECT\n"
    ...
    public static function get_keyword_list_with_weight($systemIDs_csv = false, $all_sites = false, $min = 1)
    {
    ...
    public static function get_related($ID_csv, $systemIDs_csv, $this_type, $this_ID, $limit, &$related_matches_total)
    {
        ...
        $sql =
            ...
            ."  COALESCE(\n"
            ."    `pa`.`history_created_date`,\n"
            ."    `po`.`history_created_date`,\n"
            ."    `pr`.`history_created_date`\n"
            ...
            ."  `ka`.`assign_type` IN(\n"
            ."     'article','event','gallery-album','gallery-image','news item','job posting','podcast'\n"
            ."  ) AND\n"
            ."  `ka`.`assignID` = `po`.`ID`\n"
  ...
  //    z($sql);
    ...
    public function handle_report_delete(&$msg)
    {
        ...
        $msg =    status_message(
            ...
            " <b>".$keywords."</b> and ".$usage." linked reference".($usage==1 ? " has" : "s have"),
            'been deleted.',
            $targetID
        );
    }
