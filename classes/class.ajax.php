...
class Ajax extends Base
{
    ...
    public static function get_config_rows()
    {
        static $config;
        if (!isset($config)) {
    ...
    public static function get_report(
        ...
        if (trim(substr($_report_filter, 0, 9)=='<options>')) {
            ...
            foreach ($doc->option as $o) {
                ...
                $modes[] =  ((string)$o->mode ? Report::get_match_mode((string)$o->mode) : $_report_matchmode);
            }
        } else {
    ...
    public static function get_report_heads($columns)
    {
        if ($columns===false) {
            return;
        }
    ...
    public static function get_report_data($columns)
    {
        if ($columns===false) {
            return;
        }
    ...
    public static function get_shipping($method, $width, $id, $cp)
    {
        ...
        switch ($method) {
            case 'FEDEX':
                Output::push(
                    ...
                    ."  geid('ajax_result_fees_shipping_cost').style.backgroundColor ="
                    ." (result['error'] ? '#ffe8e8' : '#f0f0f0');\n"
                ...
                return
                    ...
                    ."<img id='ajax_result_".$id."_wait' class='ajax_wait' style='display:none' align='top'"
                    ." src='".BASE_PATH."img/sysimg/icon_ajax_wait.gif' alt='Please wait...' />&nbsp;\n"
                    ."  </div>\n"
                    ."  <div id=\"ajax_result_".$id."_method\" class='fl txt_r' style='width:"
                    .(int)($width-89)
                    ."px;'>&nbsp;\n"
                    ."  </div>"
  ...
  //    y($out);die;
    ...
    protected static function serve_lookup_report($reportID, $filterField = '', $filterExact = '', $filterValue = '')
    {
        ...
        $count = $Obj_Report->get_records_count(
            $report_record,
            $filterField,
            $filterExact,
            $filterValue,
