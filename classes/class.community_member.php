<?php
define('COMMUNITY_MEMBER_VERSION','1.0.104');
/*
Custom Fields used:
...
"[
  ...
  ['#module_community_profile_settings #module_community_iframe_avatar',{
    frameborder:        '0px',
    margin:             '0px',
    padding:            '0px'
  }]";
  ...
  public function __construct($ID="") {
    ...
    $this->_dashboard_url = BASE_PATH.'_popup_layer/community_member_dashboard/'.self::dashboard_width.'/'.self::dashboard_height.'/';
  ...
  public function do_dashboard($selected=''){
    ...
    $severity = 1;
    $abort = false;
    $memberID = (isset($_SESSION['person']) && isset($_SESSION['person']['memberID']) ? $_SESSION['person']['memberID'] : 0);
    if (!$memberID){
      return
        array(
          'html' =>
    ...
    switch ($submode){
      case "module_community_avatar":
        print $this->draw_avatar();
        die;
      break;
      case "module_community_avatar_save":
        $this->_set_ID($memberID);
        $data = array();
        if (isset($_FILES['avatar']) && is_uploaded_file($_FILES['avatar']['tmp_name'])){
          $path =       'UserFiles/Image/member_photos/';
          if(!file_exists($path)){
            $msg = "<b>Error</b>: - path ".$path." is invalid - please contact site administrator.";
            $severity = 2;
            $abort = true;
          }
          else {
            $ext_arr =  explode('.',$_FILES['avatar']['name']);
            ...
            switch ($ext){
              ...
              break;
              default:
                $msg = "<b>Error</b>: - file type ".$ext." is not a valid type - please try another";
                $severity = 2;
                $abort = true;
              break;
            }
          ...
          if (!$abort){
            $new_name = $path.$memberID.'_'.get_path_safe_filename($_FILES['avatar']['name']);
            $avatar =   $this->get_field('avatar');
            $parts =    $this->get_embedded_file_properties($avatar);
            $old_name = ".".substr($parts['data'],strlen('url('),-1);
            if ($old_name!='.' && file_exists($old_name)){
              unlink($old_name);
            }
            $msg = "<b>Please wait</b>: Saving profile and processing uploaded image";
            $severity = 1;
            if(file_exists($new_name)){
              unlink($new_name);
            }
            rename($_FILES['avatar']['tmp_name'],$new_name);
            $data['avatar'] =
            addslashes(
        ...
        else if (isset($_REQUEST['avatar_mark_delete']) && $_REQUEST['avatar_mark_delete']=='1'){
          $data['avatar'] = '';
          $avatar =   $this->get_field('avatar');
          $parts =    $this->get_embedded_file_properties($avatar);
          $file =     ".".substr($parts['data'],strlen('url('),-1);
          if (file_exists($file)){
            unlink($file);
          }
          $msg = "<b>Please wait</b>: Saving profile and removing previously uploaded image.";
          $severity = 1;
        }
        $this->update($data);
        print $this->draw_avatar($msg, $severity);
        die;
      break;
  ...
  public function draw_avatar($msg='',$severity=1){
    global $system_vars;
    $submode = (isset($_REQUEST['submode']) ? $_REQUEST['submode'] : "");
    header('Content-Type: text/html');
    $memberID = $_SESSION['person']['memberID'];
    $Obj_Report = new Report;
    $reportID = $Obj_Report->get_ID_by_name('community.members');
    $Obj_System = new System;
    $this->_set_ID($memberID);
    $record = $this->get_record();
    $avatar = false;
    if ($record['avatar']) {
      $parts = $this->get_embedded_file_properties($record['avatar']);
      $avatar =
         BASE_PATH
        ."img/max/100".substr($parts['data'],strlen('url('),-1);
    }
    $field_width = 470;
    $out =
       "<html xmlns=\"http://www.w3.org/1999/xhtml\" xml:lang=\"en\">\n"
      ."<head>\r\n"
      ."<title>Community Module Avatar</title>\r\n"
      ...
      ."<style type='text/css'>\n"
      ."body { margin: 0px; padding: 5px; font-size: 80%; background-color: #D4D6FF; }\n"
      ."form { margin: 0; padding: 0; }\n"
      ."#no_avatar {\n"
      ."  width:100px; height:60px; font-size:12pt; border:1px solid #000;\n"
      ...
      ."</head>\n"
      ."<body>\n";
    if ($submode=="module_community_avatar_save"){
      $out.=
         "<h1 style='font-size:12pt;text-align:center;line-height:75px;'>Processing... Please wait</h1>"
        ."<script type='text/javascript'>"
        ."parent.save_community_settings(\"".$msg."\",".$severity.");\n"
        .($severity == 2 ? "alert(\"".strip_tags($msg)."\");\n" : "")
        ."</script>"
        ."</body></html>";
      return $out;
    }
    $out.=
      ...
      ." action='"
      .$this->_dashboard_url
      ."?submode=module_community_avatar_save'>\n"
      .HTML::draw_status('module_community_avatar_msg',$msg)
      ."<div style='float:left;width:105px;height:80px'>\n"
      .($record['avatar'] ?
          "<img src=\"".$avatar."\" alt=\"Your current Avatar\" />"
        : "<div id='no_avatar'>Please upload an image</div>"
       )
      ."</div>"
      .($record['avatar'] ? "" : "<div style='float:left;width:340px;'>&nbsp;Please upload a <b>.jpg</b>, <b>.gif</b> or <b>.png</b> photo for your pages</div>")
      .Report_Column::draw_form_field($record,'avatar',$record['avatar'],'file_upload_to_userfile_folder',
        $field_width-($avatar ? 140 : 140),'',$reportID," onchange=\"this.style.display='block'\"",0,0,'','Image/member_photos/')."<br />"
      .($record['avatar'] ? "<div style='float:left;width:320px;text-align:center'>Image should be a <b>.jpg</b>, <b>.gif</b> or <b>.png</b> file</div>" : "")
      ."</div>\n"
      ."<div class='clr_b'></div></form></body>\n"
      ."</html>";
    return $out;
  }
  ...
  public function draw_dashboard($selected='') {
    ...
    $report =
      array(
        'articles' =>   draw_auto_report('community_member.dashboard.articles', $toolbar, $this->_dashboard_url.'articles'),
        'events' =>     draw_auto_report('community_member.dashboard.events', $toolbar, $this->_dashboard_url.'events'),
        'news-items' => draw_auto_report('community_member.dashboard.news-items', $toolbar, $this->_dashboard_url.'news-items'),
        'podcasts' =>   draw_auto_report('community_member.dashboard.podcasts', $toolbar, $this->_dashboard_url.'podcasts')
      );
    $popup =
      array(
    ...
    $out['js'] =
      ...
      ."    obj.onmouseout  = function(e){img_state_v('tab_img_'+id,(id==global_active_tab ? 'a' : 'n'));return false;};\n"
      ."  }\n"
      ."  else {\n"
      ...
      ."}\n"
      ."window.save_community_settings_avatar = function(){\n"
      ."  geid('module_community_iframe_avatar').contentWindow.document.getElementById('form_avatar').submit();\n"
      ."}\n"
    ...
    $out['html'].=
      ...
      ."<a href=\"javascript:details('community_member.dashboard.articles','','".$popup['articles']['h']."','".$popup['articles']['w']."','','')\">[ICON]12 12 3512 Add New Article[/ICON]</a></div><br class='clr_b' />\n"
      ...
      ."      <div class='fl' style='padding:3px 0 0 10px'>"
      ."<a href=\"javascript:details('community_member.dashboard.events','','".$popup['events']['h']."','".$popup['events']['w']."','','')\">[ICON]12 12 3512 Add New Event[/ICON]</a></div><br class='clr_b' />\n"
      ."      <div style='overflow:auto;height:320px;'>"
      ...
      ."      <div class='fl' style='padding:3px 0 0 10px'>"
      ."<a href=\"javascript:details('community_member.dashboard.news-items','','".$popup['news-items']['h']."','".$popup['news-items']['w']."','','')\">[ICON]12 12 3512 Add New Event[/ICON]</a></div><br class='clr_b' />\n"
      ."      <div style='overflow:auto;height:320px;'>"
      ...
      ."      <div class='fl' style='padding:3px 0 0 10px'>"
      ."<a href=\"javascript:details('community_member.dashboard.podcasts','','".$popup['podcasts']['h']."','".$popup['podcasts']['w']."','','')\">[ICON]12 12 3512 Add New Podcast[/ICON]</a></div><br class='clr_b' />\n"
      ."      <div style='overflow:auto;height:320px;'>"
      ...
      ."  </div>\n"
      ."  <div class='tabs'>\n"
      ."    <a id='tab_settings' href='#settings'><img id='tab_img_settings'  src='/img/spacer' style='height:68px;background-position:0% 0px'       title='View your Community Settings' /></a>\n"
      ."    <a id='tab_articles' href='#articles'><img id='tab_img_articles'  src='/img/spacer' style='height:69px;background-position:-200% -68px'  title='Manage Articles' /></a>\n"
      ."    <a id='tab_events'   href='#events'><img id='tab_img_events'      src='/img/spacer' style='height:66px;background-position:-200% -137px' title='Manage Events' /></a>\n"
      ."    <a id='tab_news'     href='#news'><img id='tab_img_news'          src='/img/spacer' style='height:55px;background-position:-200% -203px' title='Manage News Items' /></a>\n"
      ."    <a id='tab_audio'    href='#podcasts'><img id='tab_img_audio'     src='/img/spacer' style='height:75px;background-position:-200% -258px' title='Manage Podcasts' /></a>\n"
      ."  </div>\n"
      ."    <input style='width:73px;left:8px;position:relative;top:350px;' type='button' onclick='hidePopWin(null)' value='Close' class='formButton' />\n"
      ."  <div class='clr_b'></div>"
      ."</div>";
    $out['html'] = convert_safe_to_php($out['html']);
    foreach (Page::$css_colors as $idx=>$style){
      $this->module_community_member_dashboard_css.=
        "['#module_community .color_".$idx."',{ color: '#".$style['color']."', backgroundColor: '#".$style['bgcolor']."'}],";
    }
  ...
  private function _draw_dashboard_settings(){
    ...
    $out =
      ...
      .HTML::draw_section_tabs($section_tabs_arr,'module_community',$selected_section,"status_message_hide('form_status_module_community_settings')")
      ...
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Denomination','May be shown on your community pages depending on your other settings','custom_1',$label_width)
      .draw_form_field('custom_1',$record['custom_1'],'combo_listdata',$field_width,'','','',0,0,'','module.community.lst_denomination')
      ."<div class='clr_b'></div>"
/*
      .Report_Column::draw_label('Languages Spoken','May be shown on your community pages depending on your other settings','custom_1',$label_width)
      .draw_form_field('custom_1',$record['custom_2'],'selector_listdata_csv',$field_width,'','','',0,0,'','module.community.lst_languages')
      ."<div class='clr_b'></div>"
*/
      .Report_Column::draw_label('Address','Will be shown on your site','AAddress1',$label_width)
      .draw_form_field('AAddress1',$record['AAddress1'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('&nbsp;','','',$label_width)
      .draw_form_field('AAddress2',$record['AAddress2'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Town / City','Will be shown on your site','ACity',$label_width)
      .draw_form_field('ACity',$record['ACity'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('State / Province','Will be shown on your site','AStateProvince',$label_width)
      .draw_form_field('AStateProvince',$record['AStateProvince'],'combo_listdata',$field_width,'','','',0,0,'','lst_sp')
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Postal Code','Will be shown on your site','APostal',$label_width)
      .draw_form_field('APostal',$record['APostal'],'text',60)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Country','Will be shown on your site','ACountryID',$label_width)
      .draw_form_field('ACountryID',$record['ACountryID'],'selector_listdata',$field_width,'','','',0,0,'','lst_country')
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Map Location','Used to position marker on map','map_location',$label_width)
      .draw_form_field('map_location',$record['map_location'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Latitude','Set for you automatically from Map Location','map_lat',$label_width)
      ."<div style='float:left;width:60px;border:1px solid #ccc;background-color:#f0f0f0;'>".$record['map_lat']."</div>"
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Longitude','Set for you automatically from Map Location','map_lon',$label_width)
      ."<div style='float:left;width:60px;border:1px solid #ccc;background-color:#f0f0f0;'>".$record['map_lon']."</div>"
      ."<div class='clr_b'></div>"
      ."<br />\n"
      .Report_Column::draw_label('Church Photo','Photo to be shown on your community pages<br/>Should be .jpg, .png or .gif (will be resized)','avatar',$label_width)
      ."<iframe id='module_community_iframe_avatar' name='module_community_iframe_avatar' style='float:left;width:".($field_width+5)."px;height:95px;border:1px solid #486CAE' frameborder='0' marginheight='0' marginwidth='0' src='".$this->_dashboard_url."settings?submode=module_community_avatar'>"
    ...
    foreach($days as $day){
      $out.=
        ...
        .draw_form_field('weekly_'.strToLower(substr($day,0,3)),(isset($record['weekly_'.strToLower(substr($day,0,3))]) ? $record['weekly_'.strToLower(substr($day,0,3))] : ""),'textarea',$field_width,'',0,'',0,0,'','',40)
        ."<div class='clr_b'></div>";
    }
    $out.=
       "</div>"
      .draw_section_tab_div('dashboard_contact',$selected_section)
      .Report_Column::draw_label('Telephone','Will be shown on your site','ATelephone',$label_width)
      .draw_form_field('ATelephone',$record['ATelephone'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Official Website','Will be shown on your site if given','AWeb',$label_width)
      .draw_form_field('AWeb',$record['AWeb'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Facebook Page','Will be shown on your site if given','AFacebook',$label_width)
      .draw_form_field('AFacebook',$record['AFacebook'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Twitter Handle','Will be shown on your site if given','ATwitter',$label_width)
      .draw_form_field('ATwitter',$record['ATwitter'],'text',$field_width)
      ."<div class='clr_b'></div>"
      .Report_Column::draw_label('Youtube Channel','Will be shown on your site if given','AYoutube',$label_width)
      .draw_form_field('AYoutube',$record['AYoutube'],'text',$field_width)
      ."<div class='clr_b'></div>"
    ...
    foreach($days as $day){
      $out.=
        ...
        .draw_form_field('xml:Church_Office_'.substr($day,0,3),(isset($record['xml:Church_Office_'.substr($day,0,3)]) ? $record['xml:Church_Office_'.substr($day,0,3)] : ""),'text',$field_width)
        ."<div class='clr_b'></div>";
    }
    $out.=
       "<div class='txt_c' style='margin:5px;width:".($label_width+$field_width)."px;'><b>Contact form Names and Email addresses</b></div>"
      ."<div class='contact_addresses'>\n"
      ."<div class='clr_b'></div>";
    for ($i=1; $i<=5; $i++){
      $out.=
         Report_Column::draw_label('Name #'.$i,'Will be used for Contact form','xml:contact:name_'.$i,$label_width)
        ."<div class='fl'>".draw_form_field('xml:contact:name_'.$i,(isset($record['xml:contact:name_'.$i]) ? $record['xml:contact:name_'.$i] : ""),'text',(int)($field_width/2)-35)."</div>"
        .Report_Column::draw_label('Email','','xml:contact:email_'.$i)
        ."<div class='fl'>".draw_form_field('xml:contact:email_'.$i,(isset($record['xml:contact:email_'.$i]) ? $record['xml:contact:email_'.$i] : ""),'text',(int)($field_width/2)-35)."</div>"
        ."<div class='clr_b'></div>";
    }
    $out.=
       "</div>"
      ...
      .Report_Column::draw_label('Partners','Accept shared postings from these members\nChoose at least the number of partners specified in \'Minimum Partners\' to qualify for this service','partner_csv',$label_width)
      ."<div id='div_partner_csv'>"
      .draw_form_field('partner_csv',$record['partner_csv'],'checkbox_sql_csv',$field_width,$partner_sql,'','',0,0,'','',260)."<br />"
      ."</div>"
      ."</div>"
      ."</div>"
      ."<div style='width:60px;height:25px;margin:auto'>\n"
      ."  <input type='button' id='module_community_update' class='formButton' value='Update'"
      ." onclick=\"this.disabled=true;javascript:save_community_settings_avatar();\" />\n"
      ."</div>\n"
      ;
    return $out;
  }
  ...
  public function export_sql($targetID,$show_fields){
    ...
    $extra_delete =
       "DELETE FROM `person`                 WHERE `ID` IN (SELECT `contactID` FROM `community_member` WHERE `ID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.");\n"
      ."DELETE FROM `community_membership`   WHERE `memberID` IN (".$targetID.");\n"
      ."DELETE FROM `category_assign`        WHERE `assignID` IN (SELECT `ID` FROM `postings` where `memberID` IN(".$targetID."));\n"
      ."DELETE FROM `postings`               WHERE `memberID` IN (".$targetID.");\n"
      ;
    $extra_select =
       Backup::db_export_sql_query("`community_membership`  ","SELECT * FROM `community_membership` WHERE `memberID` IN(".$targetID.") ORDER BY `memberID`",$show_fields)
      .Backup::db_export_sql_query("`category_assign`       ","SELECT * FROM `category_assign` WHERE `assignID` IN (SELECT `ID` FROM `postings` where `memberID` IN(".$targetID."));\n",$show_fields)
      .Backup::db_export_sql_query("`postings`              ","SELECT * FROM `postings` WHERE `memberID` IN(".$targetID.") ORDER BY `memberID`",$show_fields)
      .Backup::db_export_sql_query("`person`                ","SELECT * FROM `person` WHERE `ID` IN (SELECT `contactID` FROM `community_member` WHERE `ID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      ;
  ...
  public function get_member_profile($community_name,$member_name){
    $sql =
      ...
      ."  COALESCE((SELECT `shortform_name` FROM `community_member` `cm2` WHERE `cm2`.`ID` = `community_member`.`primary_ministerialID`),'') `ministerial_title`,\n"
      ."  COALESCE((SELECT `name` FROM `community_member` `cm2` WHERE `cm2`.`ID` = `community_member`.`primary_ministerialID`),'') `ministerial_name`,\n"
      ."  `community_member`.*\n"
      ."FROM\n"
      ."  `community`\n"
  ...
  public function get_events_upcoming($category_csv=false){
    $sql =
      ...
      ."  `systemID`=".SYS_ID." AND\n"
      ."  `type`='event' AND\n"
      .($category_csv!="" ? "  `category` REGEXP \"".implode("|",explode(',',str_replace(" ","",$category_csv)))."\" AND\n": "")
  ...
  public function get_partner_selector_sql($memberID){
    ...
    foreach ($communities as $community){
      $out.=
        ...
        ."  `community_member`.`ID`,\n"
        ."  CONCAT(\n"
        ."     '<a href=\"',`community`.`URL`,`community_member`.`name`,'\"',"
        ."     ' title=\"Denomination: ',`custom_1`,'\" style=\"color:#00f\" target=\"_blank\">',\n"
        ."     `community_member`.`title`,\n"
        ."     '</a> <span style=\"font-weight:normal\"> - ',\n"
        ."     `ACity`,', ',\n"
        ."     IF(`AStateProvince`!='',CONCAT(`AStateProvince`,', '),''),\n"
        ."     `ACountryID`,\n"
        ."     '</span>'\n"
        ."   ),\n"
        ."  'ffffff',\n"
  ...
  public function get_stats(){
    ...
    $find =     BASE_PATH.trim($this->_community_record['URL'],'/').'|'.BASE_PATH.trim($this->_community_record['URL'],'/').'/'.trim($r['name'],'/');
    $links =    array();
    if ($r['link_website']){  $links[] = $r['link_website']; }
    if ($r['link_facebook']){ $links[] = $r['link_facebook']; }
    if ($r['link_twitter']){  $links[] = $r['link_twitter']; }
    if ($r['link_video']){    $links[] = $r['link_video']; }
    $links =    implode('|',$links);
    if ($r['stats_cache']){
  ...
  protected function _on_action_member_setup_contact(){
    ...
    if ($r['contactID']=='0'){
      ...
      $data = array();
      if ($r['contact_NTitle']=='' && $r['contact_NFirst']=='' && $r['contact_NMiddle']=='' && $r['contact_NLast']=='' && $r['contact_NGreeting']==''){
  ...
  protected function _on_action_member_setup_map($prefix){
    ...
    if ($r[$prefix.'map_desc']=='' && ($r[$prefix.'addr_line1']||$r[$prefix.'addr_line2']||$r[$prefix.'addr_city']||$r[$prefix.'addr_postal'])){
    ...
    if ($r[$prefix.'map_loc']=='' && ($r[$prefix.'addr_line1']||$r[$prefix.'addr_line2']||$r[$prefix.'addr_city']||$r[$prefix.'addr_postal'])){
      $data[$prefix.'map_loc'] =
        trim(
          str_replace(
            "  ",
  ...
  public function set_up_member_page_vars(&$page_vars){
    ...
    if (!$this->_record){
      $page_vars['content'] = "<h2>Sorry!</h2><p>The <b>".$community_title."</b> community does not recognise this member.";
      return;
    }
