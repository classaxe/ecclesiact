...
class Community_Member extends Displayable_Item
{
  ...
  ['#module_community div.tabs img',{
    ...
    width:              '83px',
    backgroundImage:    'url(%BASE_PATH%img/sysimg/?img=module_community/dashboard_tabs.png)'
  }],
  ...
  ['#module_community_profile_settings .section_tabs',{
    ...
    public function __construct($ID = "")
    {
        ...
        $this->module_community_member_dashboard_css =
            str_replace(
                array('#WIDTH#','#HEIGHT#','%BASE_PATH%'),
                array(self::DASHBOARD_WIDTH, self::DASHBOARD_HEIGHT, BASE_PATH),
                $this->module_community_member_dashboard_css
            );
...
*/
    ...
    public function get_member_profile_images()
    {
        ...
        if (!$result) {
            ...
            if ($ID = $Obj_GA->get_ID_by_path($path)) {
                $result = $Obj_GA->get_images($ID);
            }
        }
        return $result;
    }

    protected function getLatestEntry($field) {
        if (!isset($this->_record[$field])) {
            return '';
        }
        if (!$this->_record[$field]) {
            return '';
        }
        $entries = explode('|', $this->_record[$field]);
        return array_pop($entries);
    }

    public function getLinkAttributes($type)
    {
        $url =   $this->getLatestEntry('link_'.$type);
        switch ($type) {
            case 'facebook':
                $url_s =    substr($url, 2+strpos($url, '//'));
                $icon =     '[ICON]14 20 3147 Facebook[/ICON]';
                $short =    'FB';
                $long =     'Facebook';
                break;
            case 'twitter':
                $url_s =    substr($url, strpos($url, '@'));
                $icon =     '[ICON]14 20 5420 Twitter[/ICON]';
                $short =    'TW';
                $long =     'Twitter';
                break;
            case 'website':
                $url_s =    substr($url,2+strpos($url, '//'));
                $icon =     '[ICON]16 20 800 Web Site[/ICON]';
                $short =    'Web';
                $long =     'Web Site';
                break;
            case 'video':
                $url_s =    substr($url, 2+strpos($url, '//'));
                $icon =     '[ICON]16 20 7982 Video[/ICON]';
                $short =    'Video';
                $long =     'Video';
                ...
                if (count($url_bits)>2) {
                    switch ($url_bits[2]) {
                        ...
                        case 'new.livestream.com':
                            $icon =     "[ICON]16 20 7950 Livestream[/ICON]";
                            $short =    "LS";
                            $long =     "Livestream";
                            break;
                        case 'ustream.tv':
                        case 'www.ustream.tv':
                            $icon =     "[ICON]16 20 7966 UStream[/ICON]";
                            $short =    "US";
                            $long =     "UStream";
                        break;
                        case 'vimeo.com':
                        case 'www.vimeo.com':
                            $icon =     "[ICON]16 20 7934 Vimeo[/ICON]";
                            $short =    "Vi";
                            $long =     "Vimeo";
                        break;
                        case 'youtube.com':
                        case 'www.youtube.com':
                            $icon =     "[ICON]16 20 6154 Youtube[/ICON]";
                            $short =    "YT";
                            $long =     "Youtube";
                        break;
                    }
    ...
    public function get_coords($address = false)
    {
        ...
        'office_map_geocode_address' =>    $o['match_address'],
        ...
        'service_map_geocode_address' =>   $s['match_address'],
    ...
    public function get_stats()
    {
        ...
        $find_arr =             [
            $communityURL.'/'.trim($r['name'])
        ];
        if ($r['name_aliases']) {
            $aliases = explode(',', $r['name_aliases']);
            foreach ($aliases as $alias) {
                $find_arr[] =   $communityURL.'/'.trim($alias);
            }
        }
