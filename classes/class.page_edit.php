<?php
define('VERSION_PAGE_EDIT','1.0.15');
/*
Version History:
  1.0.15 (2013-10-04)
...
class Page_Edit extends Page {
  function draw() {
    ...
    global $systemID, $page_name, $page, $path_extender, $title, $subtitle, $content, $keywords, $include_title_heading;
    ...
    global $permPUBLIC, $permSYSLOGON, $permSYSMEMBER, $locked, $comments_allow, $comments_count, $ratings_allow;
    ...
    $page_allow_memberID =          System::has_feature('module-community') && ($isAPPROVER || $isSYSADMIN || $isMASTERADMIN);
    ...
    switch ($submode) {
      ...
      case 'save_and_close':
        ...
        // Yellow fever correction:
        $pattern = '/<span contenteditable="false" style="cursor: default; color: rgb\(0,0,0\); background-color: rgb\(255,255,0\)">([^<]+)<\/span>/';
        ...
        $Obj_RC->bulk_update($data, $bulk_update, 'component_parameters', implode(option_separator,explode("\r\n",$component_parameters)));
        $Obj_RC->bulk_update($data, $bulk_update, 'content', $content);
        $Obj_RC->bulk_update($data, $bulk_update, 'content_text', preg_replace("/\[ECL\]([^\[])*(\[\/ECL\])*/","",strip_tags($content)));
        ...
        if ($ID===false) {
          if (Record::get_last_db_error_msg_generic()=='DUPLICATE_ENTRY') {
            $msg = "<b>Error:</b> A page already exists with the name <b>".$page_name."</b> and the same parent on this site. Please use a different name or parent.";
            $ID="";
            $page = $page_name;
          }
        }
          ...
          // ********************************
          // * Post-operation phase Actions *
          // ********************************
          // Execute report actions if required:
          if (isset($action) && $action=="insert") {
            $ObjReport->actions_execute('report_insert_post',$this->_get_table_name(),'Page',$ID,$_POST);
          }
          else {
            $ObjReport->actions_execute('report_update_post',$this->_get_table_name(),'Page',$ID,$_POST);
          }
          if (!$bulk_update) {
            if ($isAPPROVER) {
              $this->_set_ID($ID);
    ...
    $out =	"";
    $js = Report_Form::_get_js_form_code($mode,$submode,$ID,$msg,$report_name,$selectID,(isset($action) ? $action : false));
    if ($js!='') {
    ...
    if ($ID!="" && !$bulk_update){
      ...
      $systemID  =              SYS_ID;
      $page_name =              ($page!='' || $bulk_update ? $page : $this->get_unique_name('new page',$systemID));
      $ratings_allow =          'none';
      $themeID =                1; // Set to use system default
      $title = get_title_for_path($page_name);
      $content =
         ($page_no_default_content ?
            $content
          :
             ($page_include_title_heading ? "" : "<h1>Main Heading</h1>\n")
            ."<p>This is where you add content for the new page.\n"
            ."<ul>\n"
            ."  <li>Set <b><span style='BORDER: #808080 1px solid; BACKGROUND-COLOR: #c0c0f0;PADDING: 2px;'>Permissions</span></b> "
            ."to decide who has access to this page otherwise no-one will see it, assuming your account allows you to do this.</li>\n"
            ."  <li><b>Don't change the font size</b> in this editor panel if you want to allow visitors to resize text using the text resizer "
            ...
            ."<li>If you want to make a heading, select the appropriate <b>Heading</b> level in the Format dropdown box instead.<br />"
            ."Search engines rank sites that use headings correctly higher than those which simply vary text size to indicate headings.</li>\n"
            ."</ul>"
         );
    }
    ...
    $out.=
      ...
      ."              <tr>\n"
      ."                <td><table cellpadding='1' border='0' cellspacing='0' class='auto_report admin_containerpanel'>\n"
      .($isMASTERADMIN ?
           "               <tr><td class='nowrap'><div class='lbl' style='width: 60px;'><label for='systemID'>Site</label></div></td>\n"
          ."                    <td>"
          .draw_form_field("systemID",$systemID,"selector","760",$formSelectorSystemIDSQL,'','','',$bulk_update)
          ."  </td></tr>\n"
       :
           ""
       )
      ."                  <tr>\n"
      ."                    <td class='nowrap'><div class='lbl' style='width: 60px;'><label for='title'>Title</label></div></td>\n"
      ."                    <td>"
      ."<div class='fl'>".draw_form_field("title",$title,"text",270,'','','','',$bulk_update)."</div>"
      ."<div class='lbl fl txt_r' style='width:40px;padding-right:5px'><label for='include_title_heading'>Show</label></div>"
      ."<div class='fl' style='width:15px'>".draw_form_field("include_title_heading",$include_title_heading,"bool",0,'','','','',$bulk_update)."</div>\n"
      ...
      ."  </tr>\n"
      ."                  <tr>\n"
      ."                    <td class='nowrap'><div class='lbl' style='width: 60px;'><label for='layoutID'>Layout</label></div></td>\n"
      ."                    <td>\n"
      ...
      ."              </tr>\n"
      ."              <tr>\n"
      ."                <td><table cellpadding='2' border='0' cellspacing='0' class='auto_report admin_containerpanel'>\n"
      ."                  <tr>\n"
      ."                    <td>"
...
// General
      ...
      .($page_allow_subtitle ?
          ...
          ."<div class='fl'>".draw_form_field("subtitle",$subtitle,"text",760,'','','','',$bulk_update)."</div><div class='clr_b'></div>"
       : ""
      )
      ."<div class='lbl'>Content</div><br />\n"
      ."<div class='fl'>".draw_form_field("content",$content,"html_multi_language",840,'','','','',$bulk_update,'',$toolbarSet,($page_allow_subtitle ? 380 : 400))."</div>"
      ."</div>"
      ."</div>"
...
// Publish
      ...
      .($isMASTERADMIN || (System::has_feature('Comments') && $isAPPROVER) ?
          ...
          .draw_form_field("comments_allow",$comments_allow,"radio_listdata",'','','','','',$bulk_update,'','lst_comment_allow')
      ...
      .($isMASTERADMIN || (System::has_feature('Ratings') && $isAPPROVER) ?
          ...
          .draw_form_field("ratings_allow",$ratings_allow,"radio_listdata",'','','','','',$bulk_update,'','lst_rating_allow')
          ."<div class='clr_b pixel'></div>"
        :  ""
       )
      ...
      .($isMASTERADMIN ?
           "<div class='fl' style='padding-left:10px;'>".draw_form_field("locked",$locked,"bool",'','','','','',$bulk_update)."</div>"
          ."<div class='lbl'><label for='locked'>Page is Locked for Modifications</label></div><div class='clr_b pixel'></div>\n"
        : ""
       )
      .($isAPPROVER ?
         "<div class='fl' style='padding-left:10px;'>".draw_form_field("permPUBLIC",$permPUBLIC,"bool",'','','','','',$bulk_update)."</div>"
        ."<div class='lbl'><label for='permPUBLIC'>Public</label></div><div class='clr_b pixel'></div>\n"
        ."<div class='fl' style='padding-left:10px;'>".draw_form_field("permSYSLOGON",$permSYSLOGON,"bool",'','','','','',$bulk_update)."</div>"
        ."<div class='lbl'><label for='permSYSLOGON'>Site Users</label></div><div class='clr_b pixel'></div>\n"
        ."<div class='fl' style='padding-left:10px;'>".draw_form_field("permSYSMEMBER",$permSYSMEMBER,"bool",'','','','','',$bulk_update)."</div>"
        ."<div class='lbl'><label for='permSYSMEMBER'>Site Members</label></div><div class='clr_b pixel'></div>\n"
        ."<br /><div class='lbl'>Page is also available to group members having 'Viewer' or 'Editor' rights in these groups:</div><br /><br />\n"
        ."<div style='padding-left:10px;'>"
        .(!$bulk_update ?
            draw_form_field("groups_assign",$groups_assign,"groups_assign",800,$formSelectorGroupSQL,'','','','','','',215)
...
// Keywords
      .($Obj_System->has_feature('Keywords') || $isMASTERADMIN ?
        ...
        .(!$bulk_update ?
            "<div>"
           .draw_form_field("keywords",$keywords,"keywords_assign",780,'','','','',$bulk_update,'','',320)
           ."</div>\n"
...
// Advanced
      ...
      ."<div class='txt_c'>"
      .draw_form_field("component_parameters",$component_parameters,"option_list",780,'','','','',$bulk_update,'','',240)
      ."</div>\n"
      ."<table>\n"
      .($page_allow_memberID ?
         "  <tr>\n"
        ...
        ."      <tr>\n"
        ."        <td class='nowrap' style='width:115px;'><div class='lbl'><label for='memberID'>Owner Member</label></div></td>"
        ."        <td>"
        .draw_form_field("memberID",$memberID,"selector","690",$formSelectorMemberIDSQL,'','','',$bulk_update)
        ."</td>\n"
        ."      </tr>\n"
        ."    </table></td>\n"
        ."  </tr>\n"
      ...
      .($isSYSADMIN || $isMASTERADMIN ?
        ...
        ."    <td><table cellpadding='1' border='0' cellspacing='0' width='100%' class='admin_containerpanel'>\n"
        ."      <tr>\n"
        ."        <td class='nowrap' style='width:115px;'><div class='lbl'><label for='path_extender'>Path Extender</label></div></td>"
        ."        <td>"
      ...
      .($page_allow_buttonsuite ?
        ...
        ."        <td>"
        .draw_form_field("navsuite1ID",$navsuite1ID,"selector","690",$formSelectorNavsuiteSQL,'','','',$bulk_update)
        ."</td>\n"
        ."      </tr>\n"
        ."      <tr>\n"
        ."        <td><div class='lbl'>Button Suite 2</div></td>"
        ."        <td>"
        .draw_form_field("navsuite2ID",$navsuite2ID,"selector","690",$formSelectorNavsuiteSQL,'','','',$bulk_update)
        ."</td>\n"
        ."      </tr>\n"
        ."      <tr>\n"
        ."        <td><div class='lbl'>Button Suite 3</div></td>"
        ."        <td>"
        .draw_form_field("navsuite3ID",$navsuite3ID,"selector","690",$formSelectorNavsuiteSQL,'','','',$bulk_update)
        ."</td>\n"
        ."      </tr>\n"
        ."    </table></td>\n"
        ."  </tr>\n"
      ...
      ."        <td>"
      .draw_form_field("componentID_pre",$componentID_pre,"selector","680",$formSelectorComponentSQL,'','','',$bulk_update)
      ."</td>\n"
      ."      </tr>\n"
      ."      <tr>\n"
      ."        <td style='width:125px;'><div class='lbl'>Component (late)</div></td>"
      ."        <td>"
      .draw_form_field("componentID_post",$componentID_post,"selector","680",$formSelectorComponentSQL,'','','',$bulk_update)
      ."</td>\n"
      ."      </tr>\n"
      ."    </table></td>\n"
      ."  </tr>\n"
...
// Meta Data
      ...
      ."<div style='height:".$height."px;width:".$width."px;'>"
      ."<div class='lbl'>Metadata is used by search engines to help them catalogue the page. These metadata entries apply to this page.</div><div class='clr_b'></div><br />\n"
      ."<div style='padding-left:20px;'>"
      ."<span class='formLabel'>Description</span><br />\n"
      .draw_form_field("meta_description",$meta_description,"textarea","780px",'','','','',$bulk_update,'','',160)."<br />\n"
      ."</div>\n"
      ."<div style='padding-left:20px;padding-top:30px;'>"
      ."<span class='formLabel'>Keywords</span><br />\n"
      .draw_form_field("meta_keywords",$meta_keywords,"textarea","780px",'','','','',$bulk_update,'','',160)."<br />\n"
      ."</div>\n"
      ."</div>\n"
      ."</div>"
      ...
      ."<input type='button' value='Save' onclick=\"this.value='Please Wait...';this.disabled=1;show_popup_please_wait();geid('submode').value='save';geid('form').submit();\" class='formbutton' style='width: 60px;'/>\n"
      ."<input type='button' value='Save and Close' onclick=\"this.value='Please Wait...';this.disabled=1;show_popup_please_wait();geid('submode').value='save_and_close';geid('form').submit();\" class='formbutton' style='width: 120px;'/>\n"
      .($canAdd ? "<input type='button' value='Save and New...' onclick=\"this.value='Please Wait...';this.disabled=1;show_popup_please_wait();geid('submode').value='save_and_new';geid('form').submit();\" class='formbutton' style='width: 120px;'/>\n" : "")
      ."</td>\n"
      ."              </tr>\n"
      ."            </table></td>\n"
      ."          </tr>\n"
