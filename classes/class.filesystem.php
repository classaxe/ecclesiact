<?php
define('VERSION_FILESYSTEM','1.0.16');
/*
Version History:
  1.0.16 (2014-04-18)
...
class FileSystem {
  ...
  function delete_dir_entry($key,$value) {
  ...
  function dir_getMatchingFiles($files, $search) {
  ...
  function dir_getContents($dir,$files=array()) {
    if(!($res=opendir($dir))) {
  ...
  function dir_wildcard_search($path,$pattern){
    return FileSystem::dir_getMatchingFiles(FileSystem::dir_getContents($path), $pattern);
  }

  function draw_dir_tree($dir_array, $level=0,$checkboxes=false,$title='Files',$expanded=0) {
    global $page_vars;
    ...
    foreach ($dir_array as $entry){
      if (isset($entry['type'])) {
        ...
        switch ($entry['type']) {
          case 'd':
            $out.= str_repeat("  ",$level)."  L".($level+1)." = insFldX(".($level==0 ? "foldersTree" : "L$level").", gFld(\"".$entry['name']."\",\"\"));\n";
            $out.= str_repeat("  ",$level)."  L".($level+1).".xID = \"".$_get."\";\n";
          break;
          case 'f':
            $out.= str_repeat("  ",$level)."  L".($level+1)." = insDocX(".($level==0 ? "foldersTree" : "L$level").", gLnk(\"R\",\"".$entry['name']."\", \"".$_get."\"));\n";
            $out.= str_repeat("  ",$level)."  L".($level+1).".xID = \"".$_get."\";\n";
  ...
  function file_append($name,$data) {
    if ($fp=fopen($name,'a')){
  ...
  function get_dir_tree($dir) {
    ...
    foreach ($entries as $entry) {
      ...
      $path = $dir.$name;
      if ($entry['dir']) {
        $out[] = array('name'=>$name,'type'=>'d','size'=>0,'path'=>$dir,'crc32'=>dechex(crc32($path)));
        $out[] = $this->get_dir_tree($path.'/');
      }
      else {
        $out[] = array('name'=>$name,'type'=>'f','size'=>filesize($path),'path'=>$dir,'crc32'=>dechex(crc32($path)));
      }
    }
    $d->close();
    return $out;
  ...
  function get_file($ID){
    ...
    if (get_person_permission("MASTERADMIN") && $this->page_vars['path']=='//details/system/'.SYS_ID){
      ...
      $cp_settings =
        Component_Base::get_parameter_defaults_and_values(
          $ident, '', false, $parameter_spec, array()
        );
    ...
    if ($filePath=="" || $fileName=="") {
      ...
      do_log(2,__CLASS__."::".__FUNCTION__."()",'',"404 file not found for file ID ".$this->ID." for ".$_SERVER["REQUEST_URI"]);
      header($_SERVER["SERVER_PROTOCOL"]." 404 Not Found",true,404);
      print "<h1>404</h1><p>The resource ".$_SERVER['REQUEST_URI']." wasn't found here.</p>";
      die;
    }
    do_log(0,__CLASS__."::".__FUNCTION__."()",'',"200 streamed file ".$this->ID." \"".$filePath.$fileName."\" for ".$_SERVER["REQUEST_URI"]);
  ...
  private function _get_file_test_page_403(){
    if ($this->page_vars['object_type']=='Page'){
      ...
      if (!$Obj->is_visible($this->page_vars)){
        ...
        do_log(2,__CLASS__."::".__FUNCTION__."()",'',"403 Page not found when trying to download file ID ".$this->ID." from ".$_SERVER["REQUEST_URI"]);
  ...
  private function _get_file_test_page_404(){
    ...
    Page::do_tracking("404");
    do_log(2,__CLASS__."::".__FUNCTION__."()",'',"404 Page not found when trying to download file ID ".$this->ID." from ".$_SERVER["REQUEST_URI"]);
    header($_SERVER["SERVER_PROTOCOL"]." 404 Not Found",true,404);
    print "<h1>404</h1><p>The resource ".$_SERVER['REQUEST_URI']." wasn't found here.</p>";
    return true;
  }
  ...
  private function _get_file_test_has_no_folderviewer(){
    ...
    if (strpos($content,'[ECL]content_group_member_mirror[/ECL]')!==false && strpos($this->page_vars['component_parameters'],'folder_viewer.dir=')!==false){
      return false;
    }
    ...
    Page::do_tracking("404");
    do_log(3,__CLASS__."::".__FUNCTION__."()",'',"Tried to access file on a page without a folder viewer - file ID=".$this->ID." for ".$_SERVER["REQUEST_URI"]);
    header($_SERVER["SERVER_PROTOCOL"]." 404 Not Found",true,404);
    print "<h1>404</h1><p>The resource ".$_SERVER['REQUEST_URI']." wasn't found here.</p>";
    return true;
  }
  ...
  function get_file_checksum($filepath) {
  ...
  function get_file_changes($filepath) {
    if (!file_exists($filepath)){
      return "(Missing)";
    }
    $out = "";
  ...
  function get_file_version_phpdoc($filepath) {
    $file = file_get_contents($filepath);
  ...
  // * METHOD: in_dir_tree()              *
  // **************************************
  function in_dir_tree($dir_array,$crc32) {
