<?php
define('VERSION_DISPLAYABLE_ITEM','1.0.150');
/*
Version History:
  1.0.150 (2014-03-29)
...
class Displayable_Item extends Block_Layout{
  ...
  function __construct($table='', $ID='', $systemID=SYS_ID) {
    parent::__construct($table, $ID, $systemID);
  }

  public function _get_type(){ return $this->_type; }
  public function _set_type($type) { $this->_type = $type; }
  protected function _get_context_menu_ID()            { return $this->_context_menu_ID ? $this->_context_menu_ID : get_js_safe_ID($this->_get_type()); }
  protected function _get_has_enable_flag()            { return $this->_has_enable_flag; }
  protected function _get_has_publish_date()           { return $this->_has_publish_date; }
  protected function _set_context_menu_ID($value)      { $this->_context_menu_ID = $value; }
  protected function _set_has_enable_flag($value)      { $this->_has_enable_flag = $value; }
  protected function _set_has_publish_date($value)     { $this->_has_publish_date = $value; }
  ...
  protected function _common_load_user_rights(){
    ...
    $this->_current_user_rights['canPublish'] =         $isMASTERADMIN || $isSYSADMIN || $isSYSAPPROVER;
    $this->_current_user_rights['canEdit'] =            $isMASTERADMIN || $isSYSADMIN || $isSYSAPPROVER || $isSYSEDITOR;
    $this->_current_user_rights['canRate'] =            $this->record['ratings_allow']=='all' || ($this->record['ratings_allow']=='registered' && !$isPUBLIC);
    $this->_current_user_rights['isUSERADMIN'] =        $isSYSADMIN || $isSYSAPPROVER || $isUSERADMIN || $isMASTERADMIN;
    $this->_current_user_rights['isSYSADMIN'] =         $isSYSADMIN || $isMASTERADMIN;
  ...
  protected function _draw_associated_setup(){
    ...
    $this->_mode =      'detail';
    $this->_ident =     get_js_safe_ID($this->_mode.'_'.$this->_get_type().(substr($this->_get_type(),-1,1)=='s' ? '' : 's'));
  ...
  public function _draw_detail_include_og_support(){
    ...
    Page::push_content(
      ...
      .($img ? "<meta property=\"og:image\" content=\"".trim($system_vars['URL'],"/").BASE_PATH.trim($img,"/")."\" />\n" : "")
  ...
  protected function _draw_detail_setup(){
    ...
    if (isset($this->record['enabled']) && $this->record['enabled']==0 && !$this->_current_user_rights['canPublish']){
      $this->_html.= $this->_draw_detail_draw_error('403');
      return false;
    }
  ...
  function draw_link($type, $record=false, $args=array()){
    if (!$record) {
      $record = $this->record;
    }
    $ID =           $record['ID'];
    ...
    switch ($type) {
      case 'add_to_outlook':
        return
          ...
          ."/export/icalendar/".$ID."\" "
          ."title=\"Add this ".$this->_get_object_name().$this->plural(1)." to your Outlook Calendar (Requires Outlook 2000 or later)\" rel='external'"
          .">"
      ...
      case 'map':
        return
          ...
          .(isset($record['systemURL']) ? trim($record['systemURL'],'/') : '')."','event_map','".$ID."');return false;\""
      ...
      case "media_download":
        ...
        return
          ...
          .($record['enclosure_secs'] || $record['enclosure_size'] ?
              " ("
             .((int)$record['enclosure_secs']!=0 ? "length ".seconds_to_hhmmss($record['enclosure_secs']) : "")
             .((int)$record['enclosure_size']!=0 ? ((int)$record['enclosure_secs']!=0 ? ", size " : "").number_format(two_dp($record['enclosure_size']/1024))."kb" : "")
             .")"
           : "")
          ."\">"
      ...
      case "media_popup_player":
        ...
        return
           "<a href=\"".$media_popup_URL."\" onclick=\"popWin('".$media_popup_URL."','".$ID."','resizable=1',320,120);return false;\" "
          ." rel='external'"
      ...
      case 'read_more':
        return
          ...
          .">"
          .(isset($args['label']) && $args['label'] ? $args['label'] : HTML::draw_icon('more',$systemID!=SYS_ID))
          ."</a>";
      ...
      case "register_event_large":
        return
          ...
          ." title=\"Register for this ".$this->_get_object_name().$this->plural(1)."\">"
          ."<img src='".BASE_PATH."img/spacer' class='icons icon_register_event' alt='Register for this event"
  ...
  function draw_listings_json($instance='',$args=array(),$cp_are_fixed=false){
    global $system_vars;
    $out = array(
      'css' =>  '',
      'js' =>   '',
    ...
    Page::push_content('style_include',"<link rel=\"stylesheet\" type=\"text/css\" href=\"/css/".System::get_item_version('css')."\" />");
    Page::push_content(
      'style',
...
//    $out['html'] = "<textarea style='width:100%;height:600px;'>".print_r($out,true)."</textarea>";
  ...
  protected function _draw_listings_draw_add_icon(){
    ...
    $this->_html.=
       ...
       ." onclick=\"details('".$add_form."','','".$popup['h']."','".$popup['w']."','','','','".$uri_args."');return false;\">"
       ."[ICON]11 11 1188 Add ".$this->_get_object_name()."&hellip;[/ICON]</a>";
  }

  protected function _draw_listings_draw_no_results(){
    $this->_groups_arr = (isset($this->_cp['filter_groups_list']) && $this->_cp['filter_groups_list'] ? explode(',',$this->_cp['filter_groups_list']) : array());
    $this->_html.=
       "<div class='no_items'>"
      ."There are no "
      .(isset($this->_cp['filter_important']) && $this->_cp['filter_important']=='1' ? "important " : "")
      .(isset($this->_cp['filter_important']) && $this->_cp['filter_important']=='0' ? "unimportant " : "")
      .$this->_get_object_name().$this->plural('1,2')
      .(count($this->_categories_arr) || count($this->_groups_arr) ? " in " : "")
      .(count($this->_categories_arr)==1 ? " the <b>".implode('',$this->_categories_arr)."</b> category" : "")
      .(count($this->_categories_arr)==2 ? " either the <b>".implode('</b> or <b>',$this->_categories_arr)."</b> category" : "")
      .(count($this->_categories_arr)>2 ? " any of the <b>".implode(', ',$this->_categories_arr)."</b> categories" : "")
      .(count($this->_categories_arr) &&  count($this->_groups_arr) ? " and " : "")
      .(count($this->_groups_arr)==1 ? " the <b>".implode('',$this->_groups_arr)."</b> group" : "")
      .(count($this->_groups_arr)==2 ? " either the <b>".implode('</b> or <b>',$this->_groups_arr)."</b> group" : "")
      .(count($this->_groups_arr)>2 ? " any of the <b>".implode(', ',$this->_groups_arr)."</b> groups" : "")
      ." at this time."
      ."</div>\n";
  }
  ...
  protected function _draw_listings_load_grouping_tabs(){
    ...
    if ($results_grouping && $results_paging){
      $this->_html.="<p><b>Error</b> You cannot use Grouping Tabs WITH Paging Controls - please choose one or the other.</p>";
      return;
    }
    global $YYYY;
    $tab_items = false;
  ...
  protected function _draw_listings_load_paging_controls(){
    ...
    if (!isset($_REQUEST['command']) || $_REQUEST['command']!=$this->_safe_ID."_load"){
      Page::push_content(
        ...
        ."  var fn = function(){hidePopWin(null);externalLinks();\$('#".$this->_safe_ID."_content audio').mediaelementplayer();"
        .(Base::module_test('Church') ? "Logos.ReferenceTagging.tag(\$J('#".$this->_safe_ID."_content')[0])" : "")
        ."};\n"
    ...
    switch($results_paging){
      ...
      case 2:
        ...
        $this->_paging_controls_html.=
           "<table class=\"pnav\" cellspacing=\"0\" cellpadding=\"0\" summary=\"Paging controls for ".$this->_ident."\">\n"
          ."  <tr>\n"
          ...
          .($this->_filter_offset>0 ?
             ...
             ." onclick=\"return ".$this->_safe_ID."_paging(".($this->_filter_offset+$results_limit*-1).");\""
  ...
  protected function _draw_listings_load_records(){
    ...
    $results = $this->get_records(
      array(
        ...
        'category_master' =>        (isset($this->_cp['filter_category_master']) ?    $this->_cp['filter_category_master'] : false),
        'container_path' =>         (isset($this->_cp['filter_container_path']) ?     $this->_cp['filter_container_path'] : ''),
        'container_subs' =>         (isset($this->_cp['filter_container_subs']) ?     $this->_cp['filter_container_subs'] : ''),
        'DD' =>                     '',
        'filter_date_duration' =>   (isset($this->_cp['filter_date_duration']) ?      $this->_cp['filter_date_duration'] : ''),
        'filter_date_units' =>      (isset($this->_cp['filter_date_units']) ?         $this->_cp['filter_date_units'] : ''),
        'filter_range_address' =>   (isset($this->_cp['filter_range_address']) ?      $this->_cp['filter_range_address'] : ''),
        'filter_range_distance' =>  (isset($this->_cp['filter_range_distance']) ?     $this->_cp['filter_range_distance'] : ''),
        'filter_range_lat' =>       (isset($this->_cp['filter_range_lat']) ?          $this->_cp['filter_range_lat'] : ''),
        'filter_range_lon' =>       (isset($this->_cp['filter_range_lon']) ?          $this->_cp['filter_range_lon'] : ''),
        'filter_range_units' =>     (isset($this->_cp['filter_range_units']) ?        $this->_cp['filter_range_units'] : ''),
        'important' =>              (isset($this->_cp['filter_important']) ?          $this->_cp['filter_important'] : ''),
        'limit' =>                  $this->_cp['results_limit'],
        'memberID' =>               (isset($this->_cp['filter_memberID']) ?           $this->_cp['filter_memberID'] : ''),
        'MM' =>                     $MM,
        'offset' =>                 $this->_filter_offset,
        'order_by' =>               (isset($this->_cp['results_order']) ?             $this->_cp['results_order'] : 'date'),
        'personID' =>               (isset($this->_cp['filter_personID']) ?           $this->_cp['filter_personID'] : ''),
        'what' =>                   (isset($this->_cp['filter_what']) ?               $this->_cp['filter_what'] : 'all'),
        'YYYY' =>                   $YYYY
      )
    );
  ...
  protected function _draw_listings_setup($instance='', $args=array(), $cp_are_fixed=false){
    ...
    $this->_mode =              'list';
    $this->_ident =             get_js_safe_ID($this->_mode.'_'.$this->_get_type().(substr($this->_get_type(),-1,1)=='s' ? '' : 's'));
    $this->_safe_ID =           get_js_safe_ID($this->_ident."_".$this->_instance);
  ...
  function draw_comments_block($allow_add='none'){
    $Obj = new Comment;
    return
       "<div class='clr_b'></div>\n"
      ...
      ."<div class='clr_b'></div>\n"
      ."</div>"
      .(System::has_feature('Comments') && ($allow_add=='all' || ($allow_add=='registered' &&  get_person_permission("SYSLOGON"))) ?
  ...
  function draw_from_recordset($records,$args){
    ...
    // Link:
      ...
      if ($args['thumbnail_show']){
        switch ($args['thumbnail_image']){
          case "s":   $img = $record['thumbnail_small'];  break;
          case "m":   $img = $record['thumbnail_medium']; break;
          case "l":   $img = $record['thumbnail_large'];  break;
      ...
      if ($args['category_show']) {
        ...
        $categories = $Obj_Category->get_labels_for_values("'".implode("','",array_keys($categories))."'","'".$category_type."'");
//        y($categories);die;
      ...
      $out.=
        ...
        .($args['subtitle_show'] && $record['subtitle'] ? "<h2 class='subtitle'>".$record['subtitle']."</h2>" : "")
        ...
        .(isset($args['item_footer_component']) && $args['item_footer_component'] ? draw_component_by_name($args['item_footer_component'],$record) : "")
        ."<div class='clr_b'></div>"
        ."</div>"
  ...
  public function draw_object_map_html($ident=false){
    ...
    if (!count($this->_data_items)){
      $this->_html.=
         "<h2>Sorry!</h2>\n"
        ."<p>There are no map locations defined for the ".(count($this->_ID_arr)!=1 ? count($this->_ID_arr) : '')." selected item".(count($this->_ID_arr)==1 ? '' : 's').".</p>\n";
      return $this->_html;
    }
    ...
    if ($this->_range){
      ...
      $this->_Obj_Map->map_centre($this->_data_items[0]['map_lat'],$this->_data_items[0]['map_lon'],$this->_default_zoom);
    }
  ...
  protected function _draw_object_map_html_draw_map_points(){
    foreach ($this->_data_items as $item){
      $desc =                   trim(isset($item['map_desc']) && $item['map_desc'] ? $item['map_desc'] : $item['map_loc']);
      $circle_radius =          (isset($item['map_area']) && $item['map_area']>0 ? sqrt(($item['map_area']/pi())) : false);
  ...
  protected function _draw_object_map_html_setup(){
    ...
    $this->_map_title =         (get_var('map_title') ? sanitize('html',get_var('map_title')) : $this->_get_object_name().' addresses');
  ...
  public function draw_panel_box($box=1,$title,$content,$link,$link_page,$rss_link,$ID='',$width=0,$shadow=0) {
    global $system_vars;
    $edit_params =    $this->get_edit_params();
    $add_form = $edit_params['report'];
    ...
    if ($canEdit){
      ...
      else if (isset($this->_cp['filter_category_list']) && $this->_cp['filter_category_list']!='*'){
    ...
    return
	  ...
	  .($box!=0 ?
        ...
        .($canEdit ?
             ...
             ." onclick=\"details('".$add_form."','','".$popup['h']."','".$popup['w']."','','','','".$uri_args."');return false;\"  title='Add ".$this->_get_object_name()."&hellip;'>"
        ...
        .($link ?
           ...
           ." onmouseover=\"window.status='View all ".$this->_get_object_name()."s';return true;\" onmouseout=\"window.status='';return true;\">"
           .$title
           ."</a>"
         : $title
         )
        ...
        .($rss_link==1?
           ...
           ." onmouseover=\"window.status='View RSS ".$this->_get_object_name()." Feed';return true;\" onmouseout=\"window.status='';return true;\">"
  ...
  function draw_ratings_block($submode=false,$value=false){
    return Rating::draw_block($submode,$value);
  }

  function draw_related_block() {
  ...
  function draw_search_results_paging_nav($result,$find) {
    global $page_vars;
    $nav =          array();
    ...
    if ($retrieved && $limit) {
      ...
      if ($offset>0) {
        $nav[] =
           "<a href=\"".$URL.$offset."\" onclick=\"return search_offset('".($offset-$limit)."','".$type."')\""
      ...
      for ($i=0; $i<$result_pages; $i++) {
        $title =
           ($i*$limit == $found ?
            ...
            : $this->_get_object_name()." Results ".(1+$i*$limit)." - ".((1+$i)*$limit<$found ? (1+$i)*$limit : $found)
           );
        if ($i*$limit == $offset) {
          $nav[] =
             "<span title=\"".$title."\">"
          ...
          $nav[] =
             "<a href=\"".$URL.($i*$limit)."\" onclick=\"return search_offset('".($i*$limit)."','".$type."')\""
            ." title=\"".$title."\">".($i+1)."</a>";
        }
      }
      if ($offset+$limit<$found) {
        $nav[] =
           "<a href=\"".$URL.($offset+$limit)."\" onclick=\"return search_offset('".($offset+$limit)."','".$type."')\""
...
//    print count($nav);die;
    ...
    if (count($nav)>12) {
      ...
      if ($page<5 || $page+5>=$result_pages){
        ...
        $nav =
          array(
            $nav[0],$nav[1],$nav[2],'&hellip;',
            $nav[$page-1],$nav[$page],$nav[$page+1],$nav[$page+2],$nav[$page+3],
            '&hellip;',$nav[count($nav)-3],$nav[count($nav)-2],$nav[count($nav)-1]
          );
      }
    }
//    y($nav);die;
    return
      ...
      .($found != $retrieved ?
        ...
        ."</div>"
        .(count($nav) ? "<div class='fl va_t' style='padding-left:20px;padding-bottom:5px;'>".implode(" ",$nav)."</div>" : "")
        : "</div>"
       )
       ."<div class='clr_b'></div>"
       ."</div>";
...
//    z($sql);
  ...
  function get_keywords() {
    $sql =
      ...
      ."  `keywords`.`ID`,\n"
      ."  `keywords`.`keyword`,\n"
      ."  (SELECT COUNT(`ka`.`ID`) FROM `keyword_assign` AS `ka` WHERE `keywords`.`ID` = `ka`.`keywordID`) AS `count`\n"
      ."FROM\n"
      ."  `keywords`\n"
  ...
  function get_URL($record=false) {
    ...
    switch ($type) {
      ...
      break;
      case "product":
        return (isset($record['relative_URL']) ? $record['relative_URL'] : BASE_PATH.trim($record['path'],'/'));
      break;
      default:
  ...
  protected function _draw_render_JSON(){
    ...
    $this->_js.=Page::pop_content('javascript')."\n".Page::pop_content('javascript_onload');
  ...
  protected function _try_delete_item_msg_insufficient_rights(){
    ...
    Page::push_content(
      'javascript_onload',
      "  popup_msg=\"".$msg."\";popup_dialog('Item Delete',\"<div style='padding:4px'>\"+popup_msg+\"</div>\",'320',120,'OK','','');"
    );
  }



  function truncate_more(&$text){
  ...
  function truncate_text(&$text,$len){
    if(strlen($text)>$len){
