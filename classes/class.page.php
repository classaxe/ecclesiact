...
class Page extends Displayable_Item {
  ...
  static $style = "";
  static $css_colors =  array();
  static $css_color_idx = 1;
...
//    z($sql);
  ...
  private function do_clone($page_heading_title){
    ...
    $canEdit =          ($page_vars['layoutID']!=2 && ($isMASTERADMIN || ($page_vars['locked']==0 && ($isSYSADMIN || $isSYSEDITOR))));
...
//    y($parent_path.$new_page);
    ...
    if ($newPageID) {
      ...
      return
        ...
        ."<div class='status_okay'><b>Success:</b><br />Copied this page to <b><a href=\"".BASE_PATH.trim($parent_path,'/').'/'.$new_page."\">".$new_page."</a></b></div>\n"
        .$this->draw_detail_content($page_heading_title,$page_vars);
    }
    else {
      $this->do_tracking("403");
      return
         $this->draw_toolbar_page_edit(1,1,0)
        ."<div class='status_error'><b>Error copying page:</b><br />A page named <b>".$new_page."</b> already exists in the system.</div>\n"
        .$this->draw_detail_content($page_heading_title,$page_vars);
    }
  }

  function draw_detail($page) {
  ...
  //  Print "<!-- \$visible=".$visible.", \$isSYSMEMBER=$isSYSMEMBER, \$page_vars['permSYSMEMBER']=".$page_vars['permSYSMEMBER']."-->";
    $page_heading_title =
      (isset($page_vars['include_title_heading']) && $page_vars['include_title_heading'] ?
    ...
    if ($page_vars['ID']) {
      ...
      if ($canEdit && $submode=='edit' && $print!="1") {
        ...
        return
           ""
          .$Obj_RC->draw_form_field(array(),'content',$page_vars['content'],'html_multi_language','100%','','','',0,0,'',$toolbarSet,300)
          ."<input type='button' name='save_page' value='Save' class='formButton' style='width: 100px;' onclick=\""
          ."if (confirm('SAVE CHANGES\\n\\nAre you sure you wish to save changes to this page?\\nThis change cannot be undone.')){ geid('submode').value='save_page';geid('form').submit();}else {alert('SAVE CHANGES\\n\\nNo changes have been saved.'); }\"/>"
          ."<input type='button' class='formButton' value='Cancel'  style='width: 100px;' onclick=\"geid('submode').value='';geid('form').submit();\"/><br />";
      }
      if (($canEdit || $canPublish) && $submode=='save_as') {
        ...
        return
          ...
          .($isMASTERADMIN ?
              "<div class='clr_b'>\n"
             ."  <div class='fl' style='width:100px;'>Save ".($page_vars['systemID']=='1' ? "Global page " : "")."to:&nbsp;</div>\n"
             ."  <div class='fl'>\n"
             ."<select name=\"new_systemID\" style=\"width: 255px;\" class='formField'>\n"
             .draw_select_options("SELECT `ID` AS `value`,`textEnglish` AS `text` FROM `system` ORDER BY `text`",SYS_ID)
             ."</select></div>\n"
             ."</div>\n"
           : "")
          ."<div class='clr_b'>\n"
          ...
          ."  <div class='fl'>".draw_form_field('new_page',$page_vars['page'],'posting_name_unprefixed',250)."</div>\n"
          ."</div>\n"
          ."<div class='clr_b' style='margin: 0 auto; text-align:center;'><input type='button' name='save_as' value='Save' class='formButton' style='width: 50px;' onclick=\"geid('submode').value='save_as';geid('form').submit();\"/>\n"
          ."<input type='button' class='formButton' value='Cancel'  style='width: 50px;' onclick=\"geid('submode').value='';geid('form').submit();\"/>\n"
          ."</div>\n"
          ."</div>\n"
          .$this->draw_detail_content($page_heading_title,$page_vars);
      }
  ...
  function draw_detail_content($page_heading_title,$page_vars) {
    global $msg;
    ...
    return
      ...
      ."<div class='content'>"
      .($status_msg ? HTML::draw_status('form_edit_inpage',$status_msg) : "")
      .($page_vars['componentID_post']!=1 ? draw_component($page_vars['componentID_post']) : $page_vars['content_zones'][0])
      ."</div>"
//            .($page_vars['comments_allow'] ? "<a href=\"#anchor_comments_list\">View Comments</a>" : "")
  ...
  public static function draw_http_error($status) {
    ...
    $uri_arr = array();
    if ($command!='') {       $uri_arr[] = "command=$command";}
    if ($mode!=''){           $uri_arr[] = "mode=$mode";}
    if ($report_name!=''){    $uri_arr[] = "report_name=$report_name";}
    if ($selectID!=''){       $uri_arr[] = "selectID=$selectID";}
    if ($print!=''){          $uri_arr[] = "print=$print";}
    if ($targetID!=''){       $uri_arr[] = "targetID=$targetID";}
    $uri =  implode('&',$uri_arr);
  ...
  private function draw_toolbar_page_edit($allowPopupEdit=0,$allowSaveAs=0,$withCopy=0,$newPage='',$newPageID='') {
    global $page_vars;
    $Obj = new Page;
    $args =
      array(
  ...
  function get_css_idx($color,$bgcolor){
  ...
  function get_field($arg1,$arg2=false) {
    if ($arg2===false){
  ...
  function get_unique_name($name,$systemID=SYS_ID) {
...
//    y($max);y($records);
  ...
  function get_ID_by_path($path,$systemID=false) {
    $sql =
       "SELECT\n"
      ."  `ID`\n"
      ."FROM\n"
...
//    z($sql);
  ...
  function get_page_by_path($name,$systemID=SYS_ID) {
    $results = array();
    $sql =
       "SELECT\n"
      ."  *,\n"
  ...
  function get_page_disambiguation($name,$systemID=SYS_ID) {
    $results = array();
    $sql =
       "SELECT\n"
      ."  *,\n"
...
//      z($sql);
  ...
  function get_resolved_path($path=''){
    $path = trim($path,"/");
    if ($path=='') {
      return '0|'.$path;
    }
  ...
  function get_search_results($args){
   ...
   // Paging query:
    $sql =
      ...
      .($search_date_end!="" ? "  `p`.`history_created_date` < DATE_ADD('".$search_date_end."',INTERVAL 1 DAY) AND\n" : "")
    ...
    if ($records) {
      foreach ($records as $row) {
        ...
        if ($visible) {
          ...
          else if($out['count']<$search_offset+$limit) {
...
//    y($out);die;
  ...
  function get_selector_sql_parents($thisPageID=false) {
    ...
    if ($isMASTERADMIN) {
      ...
      foreach ($records as $record) {
        ...
        $sql_out.=
          ...
          ."  ),\n"
          .($thisPageID ?
              "  IF(`pages`.`ID` IN(".$thisPageID.") OR `pages`.`parentID` IN(".$thisPageID."),'ffe0e0','".$bgcolor."'),\n"
            : "  '".$bgcolor."',\n"
           )
          ."  0\n"
          ."FROM\n"
...
//    z($sql_out);die;
    ...
    $sql_out.=
      ...
      ."  ),\n"
      .($thisPageID ?
          "  IF(`pages`.`ID` IN(".$thisPageID.") OR `pages`.`parentID` IN(".$thisPageID."),'ffe0e0','e8ffe8'),\n"
        : "  'e8ffe8',\n"
       )
      ."  0\n"
      ."FROM\n"
...
//    z($sql_out);die;
  ...
  function hasDynamicTags($content){
  ...
  function on_delete_pre_set_nested_paths(){
...
//    die;
  ...
  function on_copy_post_set_nested_paths(){
    global $action_parameters;
  ...
  function on_insert_post_set_nested_paths(){
    global $action_parameters;
...
//        z($sql);die;
  ...
  function on_update_pre_set_nested_paths(){
...
//    y($action_parameters);die;
    ...
    $bulk_update =
      (isset($action_parameters['data']['bulk_update']) && $action_parameters['data']['bulk_update']==1 ? true : false);
    $sql =
       "SELECT\n"
      ."  `systemID`,\n"
      ."  `parentID`,\n"
...
//        z($sql);die;
  ...
  function print_form_data(){
    ...
    $skip_csv =
      ...
      ."goto,limit,MM,mode,offset,print,report_name,search_categories,search_date_end,search_date_start,search_keywords,"
      ."search_offset,search_text,search_type,selected_section,selectID,sortBy,submode,targetField,targetFieldID,targetID,"
      ."targetReportID,targetValue,topbar_search,YYYY,undefined"
      ;
  ...
  function prepare_html_head() {
    ...
    $CM_level =         ($isMASTERADMIN ? 3 : ($isSYSADMIN ? 2 : ($isUSERADMIN || $isSYSAPPROVER || $isSYSEDITOR ? 1 : 0)));
    ...
    $this->push_content('html_top',
      ...
      .($page_vars['meta_description'] ? "<meta name=\"description\" content=\"".$page_vars['meta_description']."\"/>\n" : "")
      .($page_vars['meta_keywords'] ? "<meta name=\"keywords\" content=\"".$page_vars['meta_keywords']."\"/>\n" : "")
    );
    $this->push_content('head_top',
       ($favicon ? "<link rel=\"shortcut icon\" href=\"".BASE_PATH."img/sysimg/".$favicon."\"/>\n" : "")
      ."<link rel=\"search\" type=\"application/opensearchdescription+xml\" title=\"".$system_vars['textEnglish']." Search\" href=\"".BASE_PATH."osd/\" />\n"
      .(System::has_feature('Articles')   ? "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$system_vars['textEnglish']." RSS Articles Feed\"     href=\"".BASE_PATH."rss/articles\" />\n" : "")
      .(System::has_feature('Events')     ? "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$system_vars['textEnglish']." RSS Events Feed\"       href=\"".BASE_PATH."rss/events\" />\n" : "")
      .(System::has_feature('Jobs')       ? "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$system_vars['textEnglish']." RSS Job Postings Feed\" href=\"".BASE_PATH."rss/jobs\" />\n" : "")
      .(System::has_feature('News')       ? "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$system_vars['textEnglish']." RSS News Feed\"         href=\"".BASE_PATH."rss/news\" />\n" : "")
      .(System::has_feature('Podcasting') ? "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"".$system_vars['textEnglish']." RSS Podcasts Feed\"     href=\"".BASE_PATH."rss/podcasts\" />\n" : "")
    );
    $this->push_content(
      'style_include',
       $Obj_System->draw_css_include()
      .($mode!='details' ? "<link rel=\"stylesheet\" type=\"text/css\" href=\"".BASE_PATH."css/layout/".$page_vars['layoutID']."/".$cs_layout."\" />" : "")
      .(isset($page_vars) && trim($page_vars['theme']['style'])!='' ?
         "<link rel=\"stylesheet\" type=\"text/css\" href=\"".BASE_PATH."css/theme/".$page_vars['theme']['ID']."/".dechex(crc32($page_vars['theme']['style']))."\" />" : "")
    );
    $this->push_content('style',
       ...
       : "")
      .($isIE ?
            ".css3, .form_box, .shadow { behavior: url(".BASE_PATH."css/pie/".System::get_item_version('css_pie')."); }\n"
         :
           ""
        )
      .".zoom_text { font-size: ".(isset($_COOKIE['textsize']) && $_COOKIE['textsize']=='big' ? "120" : "80")."%;}\r\n"
    // Page
    ...
    $this->push_content('javascript',
      ...
      ."currentLanguage =   \"".(isset($_SESSION['lang']) ? $_SESSION['lang'] : $system_vars['defaultLanguage'])."\"\n"
      ...
      .($system_vars['debug_no_internet']!=1 && $system_vars['google_analytics_key']!='' && $mode!='details' && $mode!='report'?
      ...
      .($system_vars['debug_no_internet']!=1 && $system_vars['piwik_id'] && $mode!='details' && $mode!='report' ?
         "var _paq = _paq || [];\n"
        ."(function(){\n"
   ...
   $this->push_content(
    ...
    $this->push_content('body_top',   "<body class=\"".(isset($_COOKIE['textsize']) && $_COOKIE['textsize']=='big' ? "zoom_big" : "zoom_small")."\">");
    $this->push_content('body',
      ...
      ."<a href=\"#".$anchor_ID."\" class='fl'><img src=\"".BASE_PATH."img/spacer\" alt=\"Skip to Content\" height=\"1\" width=\"1\" class=\"border_none fl b margin_none\" /></a>\r\n"
      ...
      ."      <div id=\"popupControls\"><img src=\"".BASE_PATH."img/spacer\" class=\"icons\" height=\"10\" width=\"10\" style='background-position:-2590px 0px;' onclick=\"hidePopWin(null)\" alt='Close' /></div>"
      ."    </div>\n"
    ...
    if (Base::module_test('Church') && $system_vars['debug_no_internet']!=1 && $mode!='details' && $mode!='report'){
