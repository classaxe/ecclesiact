...
class Product extends Displayable_Item {
  ...
  function __construct($ID="",$systemID=SYS_ID){
    ...
    $this->_cp_vars_detail = array(
      'block_layout' =>             array('match' => '',                'default'=>'Product',       'hint'=>'Name of Block Layout to use'),
      'block_layout_push_products' => array('match' => '',              'default'=>'Product',       'hint'=>'Name of Block Layout to use for Push Products'),
      'cart_add_show' =>            array('match' => 'enum|0,1',        'default'=>'1',             'hint'=>'0|1'),
      'cart_checkout_link_show' =>  array('match' => 'enum|0,1',        'default'=>'1',             'hint'=>'0|1'),
      'cart_checkout_link_text' =>  array('match' => '',                'default'=>'',              'hint'=>'Leave blank to use icon'),
      'cart_emptycart_link_show' => array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'cart_emptycart_link_text' => array('match' => '',                'default'=>'',              'hint'=>'Leave blank to use icon'),
      'cart_skin' =>                array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'cart_skin_image' =>          array('match' => '',                'default'=>"/img/sysimg/cart_skin.gif,C6D6FF,4273E7,C6D6FF,3D4C8B,E7EFEF,A5A594,E7F7FF,4A63D6,0000FF,7BA5F7",             'hint'=>'Graphic to use for cart operations'),
      'cart_skin_classname' =>      array('match' => '',                'default'=>'cart_skin',     'hint'=>'Graphic to use for cart operations'),
      ...
      'product_grouping_show' =>    array('match' => 'enum|0,1',        'default' =>'0',            'hint'=>'0|1'),
      'product_price_show' =>       array('match' => 'enum|0,1',        'default' =>'1',            'hint'=>'0|1'),
    ...
    $this->_cp_vars_listings = array(
      ...
      'block_layout' =>             array('match' => '',                'default'=>'Product',       'hint'=>'Name of Block Layout to use'),
      'box' =>                      array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1|2'),
      'box_footer' =>               array('match' => '',                'default'=>'',              'hint'=>'Text below displayed Products'),
      'box_header' =>               array('match' => '',                'default'=>'',              'hint'=>'Text above displayed Products'),
      'box_rss_link' =>             array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'box_title' =>                array('match' => '',                'default'=>'Products',      'hint'=>'text'),
      'box_title_link' =>           array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'box_title_link_page' =>      array('match' => '',                'default'=>'all-products',  'hint'=>'page'),
      'box_width' =>                array('match' => 'range|0,n',       'default'=>'0',             'hint'=>'0..x'),
      'cart_add_show' =>            array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'cart_checkout_link_show' =>  array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'cart_checkout_link_text' =>  array('match' => '',                'default'=>'',              'hint'=>'Leave blank to use icon'),
      'cart_emptycart_link_show' => array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      'cart_emptycart_link_text' => array('match' => '',                'default'=>'',              'hint'=>'Leave blank to use icon'),
      'cart_skin' =>                array('match' => 'enum|0,1',        'default'=>'0',             'hint'=>'0|1'),
      ...
      'item_footer_component' =>    array('match' => '',                'default'=>'',              'hint'=>'Name of component rendered below each displayed Product'),
      ...
      'results_order' =>            array('match' => 'enum|date,itemCode,title', 'default' =>'date',    'hint'=>'date|itemCode|title'),
  ...
  protected function _draw_listings_include_js(){
    ...
    Page::push_content(
      ...
      ."    return false;\n"
      ."  }\n"
      ."  var post_vars='command=".$this->_ident."_".$this->_instance."_cart&targetID='+productID+'&targetValue='+qty+'&offset='+offset;\n"
      ."  window.focus();\n"
      ...
      ."  ajax_post_streamed(base_url+'".trim($page_vars['path'],'/')."','".$this->_ident."_".$this->_instance."_content',post_vars,fn);\n"
      ."  return false;\n"
      ."}\n"
  ...
  protected function BL_cart_operations(){
    ...
    $js =
       "onchange=\""
      .$safeID."_cart(".$this->record['ID'].",this.value,".$offset.")\"";
    $out = "<div class='fl'>";
    if ($show_price){
      $out.=
    ...
    $cart_skin_classname =  (isset($this->_cp['cart_skin_classname']) && $this->_cp['cart_skin_classname']!='' ? $this->_cp['cart_skin_classname'] : '');
    $out.=
       Cart::draw_cart_controls($safeID,$this->record['ID'],$quantity,$offset,$cart_skin_image,$cart_skin_classname)
      ."</div>\n";
    ...
    if (Cart::has_items()){
        ...
        $out.=
         ...
         .(isset($this->_cp['cart_checkout_link_text']) && $this->_cp['cart_checkout_link_text'] ?
          ...
          :
            ...
            ." class='".($has_skin ? "cart_skin_bg cart_skin_checkout": "icons cart_noskin_checkout")."'"
      ...
      if (isset($this->_cp['cart_emptycart_link_show']) && $this->_cp['cart_emptycart_link_show']=='1'){
        $out.=
          "<a href='#'"
         ." onclick=\"if(confirm('Remove ALL items from your shopping cart?')){".$this->_ident."_".$this->_instance."_empty(".$offset.");}return false;\">"
  ...
  protected function _draw_listings_load_records(){
    $results = $this->get_records(
      array(
        ...
        'order_by' =>         $this->_cp['results_order']
      )
    );
  ...
  function draw_product_child_selector($name,$itemCode,$width=150){
    global $system_vars;
    ...
    if ($found) {
      ...
      foreach ($result['results'] as $row) {
        ...
        $out.=
          ...
          .($row['itemCode']!=strip_tags($itemCode) ? " title=\""
          .$row['itemCode']
          .($active=='expired' ? " (Expired product)" : "")
          .($active=='pending' ? " (Future product)" : "")
          ."\"" : "")
          .">\n"
          ."<a href=\"".($local ? BASE_PATH : $row['URL']).$row['itemCode']."\">"
          ."<b>"
          .($itemCode!="" ? $itemCode : "(No Item Code)")."</b></a></td>\n"

          ."    <td class='table_border va_t'"
          .($row['title']!=strip_tags($title) ? " title=\""
          .$row['title']
          .($active=='expired' ? " (Expired product)" : "")
          .($active=='pending' ? " (Future product)" : "")
          ."\"" : "")
          ."><b>"
          .($title!="" ? $title : "(Untitled)")."</b></td>\n"
  ...
  function export_sql($targetID,$show_fields){
    ...
    $Obj = new Backup;
    $extra_select =
       $Obj->db_export_sql_query("`product_relationship`  ","SELECT * FROM `product_relationship` WHERE `productID` IN(".$targetID.")",$show_fields)
      .$Obj->db_export_sql_query("`push_product_assign`   ","SELECT * FROM `push_product_assign`  WHERE `assignID` IN(".$targetID.")",$show_fields)."\n";
    return parent::sql_export($targetID,$show_fields,$header,'',$extra_delete,$extra_select);
  }

  function get_array_for_parent_itemcode_list($csv_list){
  ...
  function get_children_by_parentID($parentID=false,$apply_availability=false,$apply_visibility=false) {
    if ($parentID===false) {
  ...
  function get_ID_by_itemCode($itemCode){
    $sql =
       "SELECT\n"
      ."  `ID`\n"
      ."FROM\n"
  ...
  function get_match_for_name($name,&$mode,&$ID){
    $sql =
       "SELECT\n"
      ."  `ID`\n"
      ."FROM\n"
...
//    y($categories);
  ...
  function get_number_of_related_orders(){
    $sql =
       "SELECT\n"
...
//    z($sql);die;
  ...
  function get_products_filtered($product_category_list=false,$product_grouping_list=false,$get_children=false) {
    $out = array();
    $sql =
       "SELECT\n"
      ."  `product`.*,\n"
      ...
      ."WHERE\n"
      .($product_category_list ?    "  `product`.`category` REGEXP \"".implode("|",explode(',',$product_category_list))."\" AND\n": "")
      .($product_grouping_list ? "  `product_grouping`.`name` IN(\"".implode("\",\"",explode(",",$product_grouping_list))."\") AND\n" : "")
...
//    z($sql);
  ...
  function get_products_for_productID_list($productID_csv=false){
...
*/
  function get_records(){
    ...
    $sql =
      ...
      .($category!="*" ?    "  `product`.`category` REGEXP \"".implode("|",explode(',',$category))."\" AND\n": "")
      .($category_master ?  "  `product`.`category` REGEXP \"".implode("|",explode(',',$category_master))."\" AND\n": "")
      .($important!=='' ?   "  `product`.`important`=".$important." AND\n" : "")
      ."  (\n"
...
//      ."#$search_text\n"
      ...
      ."  `p`.`permSYSMEMBER`,\n"
      ."  `p`.`title`,\n"
      ."  IF(`p`.`active_date_from` <= NOW() AND (`p`.`active_date_to` ='0000-00-00' OR `p`.`active_date_to`>= NOW()),1,0) `active`,\n"
      ...
      .($userIsAdmin ? "" : "  (`p`.`active_date_from`='0000-00-00' OR `p`.`active_date_from`<= NOW()) AND\n")
      .($userIsAdmin ? "" : "  (`p`.`active_date_to`='0000-00-00'   OR `p`.`active_date_to` >= NOW()) AND\n")
      .($search_date_start!="" ? "  (`active_date_from` >= '".$search_date_start."') AND\n" : "")
      .($search_date_end!="" ?   "  (`active_date_from` < DATE_ADD('".$search_date_end."',INTERVAL 1 DAY)) AND\n" : "")
      ...
      .($search_categories!="" ? "  `p`.`category` REGEXP \"".implode("|",explode(', ',$search_categories))."\" AND\n": "")
...
//    z($sql);
  ...
  function handle_report_delete(&$msg) {
    ...
    $msg =
      status_message(
        2,true,$this->_get_object_name(),'',$is_are." referenced in ".$orders." order".($orders==1 ? '' : 's')." - your deletion has been cancelled.",$targetID
      );
    return false;
  }
  ...
  public function manage_relationships(){
    ...
    $out = "<h3 style='margin:0.25em'>Relationships for this ".$this->_get_object_name()."</h3>";
    if (!get_var('selectID')) {
      $out.="<p style='margin:0.25em'>No Relationships - this ".$this->_get_object_name()." has not been saved yet.</p>";
    }
    else {
      $out.= draw_auto_report('relationships_for_product',1);
    }
    return $out;
  }
  ...
  public function try_delete_item(){
    ...
    if ($orders =   $this->get_number_of_related_orders()){
      $msg =        "<b>Error</b><br /><br />This Product is referenced in ".$orders." order".($orders==1 ? '' : 's')." - your deletion has been cancelled.";
      Page::push_content(
        'javascript_onload',
