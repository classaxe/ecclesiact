      ...
      default:
        switch($report_name){
          case 'report_filters':
            ...
            $result = $Obj->sql_export($targetID,$show_fields);
        ...
        $result = $Obj->export_sql($targetID,$show_fields);
        print $result;
        die;
      break;
    }
...
//    y($records);
    ...
    $generator =    System::get_item_version('system_family')." ".System::get_item_version('codebase').".".$system_vars['db_version'];
    ...
    if ($report_record['name']=='group_members'){
    }
    ...
    $cellStyle = array(
      'borders' =>    array(
        'allborders' =>     array('style' => PHPExcel_Style_Border::BORDER_THIN, 'color' => array('rgb' => '808080')),
      ),
      'width'
    );
    $ObjWorksheet->getStyle('A4:'.(PHPExcel_Cell::stringFromColumnIndex(count($columns)-1)).'4')->applyFromArray($headerStyle);
    $ObjWorksheet->getColumnDimensionByColumn('A:'.(PHPExcel_Cell::stringFromColumnIndex(count($columns)-1)))->setWidth(20);
    $ObjWorksheet->getStyle('A5:'.(PHPExcel_Cell::stringFromColumnIndex(count($columns)-1)).(count($records)+4))->applyFromArray($cellStyle);
    for($col=0; $col<count($columns); $col++){
      ...
      for($row=0; $row<count($records); $row++){
        ...
        $value = (isset($records[$row][$columns[$col]['reportField']]) ? $records[$row][$columns[$col]['reportField']] : "");
        ...
        switch($columns[$col]['fieldType']){
          ...
          case 'file_upload':
            if ($value){
              ...
              $ObjWorksheet->getCell($cell)->getHyperlink()->setTooltip("Download ".$file_params['name']." (".$file_params['type'].", ".$file_params['size']." bytes)");
            }
          break;
          case 'int':
            $width = 5;
