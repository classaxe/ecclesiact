...
class Order extends Record
{
    ...
    public function process_product_ordered_actions()
    {
        $this->actions_process_product_ordered();
    }

    public function actions_process_product_ordered()
    {
    ...
    public function actions_process_product_pay()
    {
        $ID_arr =   explode(",", $this->_get_ID());
    ...
    public function actions_process_product_pay_failure()
    {
        $ID_arr =   explode(",", $this->_get_ID());
        $orders_processed = array();
        for ($i=0; $i<count($ID_arr); $i++) {
    ...
    public function batch_process($orders_arr)
    {
    ...
    public function command($mode, $orderID = false)
    {
        global $personID;
    ...
    public function count_credit_memos()
    {
        if ($this->_get_ID()=="") {
            return false;
        }
  ...
  //    z($sql);die;
    ...
    public function count_registered_events()
    {
        if ($this->_get_ID()=="") {
            return false;
        }
  ...
  //    z($sql);die;
    ...
    public function create($personID, $TMethod, $TCardName, $TCardNumber, $TCardExpiry_mm, $TCardExpiry_yy)
    {
    ...
    public function download_pdf($pdf_template_path)
    {
        global $system_vars;
        ...
        $data_arr = array(
            ...
            'Billing' =>
                ...
                .($_order['payment_card_partial']!='************0000' ?  $_order['payment_card_partial']."\n" : "\n")
            ...
            'tax1_name' =>
                (isset($tax_columns[0]) ? $tax_columns[0]['name'] : ""),
            'tax2_name' =>
                (isset($tax_columns[1]) ? $tax_columns[1]['name'] : ""),
            'tax3_name' =>
                (isset($tax_columns[2]) ? $tax_columns[2]['name'] : ""),
            'tax4_name' =>
                (isset($tax_columns[3]) ? $tax_columns[3]['name'] : ""),
            'itemcode' =>
                implode("\n", $items_arr['itemcode']),
            'title' =>
                implode("\n", $items_arr['title']),
            'tax1' =>
                (isset($tax_columns[0]) ? implode("\n", $tax_columns[0]['costs']) : ""),
            'tax2' =>
                (isset($tax_columns[1]) ? implode("\n", $tax_columns[1]['costs']) : ""),
            'tax3' =>
                (isset($tax_columns[2]) ? implode("\n", $tax_columns[2]['costs']) : ""),
            'tax4' =>
                (isset($tax_columns[3]) ? implode("\n", $tax_columns[3]['costs']) : ""),
            'price' =>
                implode("\n", $items_arr['price']),
            'items' =>
                $items,
            'cost_items_pre_tax' =>
                $system_vars['defaultCurrencySymbol'].two_dp($_order['cost_items_pre_tax']),
            'shipping_method' =>
                $_order['SMethod'],
            'cost_shipping' =>
                ($_order['SMethod'] !='' ? $system_vars['defaultCurrencySymbol'].two_dp($_order['cost_shipping']) : ""),
            'tax1_name_rate' =>
                (isset($tax_columns[0]) ? $tax_columns[0]['name'].' at '.$tax_columns[0]['rate'].'%' : ""),
            'tax2_name_rate' =>
                (isset($tax_columns[1]) ? $tax_columns[1]['name'].' at '.$tax_columns[1]['rate'].'%' : ""),
            'tax3_name_rate' =>
                (isset($tax_columns[2]) ? $tax_columns[2]['name'].' at '.$tax_columns[2]['rate'].'%' : ""),
            'tax4_name_rate' =>
                (isset($tax_columns[3]) ? $tax_columns[3]['name'].' at '.$tax_columns[3]['rate'].'%' : ""),
            'tax1_total' =>
                (isset($tax_columns[0]) ? $system_vars['defaultCurrencySymbol'].two_dp($tax_columns[0]['cost']) : ""),
            'tax2_total' =>
                (isset($tax_columns[1]) ? $system_vars['defaultCurrencySymbol'].two_dp($tax_columns[1]['cost']) : ""),
            'tax3_total' =>
                (isset($tax_columns[2]) ? $system_vars['defaultCurrencySymbol'].two_dp($tax_columns[2]['cost']) : ""),
            'tax4_total' =>
                (isset($tax_columns[3]) ? $system_vars['defaultCurrencySymbol'].two_dp($tax_columns[3]['cost']) : ""),
            'paymentMethod' =>
                ((float)$_order['paymentMethodSurcharge'] ? $_order['paymentMethod']." charge" : ""),
            'paymentMethodSurcharge' =>
                ((float)$_order['paymentMethodSurcharge'] ? $_order['paymentMethodSurcharge']."%" : ""),
            'cost_grand_total' =>
                $system_vars['defaultCurrencySymbol'].two_dp($_order['cost_grand_total']),
            'paymentStatus' =>
  ...
  //    $rand = mt_rand(0,mt_getrandmax());
        ...
        $cmd =
            "pdftk ".$pdf_template_path." fill_form ".$path.$xfdf_filename." output ".$path.$pdf_filename." flatten";
  //    die($cmd);
        exec($cmd);
    ...
    public function draw_order_summary($payment_status)
    {
        $sql =
            "SELECT\n"
            ."  `oi`.*,\n"
            ...
            ."  `pg`.`description` AS `product_grouping_description`,\n"
    ...
    public function draw_person_details(
        $personID = '',
        $default_CountryID = '',
        $default_SpID = '',
        ...
        Output::push(
            ...
            ."  }\n"
            ."  if (geid_val('BCountryID')=='USA' || geid_val('BCountryID')=='CAN' || "
            ."geid_val('BCountryID')=='AUS' || geid_val('BCountryID')=='MEX') {\n"
            ...
            ."    if (geid_val('BSpID')!='--') {\n"
            ."      err_msg[n++] = (n)+\")"
            ." Please enter '--- Outside AUS / CAN / MEX / USA ---' as State / Province.\";\n"
            ."    }\n"
            ."  }\n"
        ...
        return
            ...
            :
                 "<p>Please provide valid billing details. A password will be emailled to you at the email address "
                ."you give to allow you to sign in later and view orders.</p>"
            )
            ...
            ."    <td style='width:".$label_width."px'>"
            ."<label for='checkout_NFirst'>Name (First</label> / "
            ."<label for='checkout_NMiddle'>Middle</label> / "
            ."<label for='checkout_NLast'>Last)</label>"
            ."</td>\n"
            ."    <td>"
    ...
    {
        $header =
            "Selected ".$this->_get_object_name().$this->plural($targetID)." with change history and ordered items";
        $extra_delete =
    ...
    public function get_notification_summary($datetime, $systemID, $base_url)
    {
        ...
        foreach ($records as $record) {
            ...
            $out.=
                 "  <tr>\n"
                ."    <td>"
                ."<a target=\"_blank\" href=\"".$base_url.'view_order?ID='.$record['ID']."\">".$record['ID']."</a>"
                ."</td>\n"
                ."    <td>"
    ...
    public function get_order_items($exclude_credit_memo_items = false)
    {
        if ($this->_get_ID()=="") {
            do_log(3, __CLASS__.'::'.__FUNCTION__.'():', '', "ID missing");
            return false;
  ...
  //    z($sql);
    ...
    public function get_registered_event_tickets()
    {
        if ($this->_get_ID()=="") {
            return false;
        }
  ...
  //    z($sql);die;
        ...
        $sql = "SET SESSION group_concat_max_len = 1024";
        $this->do_sql_query($sql);
        return $result;
    }
    ...
    public function issue_credit_memo($targetValue)
    {
    ...
    public static function manage_ecommerce_options()
    {
        global $sortBy;
        if (!isset($sortBy)) {
            $sortBy = 'value';
      ...
      // No toolbars as we have mixed reports here - filters for one may crash the other
        return
            ...
            ."</p>"
            ."<h3 style='margin:1em 0 0.15em 0'>"
            ."<a name='delivery_methods'></a>"
            ."Delivery Methods <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('lst_delivery_method', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='delivery_status'></a>"
            ."Delivery Status <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('lst_delivery_status', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='effective_period_units'></a>"
            ."Effective Period Units <a style='font-size:75%' href='#top'>Top</a></h3>"
            ...
            ."<h3 style='margin:1em 0 0.15em 0'><a name='gateway_settings_paypal'></a>"
            ."Gateway Settings - Paypal<a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('gateway_settings_paypal', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='payment_methods'></a>"
            ."Payment Methods <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('payment_method', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='payment_status'></a>"
            ."Payment Status <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('lst_payment_status', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='product_grouping_column_types'></a>"
            ."Product Grouping Column Types <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('lst_product_grouping_columns', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='product_types'></a>"
            ."Product Types <a style='font-size:75%' href='#top'>Top</a></h3>"
            .draw_auto_report('lst_product_type', 1)
            ."<h3 style='margin:1em 0 0.15em 0'><a name='refund_status'></a>"
            ."Refund Status <a style='font-size:75%' href='#top'>Top</a></h3>"
    ...
    public static function manage_refunds($admin = 0)
    {
        switch($admin) {
            ...
            ."<p>Click on an order item to edit it, or click the order link for complete "
            ."details of the originating order.</p>"
    ...
    public function mark_paid($prefix = '', $reveal_modification = true)
    {
        ...
        if ($method['method_pays_in_full']) {
            ...
            do_log(
                ...
                '',
                'Order '.$this->_get_ID().' set Payment Amount to '.$amount
            );
        }
    }

    public function on_update_process_payment_actions()
    {
    ...
    public function payment($payment_card_number, $gateway = false, $originating_page = '')
    {
  //    die("DEBUG: \$payment_card_number=".$payment_card_number.", \$gateway=".$gateway.x());
        ...
        switch($this->_gateway_record['type']['name']) {
            ...
            case "Chase Paymentech (Test)":
                ...
                $Obj_ChasePaymentech_Gateway->payment($this);
                // pass the current order to this function, draw the HTML redirector
                die();
            break;
            case "Paypal (Live)":
            case "Paypal (Test)":
                ...
                $Obj_PayPal_Gateway->payment($this);
                // pass the current order to this function, draw the HTML redirector
                die();
            break;
            default:
