<?php
define('VERSION_PAGE_VARS','1.0.24');
/*
Version History:
  1.0.24 (2014-12-29)
...
//    y($_SESSION['history']);
  ...
  protected function _get_colours(){
    ...
    if ($this->_vars['layout']['colour1']!='') { $this->_vars['colours']['colour1'] = $this->_vars['layout']['colour1'];}
    if ($this->_vars['layout']['colour2']!='') { $this->_vars['colours']['colour2'] = $this->_vars['layout']['colour2'];}
    if ($this->_vars['layout']['colour3']!='') { $this->_vars['colours']['colour3'] = $this->_vars['layout']['colour3'];}
    if ($this->_vars['layout']['colour4']!='') { $this->_vars['colours']['colour4'] = $this->_vars['layout']['colour4'];}
  }
...
//    y(array_keys($this->_vars)); y($this->_vars['navsuite1ID']);
  ...
  protected function _get_vars_for_mode(){
    global $mode, $ID, $report_name, $system_vars;
    if ($this->_resolved){
      return;
    }
    switch ($mode){
      ...
      case "report":
        ...
        if ($report_name && $report_row = $Obj->get_titles_for_name($report_name)){
          $this->_vars['title'] =       ($mode=='report' ? $report_row['reportTitle'] : $report_row['formTitle']);
        }
        else{
...
//    return;
  ...
  protected function _get_vars_for_type(){
    ...
    if ($record){
      ...
      $this->_vars['component_parameters'] =  (isset($record['component_parameters']) ? $record['component_parameters'] : "");
      ...
      $this->_vars['path'] =          (isset($record['path']) ? $record['path'] : $record['type']."/".$record['ID']);
      ...
      $this->_vars['title'] =         $this->_ObjKnownType->_get_object_name().": ".(isset($record['title']) ? $record['title'] : $record['titleEnglish']);
    }
    else {
      $this->_vars['absolute_URL'] =  trim($system_vars['URL'],'/').$this->_ObjKnownType->get_URL($record);
