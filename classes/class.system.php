...
class System extends Record
{
    ...
    public function copy($new_name = false, $new_systemID = false, $new_date = true, $data = false)
    {
        ...
        return $Obj->copy($new_name, $new_systemID, $new_date, $data);
    }
              ...
              // other operations
                ...
                case "get_bible_verse":
                    $url =
                        ...
                        ."content=dbv&type=js2&tw=auto&tbg=FFFFFF&bw=1&bc=000000&ta=C&tc=000000&"
                        ."tf=Arial&s=18&ty=B&va=L&vc=000000&vf=Arial&vs=12&tt=1&trn=".$_REQUEST['trn'];
                    $file = trim(@stripcslashes(file_get_contents($url)));
    ...
    public static function getArchivedSites()
    {
        $sql =
             "SELECT\n"
            ."    `ID`,\n"
    ...
    public static function get_css_checksum($ID)
    {
        ...
        $cs_icons =     static::get_item_version('icons');
        $cs_labels =    static::get_item_version('labels');
        $data =
    ...
    public static function get_display_title($withSystem = false)
    {
    ...
    public static function get_global_date_range($systemIDs_csv = SYS_ID)
    {
        if (!$systemIDs_csv) {
    ...
    public static function get_item_version($what = '')
    {
        ...
        switch (strToLower($what)) {
            ...
            case "akismet_key_status":
                if ($system_vars['akismet_api_key']=='') {
                    $status = 'No Key';
                } elseif (DEBUG_NO_INTERNET || $system_vars['debug_no_internet']==1) {
                    $status = 'Offline';
                } else {
            ...
            case "bugtracker_status":
                if ($system_vars['bugs_url']=='' ||
                    ...
                    $status = ($ObjBugTracker->connect() == '1' ? 'Pass' : 'Fail');
                }
                System::$cache_version[$what] = $status;
                break;
            case "build":
      ...
      //        y($checksums);die;
            ...
            case "labels":
                ...
                break;
            case "js_ajaxupload_cs":
                System::$cache_version[$what] =
                    FileSystem::get_file_checksum(SYS_JS."ajaxupload.min.js");
                break;
            case "js_ajaxupload_version":
                System::$cache_version[$what] =
                    trim(substr(FileSystem::get_line(SYS_JS."ajaxupload.min.js"), 3));
                break;
            case "js_cke":
                System::$cache_version[$what] =
    ...
    public function get_stats()
    {
        ...
        if ($r['stats_cache']) {
            ...
            if (isset($this->_stats['cache_date']) && $this->_stats['cache_date']==$end) {
                // Cached stats are fresh today, quit early
                return;
            }
        ...
        $this->_stats = array_merge($this->_stats, $Obj_Piwik->getSiteVisitsForMonths($start, $end));
    ...
    public function scrubPiiData()
    {
        ...
        foreach ($IDs as $ID) {
            ...
            $personIDs = $ObjPerson->get_IDs_by_system($ID);
            foreach ($personIDs as $personID) {
                $ObjPerson->_set_ID($personID);
                $ObjPerson->scrubPIIDataForPerson();
            }
        }
    }
