...
class System extends Record {
  ...
  static $cache_ID_by_URL_array =  array();
  static $cache_version = array();
  static $cache_version_hit = 0;
  static $cache_version_miss = 0;
  ...
  function do_commands() {
    ...
    if (isset($_REQUEST['command'])) {
      ...
      switch ($command) {
        ...
        case "version":
          if ($userIsAdmin){
            print "<p><b>".System::get_item_version('system_family')." version ".CODEBASE_VERSION."</b></p>";
            phpinfo();
            die;
          }
        break;
  ...
  function draw_css_include() {
	...
	return
	   "<link rel=\"stylesheet\" type=\"text/css\" href=\"".BASE_PATH."css/".System::get_item_version('css')."\" />\n"
	  ."<link rel=\"stylesheet\" type=\"text/css\" href=\"".BASE_PATH."css/labels/".System::get_item_version('css_labels')."\" />\n"
      .(!$editing_system ? "<link rel=\"stylesheet\" type=\"text/css\" href=\"".BASE_PATH."css/system/".$cs_system."\" />\n" : '')
      ;
  }

  function draw_js_include($full=false,$context_adminLevel=0) {
    global $system_vars;
    return
       "<script type=\"text/javascript\" src=\"".($system_vars['debug_no_internet']==1 ? BASE_PATH."sysjs/jquery/1.11.0" : "//ajax.googleapis.com/ajax/libs/jquery/1.11.0/jquery.min.js")."\"></script>\r\n"
      ."<script type=\"text/javascript\" src=\"".($system_vars['debug_no_internet']==1 ? BASE_PATH."sysjs/jqueryui/1.10.4" : "//ajax.googleapis.com/ajax/libs/jqueryui/1.10.4/jquery-ui.min.js")."\"></script>\r\n"
      ."<script type=\"text/javascript\" src=\"".BASE_PATH."sysjs/jqueryjson/2.4"."\"></script>\r\n"
      ."<script type=\"text/javascript\" src=\"".BASE_PATH."sysjs/sys/".System::get_item_version('js_functions')."\"></script>\r\n"
      .($full || get_userID() ? "<script type=\"text/javascript\" src=\"".BASE_PATH."sysjs/member/".System::get_item_version('js_member')."\"></script>\r\n" : '')
      .($context_adminLevel>0 ? "<script type=\"text/javascript\" src=\"".BASE_PATH."sysjs/context/".$context_adminLevel."/".System::get_item_version('codebase')."\"></script>\r\n" : '')
      ;
  }
  static function draw_popup_layer(){
    global $system_vars;
    ...
    switch($mode){
      ...
      case "dashboard":
        ...
        $out['html'] =  convert_safe_to_php("<div style='width:".$width."px;height:".$height."px;overflow:auto;border-bottom:1px solid #888;padding:4px;'>".$out['html']."</div>");
  ...
  function draw_remote_config($systems,$componentID) {
    ...
    $out.=
       ...
       ."  <option value=''".($servers==''? " selected='selected'":'').">Please select group to monitor</option>\n";
    foreach ($systems as $key=>$value) {
      $out.= "  <option".($key==$servers ? " selected='selected'" : "").">".$key."</option>\n";
    }
    $out.=
        "</select> "
       ."<input type='submit' value='Reload' class='formButton' id='btn_reload' onclick=\"this.disabled=1;geid('form').submit();\" disabled='disabled' /></p><br />";
    $Obj_Ajax = new Ajax;
  ...
  function get_display_title($withSystem=false) {
  ...
  function get_gateway($gateway_settingsID=false) {
    global $system_vars;
  ...
  function get_global_date_YYYY_MM($min,$max,$systemIDs_csv=SYS_ID,$isADMIN=false) {
    $sql =
       "SELECT\n"
  ...
  function get_global_date_range($systemIDs_csv=SYS_ID) {
    $sql =
       "SELECT\n"
  ...
  static function get_IDs_for_URLs($URLs_csv,$no_cache=false){
    if ($URLs_csv==''){
      return '';
    }
  ...
  public static function get_item_version($what=''){
    ...
    switch (strToLower($what)) {
      ...
      case "akismet_key_status":
        ...
        else if($system_vars['debug_no_internet']==1) {
          $status = 'No Web';
        }
        else {
      ...
      case "bugtracker_status":
        if ($system_vars['bugs_url']=='' || $system_vars['bugs_username']=='' || $system_vars['bugs_password']==''){
          $status = 'None';
        }
        else if ($system_vars['debug_no_internet']==1) {
          $status = 'No Web';
...
//        y($checksums);die;
      ...
      break;
      case "db_cs_status":
        System::$cache_version[$what] = (System::get_item_version('db_cs_actual')==System::get_item_version('db_cs_target') ? "Pass" : "Fail");
      break;
      case "db_cs_target":
        System::$cache_version[$what] = $system_vars['db_cs_target'];
      break;
      case "classes_cs_status":
        System::$cache_version[$what] = (System::get_item_version('classes_cs_actual')==System::get_item_version('classes_cs_target') ? "Pass" : "Fail");
      break;
      ...
      break;
      case "css":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."default.css"),3));
      break;
      case "css_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."default.css");
      break;
      case "css_breadcrumbs":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."breadcrumbs.css"),3));
      break;
      case "css_breadcrumbs_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."breadcrumbs.css");
      break;
      case "css_community":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."community.css"),3));
      break;
      case "css_community_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."community.css");
      break;
      case "css_labels":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."labels.css"),3));
      break;
      case "css_labels_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."labels.css");
      break;
      case "css_pie":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."pie.htc"),3));
      break;
      case "css_pie_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."pie.htc");
      break;
      case "css_spectrum":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_STYLE."spectrum.min.css"),3));
      break;
      case "css_spectrum_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_STYLE."spectrum.min.css");
      break;
      case "custom":
        System::$cache_version[$what] = (defined('CUSTOM_VERSION') ? CUSTOM_VERSION.(defined('TREB_VERSION') ? ", TREB: ".TREB_VERSION : "") : "");
      break;
      case "db_connect":
        return DB_CONNECT;
      break;
      case "db_connect_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_SHARED."/db_connect.php");
      break;
      case "db_name":
        System::$cache_version[$what] = $db;
      break;
      case "db_detail":
        System::$cache_version[$what] = $system_vars['db_detail'];
      break;
      case "db_cstarget":
        System::$cache_version[$what] = $system_vars['db_cstarget'];
      break;
      case "db_version":
        System::$cache_version[$what] = $system_vars['db_version'];
      break;
      case "document_root":
        System::$cache_version[$what] = $_SERVER["DOCUMENT_ROOT"];
      break;
      case "fedex_rate":
        System::$cache_version[$what] = FileSystem::get_file_version(SYS_SHARED."/fedex/rate.php",1);
      break;
      case "fedex_rate_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_SHARED."/fedex/rate.php");
      break;
      case "functions":
        System::$cache_version[$what] = FileSystem::get_file_version(SYS_SHARED."/functions.php",1);
      break;
      case "functions_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_SHARED."/functions.php");
      break;
      case "getid3":
        System::$cache_version[$what] = FileSystem::get_file_version(SYS_SHARED."/getid3/getid3.php",1);
      break;
      case "getid3_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_SHARED."/getid3/getid3.php");
      break;
      case "http_software":
        System::$cache_version[$what] = $_SERVER["SERVER_SOFTWARE"];
      break;
      case "icons":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_IMAGES."icons.gif");
      break;
      case "icons_big":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_IMAGES."icons-big.gif");
      break;
      case "img":
        System::$cache_version[$what] = FileSystem::get_file_version(SYS_SHARED."img.php",1);
      break;
      case "img_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_SHARED."img.php");
      break;
      case "js_cke":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/ckeditor.js"),3));
      break;
      case "js_cke_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/ckeditor.js");
      break;
      case "js_cke_config":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/config.js"),3));
      break;
      case "js_cke_config_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/config.js");
      break;
      case "js_cke_plugin_audio":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/audio/plugin.js"),3));
      break;
      case "js_cke_plugin_audio_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/audio/plugin.js");
      break;
      case "js_cke_plugin_ecl":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/ecl/plugin.js"),3));
      break;
      case "js_cke_plugin_ecl_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/ecl/plugin.js");
      break;
      case "js_cke_plugin_more":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/more/plugin.js"),3));
      break;
      case "js_cke_plugin_more_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/more/plugin.js");
      break;
      case "js_cke_plugin_video":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/video/plugin.js"),3));
      break;
      case "js_cke_plugin_video_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/video/plugin.js");
      break;
      case "js_cke_plugin_youtube":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/youtube/plugin.js"),3));
      break;
      case "js_cke_plugin_youtube_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/youtube/plugin.js");
      break;
      case "js_cke_plugin_zonebreak":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ckeditor/plugins/zonebreak/plugin.js"),3));
      break;
      case "js_cke_plugin_zonebreak_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ckeditor/plugins/zonebreak/plugin.js");
      break;
      case "js_ecc":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."ecc.js"),3));
      break;
      case "js_ecc_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."ecc.js");
      break;
      case "js_functions":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."functions.js"),3));
      break;
      case "js_functions_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."functions.js");
      break;
      case "js_jdplayer":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."jdplayer/mediaelement-and-player.min.js"),3));
      break;
      case "js_jdplayer_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."jdplayer/mediaelement-and-player.min.js");
      break;
      case "js_jquery":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."jquery.min.js"),3));
      break;
      case "js_jquery_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."jquery.min.js");
      break;
      case "js_jquery_ui":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."jquery-ui.min.js"),3));
      break;
      case "js_jquery_ui_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."jquery-ui.min.js");
      break;
      case "js_member":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."member.js"),3));
      break;
      case "js_member_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."member.js");
      break;
      case "js_rssreader_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."rss_reader.js");
      break;
      case "js_rssreader_version":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."rss_reader.js"),3));
      break;
      case "js_spectrum_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."spectrum.min.js");
      break;
      case "js_spectrum_version":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."spectrum.min.js"),3));
      break;
      case "js_treeview_cs":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_JS."treeview.js");
      break;
      case "js_treeview_version":
        System::$cache_version[$what] = trim(substr(FileSystem::get_line(SYS_JS."treeview.js"),3));
      break;
      case "labels":
        System::$cache_version[$what] = FileSystem::get_file_checksum(SYS_IMAGES."labels.gif");
      break;
      ...
      case "libraries_cs_status":
        System::$cache_version[$what] = (System::get_item_version('libraries_cs_actual') == System::get_item_version('libraries_cs_target') ? 'Pass' : 'Fail');
      break;
      case "libraries_cs_target":
        System::$cache_version[$what] = ($system_vars['libraries_cs_target']);
      break;
      case "libraries_detail":
        System::$cache_version[$what] = ($system_vars['libraries_detail']);
      break;
      case "mysql":
        $sql =      "SELECT VERSION()";
        $Obj = new System;
        System::$cache_version[$what] = trim($Obj->get_field_for_sql($sql));
      break;
      case "php":
        System::$cache_version[$what] = phpversion();
      break;
      case "reports_cs_target":
        System::$cache_version[$what] = ($system_vars['reports_cs_target']);
      break;
      case "reports_detail":
        System::$cache_version[$what] = ($system_vars['reports_detail']);
      break;
      case "server_name":
        System::$cache_version[$what] = $_SERVER["SERVER_NAME"];
      break;
      case "system":
        System::$cache_version[$what] = (defined("SYSTEM_VERSION") ? SYSTEM_VERSION : "?");
      break;
      case "system_family":
        System::$cache_version[$what] = SYSTEM_FAMILY;
      break;
      case "system_family_url":
        System::$cache_version[$what] = SYSTEM_FAMILY_URL;
      break;
      default:
        System::$cache_version[$what] = "?";
      break;
  ...
  function get_last_loggedin_access(){
  ...
  function set_parameters_for_instance($instance,$data) {
  ...
  function update_posting_prefix($posting_prefix){
    $Obj = new Posting;
    $sql =
       "SELECT\n"
      ."  `postings`.`ID`,\n"
  ...
  function upgrade($target_buildID) {
    $record =           $this->get_record();
  ...
  function upgrade_check($start_buildID,$target_buildID){
  ...
  function upgrade_execute($target_buildID){
    $Obj = new gwSocket('');
