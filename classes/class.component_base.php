<?php
define ("VERSION_COMPONENT_BASE","1.0.17");
/*
Version History:
  1.0.17 (2014-05-19)
    1) Tweak to Component_Base::get_safe_ID() to deal with posibility that an instance-name
...
class Component_Base extends Record {
  ...
  function get_parameter($parameters,$key,$default=false) {
  ...
  public static function get_parameter_defaults_and_values(
    $ident, $instance, $force_values, $parameter_spec, $presets=array()
  ){
  ...
  function get_parameter_for_instance($instance,$parameter_csv,$key,$default=false) {
    if ($parameter_csv==="") {
      return $default;
    }
  ...
  public static function get_parameters($page=false,$systemID=SYS_ID) {
    ...
    return
       ($_page_vars['component_parameters']!="" ? $_page_vars['component_parameters'].option_separator : "")
      .($_page_vars['layout_component_parameters']!="" ? $_page_vars['layout_component_parameters'].option_separator : "")
      .($_system_vars['component_parameters']!="" ? $_system_vars['component_parameters'] : "")
      ;
  }
  ...
  function help($name,$params,$instance='') {
    ...
    foreach ($param_arr as $param) {
      ...
      $cp_site[] =      Component_Base::get_parameter_for_instance($instance,$system_vars['component_parameters'],$temp[0],'');
      $cp_layout[] =    Component_Base::get_parameter_for_instance($instance,$page_vars['layout_component_parameters'],$temp[0],'');
      $cp_item[] =      Component_Base::get_parameter_for_instance($instance,$page_vars['component_parameters'],$temp[0],'');
      if (isset($temp[1])){
    ...
    if ($ID_safe_name==0){
      Page::push_content('javascript',"cp_params=[];\n"
      );
    }
    Page::push_content('javascript',"cp_params[".$ID_safe_name."] = ".json_encode($cp_args).";\n"
    );
    $out =
  ...
  protected function _setup_load_parameters(){
    $cp_settings =
      Component_Base::get_parameter_defaults_and_values(
        $this->_ident, $this->_instance, $this->_disable_params, $this->_parameter_spec, $this->_args
      );
  ...
  protected function _setup_load_user_rights(){
    ...
    $this->_current_user_rights['canRate'] =            0;
    $this->_current_user_rights['canEdit'] =            ($isMASTERADMIN || $isSYSADMIN || $isSYSAPPROVER || $isSYSEDITOR ? 1 : 0);
    $this->_current_user_rights['canPublish'] =         ($isMASTERADMIN || $isSYSADMIN || $isSYSAPPROVER ? 1 : 0);
    $this->_current_user_rights['isPUBLIC'] =           ($isPUBLIC);
    $this->_current_user_rights['isSYSMEMBER'] =        ($isMASTERADMIN || $isSYSADMIN || $isSYSAPPROVER || $isSYSEDITOR || $isSYSMEMBER || $isUSERADMIN ? 1 : 0);
    $this->_current_user_rights['isSYSADMIN'] =         ($isSYSADMIN || $isMASTERADMIN ? 1 : 0);
