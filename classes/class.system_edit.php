<?php
define('VERSION_SYSTEM_EDIT','1.0.30');
...
class System_Edit extends System {
  ...
  private function _do_save(){
    ...
    $data =
      array(
        ...
        'component_parameters'=>        addslashes(implode(option_separator,explode("\r\n",get_var('component_parameters')))),
    ...
    if ($this->_isMASTERADMIN) {
      $data =
        array(
          ...
          'qbwc_invoice_type'=>                 addslashes(sanitize('enum',get_var('qbwc_invoice_type'),array('I','S'))),
    ...
    if ($this->_isMASTERADMIN && get_var('posting_prefix') && get_var('posting_prefix')!=$this->_posting_prefix_old) {
  ...
  private function _do_save_colourscheme(){
    ...
    if ($test_result) {
      $this->_msg = "<b>Error</b> - you already have an identical colour scheme defined - <b>".$test_result['name']."</b>";
  ...
  function draw() {
    ...
    $this->_html.=
      ...
      ."<input type='button' id='close_btn' value='Close' onclick=\"window.close()\" class='formbutton' style='width: 60px;'/>\n"
      ."<input type='button' id='save_btn' value='Save' onclick=\"geid('close_btn').disabled=1;geid('save_and_close_btn').disabled=1;this.disabled=1;show_popup_please_wait();geid('submode').value='save';geid('form').submit();\" class='formbutton' style='width: 60px;'/>\n"
      ."<input type='button' id='save_and_close_btn' value='Save and Close' onclick=\"geid('close_btn').disabled=1;geid('save_btn').disabled=1;this.disabled=1;show_popup_please_wait();geid('submode').value='save_and_close';geid('form').submit();\" class='formbutton' style='width: 120px;'/>\n"
      ."</div>\n"
      ."</div>\n";
    return $this->_html;
  }
  ...
  private function _draw_css(){
    ...
    Page::push_content(
      'style_include',
      "<link rel=\"stylesheet\" type=\"text/css\" href=\"".$base_path."css/system/nocache/".dechex(mt_rand(0,mt_getrandmax()))."\" />"
    );
    Page::push_content(
      'style',
       ".settings_group_header { padding:1px 2px 1px 2px; font-size: 80%; }\n"
      .".settings_group { margin: 0 2px 10px 2px; padding:1px; border:1px solid #c0c0c0; background-color:#fff; font-size: 80% }\n"
  ...
  private function _draw_section_advanced(){
    ...
    $this->_html.=
      ...
      ."    <label style='width:150px' for='debug'><b>SQL Debug File</b><br />(Affects performance)</label>"
      ."    <div class='val'>".draw_form_field("debug",$this->record['debug'],"bool")."</div>\n"
      ."    <label style='width:200px' for='debug'><b>No Internet Mode</b><br />(Disables external services)</label>"
      ."    <div class='val'>".draw_form_field("debug_no_internet",$this->record['debug_no_internet'],"bool")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val'>".draw_form_field("smtp_host",$this->record['smtp_host'],"text","125")."</div>"
      ."    <label style='width:40px; text-align:right' for='smtp_port'>Port</label>\n"
      ."    <div class='val'>".draw_form_field("smtp_port",$this->record['smtp_port'],"int","25")."</div>"
      ."    <label style='width:40px; text-align:right' for='smtp_authenticate'>Auth.</label>\n"
      ."    <div class='val'>".draw_form_field("smtp_authenticate",$this->record['smtp_authenticate'],"bool")."</div>\n"
      ."    <label style='width:80px;text-align:right;' for='smtp_username'>Username</label>\n"
      ."    <div class='val'>".draw_form_field("smtp_username",$this->record['smtp_username'],"text","100")."</div>"
      ."    <label style='width:86px;text-align: right;' for='smtp_password'>Password</label>\n"
      ."    <div class='val'>".draw_form_field("smtp_password",$this->record['smtp_password'],"password","100")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val'>".draw_form_field("gatewayID",$this->record['gatewayID'],"selector","130px",Gateway_Setting::get_selector_sql())."</div>"
      ."    <label style='width:110px;text-align:right;' for='defaultCurrencySymbol'>Currency Symbol</label>\n"
      ."    <div class='val'>".draw_form_field("defaultCurrencySymbol",$this->record['defaultCurrencySymbol'],"selector_listdata",'30','','','','','','','lst_currency_symbols')."</div>"
      ."    <label style='width:110px;text-align:right;' for='defaultCurrencySuffix'>Currency Suffix</label>\n"
      ."    <div class='val'>".draw_form_field("defaultCurrencySuffix",$this->record['defaultCurrencySuffix'],"text","30px")."</div>"
      ."    <label style='width:80px;text-align:right;' for='defaultTaxZoneID'>Tax Zone</label>\n"
      ."    <div class='val'>".draw_form_field("defaultTaxZoneID",$this->record['defaultTaxZoneID'],"selector","130px",Tax_Regime::get_selector_sql())."</div>"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:80px;'>Tax Benefits</label>\n"
      ."    <div class='val'>".draw_form_field("tax_benefit_1_name",$this->record['tax_benefit_1_name'],'text','130px')."</div>"
      ."    <label style='width:20px;' for='tax_benefit_1_name'>#1</label>\n"
      ."    <div class='val'>".draw_form_field("tax_benefit_2_name",$this->record['tax_benefit_2_name'],'text','130px')."</div>"
      ."    <label style='width:20px;' for='tax_benefit_2_name'>#2</label>\n"
      ."    <div class='val'>".draw_form_field("tax_benefit_3_name",$this->record['tax_benefit_3_name'],'text','130px')."</div>"
      ."    <label style='width:20px;' for='tax_benefit_3_name'>#3</label>\n"
      ."    <div class='val'>".draw_form_field("tax_benefit_4_name",$this->record['tax_benefit_4_name'],'text','130px')."</div>"
      ...
      ."    <div class='val'>".draw_form_field("qbwc_user",$this->record['qbwc_user'],'text','130px')."</div>\n"
      ."    <label style='width:90px' for='qbwc_pass'>Password</label>"
      ."    <div class='val'>".draw_form_field("qbwc_pass",$this->record['qbwc_pass'],'password','130px')."</div>\n"
      ."    <label style='width:90px' for='qbwc_invoice_type'>Invoice Type</label>"
      ."    <div class='val'>".draw_form_field("qbwc_invoice_type",$this->record['qbwc_invoice_type'],'radio_csvlist','','',0,'',0,0,'','I|Invoice,S|Sales Order')."</div>\n"

//function draw_form_field($field,$value,$type,$width="",$selectorSQL="",$reportID=0,$jsCode="",$readOnly=0,$bulk_update=0,$label="",$formFieldSpecial='',$height='') {

      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:90px' for='qbwc_AssetAccountRef'>Asset Account</label>"
      ."    <div class='val'>".draw_form_field("qbwc_AssetAccountRef",$this->record['qbwc_AssetAccountRef'],'text','130px')."</div>\n"
      ."    <label style='width:90px' for='qbwc_COGSAccountRef'>GOGS Account</label>"
      ."    <div class='val'>".draw_form_field("qbwc_COGSAccountRef",$this->record['qbwc_COGSAccountRef'],'text','130px')."</div>\n"
      ."    <label style='width:100px' for='qbwc_IncomeAccountRef'>Income Account</label>"
      ."    <div class='val'>".draw_form_field("qbwc_IncomeAccountRef",$this->record['qbwc_IncomeAccountRef'],'text','130px')."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:90px' for='qbwc_export_orders'>Export Orders</label>"
      ."    <div class='val'>".draw_form_field("qbwc_export_orders",$this->record['qbwc_export_orders'],'selector_csvlist','130px','','','','','','','0|(None)|d0d0d0,2|All|e0ffe0')."</div>\n"
      ."    <label style='width:90px' for='qbwc_export_people'>Export People</label>"
      ."    <div class='val'>".draw_form_field("qbwc_export_people",$this->record['qbwc_export_people'],'selector_csvlist','130px','','','','','','','0|(None)|d0d0d0,1|Customers|ffe0c0,2|All|e0ffe0')."</div>\n"
      ."    <label style='width:100px' for='qbwc_export_products'>Export Products</label>"
      ."    <div class='val'>".draw_form_field("qbwc_export_products",$this->record['qbwc_export_people'],'selector_csvlist','130px','','','','','','','0|(None)|d0d0d0,1|Ordered|ffe0c0,2|All|e0ffe0')."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:90px' for='qbwc_export_orders_billing_addr'>Billing Addr.</label>"
      ."    <div class='val' style='width:134px'>".draw_form_field("qbwc_export_orders_billing_addr",$this->record['qbwc_export_orders_billing_addr'],'radio_csvlist','','',0,'',0,0,'','0|QB,1|Send')."</div>\n"
      ."    <label style='width:90px' for='qwbc_export_orders_product_desc'>Product Desc.</label>"
      ."    <div class='val' style='width:134px'>".draw_form_field("qbwc_export_orders_product_desc",$this->record['qbwc_export_orders_product_desc'],'radio_csvlist','','',0,'',0,0,'','0|QB,1|Send')."</div>\n"
      ."    <label style='width:100px' for='qbwc_export_orders_taxcodes'>Item Taxes.</label>"
      ."    <div class='val' style='width:130px'>".draw_form_field("qbwc_export_orders_taxcodes",$this->record['qbwc_export_orders_taxcodes'],'radio_csvlist','','',0,'',0,0,'','0|QB,1|Send')."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:200px'><b>Custom tables used (CSV list)</b></div>\n"
      ."    <div class='val'>".draw_form_field("db_custom_tables",$this->record['db_custom_tables'],"text",550)."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:200px'><b>Posting Prefix Format</b></div>\n"
      ."    <div class='val'>"
      .draw_form_field("posting_prefix",$this->record['posting_prefix'],"selector_listdata",550,'','','','','','','lst_posting_prefix|0')
      .(get_var('posting_prefix') && $this->_posting_prefix_old && get_var('posting_prefix')!=$this->_posting_prefix_old ?
      ...
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group_header'><b>News and Event sources - get shared news and events from these sites:</b><br />\n"
      ."(<b>Performance Tip:</b> Enclose source URL in brackets if the site lives in current database.)<br />"
      .draw_form_field("provider_list",$this->record['provider_list'],"csv",($this->_width-10),'','','','',false,'','',60)."\n"
      ."  </div>\n"
      ."</div>";
  }
  ...
  private function _draw_section_css(){
    $this->_html.=
      ...
      ."  <div class='settings_group_header'><b>These styles apply any page in this site but can be overridden at the layout or page level.</b><br />\n"
      .draw_form_field("style",$this->record['style'],"textarea",($this->_width-10),'','','','',false,'','',355)
      ."  </div>"
      ."</div>\n";
  }
  ...
  private function _draw_section_colours(){
    $this->_html.=
      ...
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl'><b>Colour Scheme</b>".($this->_msg!="" ? " &nbsp; <span style='color:#f00'>".$this->_msg."</span>" : "")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:120px' for='colour_schemeID'>Use this:</label>\n"
      ."    <div class='val'>"
      .draw_form_field("colour_schemeID",$this->_colour_schemeID,"selector","350px",Colour_Scheme::get_selector_sql())." "
      ."<input type='button' class='formButton' style='width: 90px' value='Load' "
      ."onclick=\"if (geid_val('colour_schemeID')==1) { alert('Please select a colour scheme to load'); } else { if(confirm('This action will overwrite the current scheme - continue?')) { geid_set('submode','load_scheme');geid('form').submit();} else {alert('Action Cancelled')}}\"/>\n"
      ."<input type='button' class='formButton' style='width: 90px' value='Save As...' "
      ."onclick=\"var cs_name=prompt('Please enter a name for this scheme','');if(cs_name){geid_set('targetValue',cs_name);geid_set('submode','save_scheme');geid('form').submit();}\"/>\n"
      ."<input type='button' class='formButton' style='width: 90px' value='Delete' "
      ."onclick=\"if (geid_val('colour_schemeID')==1) { alert('Please select a colour scheme to delete'); } else if(geid('colour_schemeID').options[geid('colour_schemeID').selectedIndex].text.substr(0,2)=='* ') { alert('You cannot delete a globally defined colour scheme'); } else if(confirm('Delete this colour scheme?')) {geid('submode').value='delete_scheme';geid('form').submit();}\"/>\n"
      ."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val' style='width:130px'>".draw_form_field("defaultBgColor",$this->record['defaultBgColor'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='text_heading' title='Text Heading Colour'>Text Headings</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("text_heading",$this->record['text_heading'],"swatch","60px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val' style='width:130px'>".draw_form_field("table_border",$this->record['table_border'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='table_header' title='Table Header Cells Colour'>Table Headers</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("table_header",$this->record['table_header'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='table_data' title='Table Data Cells Colour'>Table Data</label>\n"
      ."    <div class='val' style='width:100px'>".draw_form_field("table_data",$this->record['table_data'],"swatch","60px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val' style='width:130px'>".draw_form_field("colour1",$this->record['colour1'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='colour2' title='Accent Colour #2'>Accent # 2</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("colour2",$this->record['colour2'],"swatch","60px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:120px' for='colour3' title='Accent Colour #3'>Accent # 3</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("colour3",$this->record['colour3'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='colour4' title='Accent Colour #4'>Accent # 4</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("colour4",$this->record['colour4'],"swatch","60px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl'><b>Calendar Colour Scheme</b></div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div style='float:right;height:200px'><div class='constrain'><div style='position:relative;padding:0px 10px'>"
      ...
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_border",$this->record['cal_border'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_current' title='Current Weekday'>Current</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_current",$this->record['cal_current'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_head' title='Calendar Heading'>Heading</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_head",$this->record['cal_head'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_days' title='Day Headings'>Day Headings</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_days",$this->record['cal_days'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_current_we' title='Current Weekend'>Current Wknd</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_current_we",$this->record['cal_current_we'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_today' title=\"Today's date\">Today's Date</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_today",$this->record['cal_today'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_then' title='Inactive Weekday'>Inactive Weekday</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_then",$this->record['cal_then'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_event' title='Event indicator ring'>Event</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_event",$this->record['cal_event'],"swatch","60px")."</div>\n"
      ."    <label style='width:120px' for='cal_then_we' title='Inactive Weekend'>Inactive Weekend</label>\n"
      ."    <div class='val' style='width:130px'>".draw_form_field("cal_then_we",$this->record['cal_then_we'],"swatch","60px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."</div>";
  }
  ...
  private function _draw_section_features(){
    ...
    $this->_html.=
      ...
      .draw_form_field("features",$this->record['features'],"checkbox_listdata_csv","360",'','','','',false,'','lst_features|1',340)
      ."</td>\n"
      ."                    <td class='va_t txt_c' style='height:300px;'>\n"
      ."<b>&nbsp;Modules installed on this site</b><br />\n"
      .draw_form_field("installed_modules",$this->record['installed_modules'],"checkbox_csvlist_scrollbox","360",'','','','',false,'',$this->get_modules_available(),340)
      ."</td>\n"
      ."                  </tr>\n"
      ."                </table></div>";
  ...
  private function _draw_section_general(){
    $this->_html.=
      ...
      ."    <div class='val' style='float:left;width:250px;'>".draw_form_field("textEnglish",$this->record['textEnglish'],"text","250")."</div>"
      ."    <div style='float:left;width:362px;text-align:right'><b>URL</b> ".draw_form_field("URL",$this->record['URL'],"text","250")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:160px'><b>Admin Name</b></div>\n"
      ."    <div class='val'>".draw_form_field("adminName",$this->record['adminName'],"text","250")."</div>"
      ."    <div class='lbl' style='width:104px;text-align:right'><b>Admin Email</b>&nbsp;</div>\n"
      ."    <div class='val'>".draw_form_field("adminEmail",$this->record['adminEmail'],"text","250")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:414px'>&nbsp;</div>\n"
      ."    <div class='lbl' style='width:104px;text-align:right'><b>Bounce Email</b>&nbsp;</div>\n"
      ."    <div class='val'>".draw_form_field("bounce_email",$this->record['bounce_email'],"text","250")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:160px'><b>New Access Accounts</b></div>\n"
      ."    <div class='val'>"
      ."      <div style='float:left;width:250px;'>".draw_form_field("system_signup",$this->record['system_signup'],"selector_listdata","250",'','','','','','','lst_system_signup_options')."</div>"
      ."      <div style='float:left;width:362px;text-align:right'><b>Event Cancellation notice (days) </b> ".draw_form_field("system_cancellation_days",$this->record['system_cancellation_days'],"int","20")."</div>\n"
      ."    </div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ...
      ."    <div class='val'>".draw_form_field("notify_email",$this->record['notify_email'],"text",550)."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <label style='width:200px' for='notify_triggers'>Notify on these Triggers</label>\n"
      ."    <div class='val'>".draw_form_field("notify_triggers",$this->record['notify_triggers'],"checkbox_listdata_csv",550,'','','','',false,'','lst_notify_triggers|1',20)."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:170px'><b>Default Layout</b></div>\n"
      ."    <div class='val'>".draw_form_field("defaultLayoutID",$this->record['defaultLayoutID'],"selector","608px",Layout::get_selector_sql(false))."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:170px'><b>Default Theme</b></div>\n"
      ."    <div class='val'>".draw_form_field("defaultThemeID",$this->record['defaultThemeID'],"selector","608px",Theme::get_selector_sql(false))."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:170px'><b>Displayed Date Format</b></div>\n"
      ."    <div class='val'>".draw_form_field("defaultDateFormat",$this->record['defaultDateFormat'],"combo_listdata","608px","","","","","","","lst_date_format")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:170px'><b>Displayed Time Format</b></div>\n"
      ."    <div class='val'>".draw_form_field("defaultTimeFormat",$this->record['defaultTimeFormat'],"selector_listdata","608px","","","","","","","lst_time_format")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:170px'><b>Default Timezone</b></div>\n"
      ."    <div class='val'>"
      .draw_form_field("timezone",$this->record['timezone'],"selector_timezone","608px")
      ."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:170px'><b>Akismet Key <a href='http://wordpress.com/signup/' rel='external'>Get WP Key</a></b></div>\n"
      ."    <div class='val' style='width:510px'>".draw_form_field("akismet_api_key",$this->record['akismet_api_key'],"text","100px")."</div>\n"
      ...
      ."    <div class='val' style='width:200px;'>".draw_form_field("bugs_url",$this->record['bugs_url'],"selector_listdata","200px",'','','','','','','lst_bugtracker_URLs')."</div>\n"
      ."    <div class='lbl txt_r' style='width:50px;'><b>Email</b>&nbsp;</div>\n"
      ."    <div class='val' style='width:155px;'>".draw_form_field("bugs_username",$this->record['bugs_username'],"text","155px")."</div>\n"
      ."    <div class='lbl txt_r' style='width:50px'><b>Pwd</b>&nbsp;</div>\n"
      ."    <div class='val' style='width:55px;'>".draw_form_field("bugs_password",$this->record['bugs_password'],"password","45px")."</div>\n"
      ...
      ."    <div class='val' style='width:200px;'>".draw_form_field("piwik_id",$this->record['piwik_id'],"int","100px")."</div>\n"
      ."    <div class='lbl txt_r' style='width:50px'><b>User</b>&nbsp;</div>\n"
      ."    <div class='val' style='width:155px;'>".draw_form_field("piwik_user",$this->record['piwik_user'],"text","155px")."</div>\n"
      ."    <div class='lbl txt_r' style='width:50px'><b>Token</b>&nbsp;</div>\n"
      ."    <div class='val' style='width:155px;'>".draw_form_field("piwik_token",$this->record['piwik_token'],"text","155px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div class='lbl' style='width:170px'><b>Google Analytics <a href='http://www.google.com/analytics' rel='external'>Get ID</a></b></div>\n"
      ."    <div class='val'>".draw_form_field("google_analytics_key",$this->record['google_analytics_key'],"text","100px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      ."  <div class='settings_group'>\n"
      ."    <div class='lbl' style='width:170px'><b>Path to Favicon file</b><br />(16x16px .ico format)</div>\n"
      ."    <div class='val'>".draw_form_field("favicon",$this->record['favicon'],"server_file","600px")."</div>\n"
      ."    <div class='clr_b'></div>\n"
      ."  </div>\n"
      .($this->_isMASTERADMIN || System::has_feature('multi-language') ?
           "  <div class='settings_group'>\n"
          ."    <div class='lbl' style='width:170px'><b>Default Language</b></div>\n"
          ."    <div class='val' style='width:150px'>".draw_form_field("defaultLanguage",$this->record['defaultLanguage'],"selector_listdata","150px",'','','','','','','lst_iso-639-1')."</div>\n"
          ."    <div class='lbl txt_r' style='width:85px'><b>Available</b>&nbsp;</div>\n"
          ."    <div class='val'>".draw_form_field("languages",$this->record['languages'],"selector_listdata_csv","360px",'','','','','','','lst_iso-639-1','30')."</div>\n"
          ."    <div class='clr_b'></div>\n"
          ."  </div>\n"
        : ""
       )
  ...
  private function _draw_section_logs(){
    ...
    $this->_html.=
      ...
      ."<input type=\"button\" value=\"Delete selected\" onclick=\"geid_set('submode','delete_file');geid('form').submit();\"/>\n"
      ."    </div>\n"
      ."    <div class='clr_b'></div>\n"
      ."    <div>"
      .($dirTree!==false ? $Obj_FS->draw_dir_tree($dirTree,0,true,'Log Files',true) : "Sorry - you cannot yet view logs for another system.")
      ."    </div>\n"
      ."  </div>"
      ."</div>";
  }
  ...
  private function _draw_section_membership_rules(){
    ...
    $this->_html.=
      ...
      .draw_form_field("membership_rules",$this->record['membership_rules'],"bool")." "
      ."&nbsp;Membership Expiry: "
      .draw_form_field("membership_expiry_type",$this->record['membership_expiry_type'],"selector_listdata","200px",'','','','','','','lst_membership_expiry_type')."</b><br />\n"
      ."  </div>\n"
      ."<a class=\"iframe\" href=\"".BASE_PATH."report/membership_rules_for_system?print=2&amp;selectID=".$this->record['ID']."\" rel=\"style=width:".($this->_width-10)."px;height:400px;\">Embedded Content</a>\n"
      ."</div>";
  }
  ...
  private function _draw_section_notes(){
    ...
    $this->_html.=
      ...
      .draw_form_field("notes",$this->record['notes'],"textarea",($this->_width-10),'','','','','','','',400)."\n"
      ."</div>";
  }
  ...
  private function _draw_section_parameters(){
    $this->_html.=
      ...
      ."  <div class='settings_group_header'><b>These Parameters apply any page in this site but can be overridden at the layout or page level.</b><br />\n"
      .draw_form_field("component_parameters",$this->record['component_parameters'],"option_list",($this->_width-10),'','','','',false,'','',360)
      ."  </div>\n"
      ."</div>";
  }
  ...
  private function _draw_section_status(){
    ...
    $this->_html.=
      ...
      ."  <table cellpadding='2' border='0' cellspacing='0' class='admin_containerpanel' width='".($this->_width-10)."'>\n"
      ."    <tr>\n"
      ."      <td style='width:50%'><b>This site</b></td>\n"
      ."      <td style='width:50%'><b>Remote site</b> "
      ."<input class='formField' type=\"text\" name=\"remote_url\" id=\"remote_url\" value=\"".$remote_url."\" style=\"width: 220px;\" onkeypress=\"keytest_enter_transfer(event,'go_btn')\" /> "
      ."<input class='formButton' type='button' id=\"go_btn\" value='Go' style='width:25px;' onclick=\"geid('save_btn').disabled=1;geid('close_btn').disabled=1;geid('save_and_close_btn').disabled=1;geid('go_btn').value='...';geid('go_btn').disabled=1;geid('submode').value='connect';geid('form').submit()\"/></td>\n"
      ."    </tr>\n"
      ."    <tr>\n"
      ."      <td>"
