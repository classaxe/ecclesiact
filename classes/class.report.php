...
class Report extends Displayable_Item {
  ...
  static $cache_feature_array =     array();
  static $cache_titles =            false;
  static $cache_popup_size =        array();
  ...
  function __construct($ID='') {
    ...
    $this->_set_has_groups(false); // Needs to stay false - Reports are used on multiple sites, having group_assign_csv
  ...
  function actions_execute($trigger,$primaryTable,$primaryObject,$targetID,$data=array(),&$msg=''){
    switch ($trigger) {
  ...
  function convert_xml_field_for_filter(&$field,$table){
    ...
    $field =
      preg_replace(
        "/(xml:[^ =\b]+)/e", "\$replacement_sql[\$match_num++]",
        $field
      );
  }

  function convert_xml_field_for_sort(&$field,$table){
    // This code converts xml virtual fields into nested sql matches to isolate the required node
    preg_match_all('(xml:[^ =\b]+)', $field, $xml_fields,PREG_SET_ORDER);
    $replacement_sql = array();
    foreach($xml_fields as $xml_field){
  ...
  function download_data($ID,$field) {
  ...
  function download_userfile_data($ID,$field) {
    $table = $this->get_field('primaryTable');
    $Obj = new Record($table,$ID);
    $value = $Obj->get_field($field);
    if ($value!='') {
  ...
  function draw_form($report_name,$controls=1,$alt_controls=''){
    $Obj_Report_Form = new Report_Form;
    return $Obj_Report_Form->draw($report_name,$controls,$alt_controls);
  }

  function draw_form_view($report_name,$ID=false,$doQuery=false,$headers=true,$with_fields=true,$language='en',$forEmail=false) {
...
//              z($selectorSQL);
              break;
              default:
                $selectorSQL = (get_person_permission("MASTERADMIN") ? $formSelectorSQLMaster : $formSelectorSQLMember);
              break;
            }
            switch ($type) {
              case "html":
...
//    die($out);
  ...
  function draw_print_form($report_name,$ID) {
    return $this->draw_form_view($report_name,$ID,true,true,false);
  }

  function draw_report($report_name,$toolbar,$ajax_mode=false) {
    deprecated();
    $Obj = new Report_Report($this->_get_ID());
    return $Obj->draw($report_name,$toolbar,$ajax_mode);
  }

  function email_form($targetID,$subject,$to,$cc='',$bcc='') {
    global $system_vars;
    $report_name =  $this->get_name();
    $body_html =    $this->draw_form_view($report_name,$targetID,true,true,false,$system_vars['defaultLanguage'],true);
...
//    print_r($data);die;
  ...
  function email_sql($targetID,$subject,$to,$cc='',$bcc='') {
    global $system_vars;
    $report_name = $this->get_name();
    $Obj = new Export();
...
//    print_r($data);die;
  ...
  function export_sql($targetID,$show_fields) {
    ...
    $extra_delete =
       "DELETE FROM `group_assign`           WHERE `assign_type` = 'Report Column' AND `assignID` IN (SELECT `ID` FROM `report_columns` WHERE `reportID` IN(".$targetID.") AND `systemID` IN (1,".SYS_ID."));\n"
      ."DELETE FROM `report_columns`         WHERE `reportID` IN ($targetID) AND `systemID` IN(1,".SYS_ID.");\n"
      ."DELETE FROM `report_filter_criteria` WHERE `filterID` IN (SELECT `ID` FROM `report_filter`  WHERE `reportID` IN (".$targetID.")".$clause.");\n"
      ."DELETE FROM `report_filter`          WHERE `reportID` IN (".$targetID.")".$clause.";\n"
      ."DELETE FROM `report_settings`        WHERE `reportID` IN (".$targetID.")".$clause.";\n"
      ;
    $Obj = new Backup;
    $extra_select =
       $Obj->db_export_sql_query("`group_assign`          ","SELECT * FROM `group_assign` WHERE `assign_type` = 'Report Column' AND `assignID` IN (SELECT `ID` FROM `report_columns` WHERE `reportID` IN(".$targetID.") AND `systemID` IN (1,".SYS_ID.")) ORDER BY `ID`",$show_fields)
      .$Obj->db_export_sql_query("`report_columns`        ","SELECT * FROM `report_columns` WHERE `reportID` IN (".$targetID.") AND `systemID` IN(1,".SYS_ID.") ORDER BY `reportID`,`tab`,`seq`,`ID`",$show_fields)
      .$Obj->db_export_sql_query("`report_filter`         ","SELECT * FROM `report_filter` WHERE `reportID` IN (".$targetID.")".$clause." ORDER BY `ID`",$show_fields)
      .$Obj->db_export_sql_query("`report_filter_criteria`","SELECT * FROM `report_filter_criteria` WHERE `filterID` IN (SELECT `ID` FROM `report_filter`  WHERE `reportID` IN (".$targetID.")".$clause.") ORDER BY `ID`",$show_fields)
      .$Obj->db_export_sql_query("`report_settings`       ","SELECT * FROM `report_settings` WHERE `reportID` IN (".$targetID.")".$clause." ORDER BY `ID`",$show_fields)
      ;
  ...
  public static function get_and_set_sortOrder($report_row,$columnList,$sortBy){
    ...
    if ($sortBy!="") {
      ...
      if ($_save_default_columnID) {
        ...
        $_Obj_Report_Defaults->set_defaults($report_row['ID'],$_save_default_columnID,$_save_default_column_reverse);
      }
    }
    return $sortBy;
  }
  ...
  private function _get_columns_remove_overridden(){
    ...
    $overridden =   array();
    foreach ($this->_report_columns as $record){
      $key = $record['fieldType']."-".$record['formField']."-".$record['tab']."-".$record['required_feature']."-".$record['required_feature_invert'];
  ...
  function _get_columns_sort_function($a, $b){
  ...
  function get_email_addresses() {
    ...
    $sql =  (get_person_permission("MASTERADMIN") ? $report_record['reportSQL_MASTERADMIN'] :  $report_record['reportSQL_SYSADMIN']);
    $sql =
  ...
  static function get_match_mode($name) {
  ...
  static function get_match_modes() {
    $Obj_Listtype = new Listtype;
    $Obj_Listtype->_set_ID($Obj_Listtype->get_ID_by_name('lst_report_match_mode'));
    return $Obj_Listtype->get_listdata('seq');
  }

  static function get_ObjPrimary($report_name,$primaryObjectName){
    $ObjPrimary = false;
...
//      y($sql);
  ...
  function get_popup_sizes_for_names(&$result,$names_csv){
...
//    y($result);
  ...
  function get_records($report_record,$columnList=false,$filterField=false,$filterExact=false,$filterValue=false,$unused_field=0,$limit=-1,$offset=0,$debug=false){
    $where =
      ($filterField!==false  ?
...
//    z($sql);
  ...
  function get_records_count($report_record,$filterField,$filterExact,$filterValue,$unused_field=0,$debug=false) {
    $where =
      ($filterField!==false  ?
         Report_Report::get_filter($filterField,$filterExact,$filterValue)
       : "");
    ...
    $pos = (strpos($query,'FROM #primary')!==false ? strpos($query,'FROM #primary') : strrpos($query,'FROM'));
    $sql =
      get_sql_constants(
...
//    z($sql);
  ...
  public static function get_sortby($columnList,$reportPrimaryTable) {
    ...
    for ($i=0; $i<count($columnList); $i++) {
      ...
      if ($label!="" && strpos($label,"Export")==null && strpos($label,"Del")==null && $sortBy == $columnList[$i]['reportField']) {
        $sortBy_columns = $columnList[$i]['reportSortBy_a'];
        break;
      }
      if ($label!="" && strpos($label,"Export")==null && strpos($label,"Del")==null && $sortBy == $columnList[$i]['reportField']."_d") {
  ...
  function get_tabs_array($columnList,&$selected_section) {
    $tab_arr = array();
    foreach($columnList as $column) {
    ...
    if (count($tab_arr)==1) { // If there's only one tab, don't bother
      $tab_arr=array();
    }
    return $tab_arr;
  }
  function get_titles_for_name($report_name) {
...
//       z($sql);
  ...
  function handle_copy($report_name,$args,&$newID,&$msg,&$msg_tooltip){
    global $system_vars;
  ...
  function handle_delete($report_name,$args,&$msg){
    global $system_vars;
    $targetID =             $args['targetID'];
  ...
  function test_feature($test_csv) {
