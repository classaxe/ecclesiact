...
class Podcast extends Posting_Contained {
  ...
  function __construct($ID="",$systemID=SYS_ID) {
    ...
    $this->_cp_vars_detail = array(
      'audioplayer_width' =>        array('match' => 'range|0,n',       'default' => '360',             'hint' => '0|x'),
      'author_show' =>              array('match' => 'enum|0,1',        'default' => '0',               'hint' => '0|1'),
      'block_layout' =>             array('match' => '',                'default' => 'Podcast',         'hint' => 'Name of Block Layout to use'),
      ...
      'item_footer_component' =>    array('match' => '',                'default' => '',                'hint' => 'Name of component rendered below displayed Podcast'),
      ...
      'thumbnail_show' =>           array('match' => 'enum|0,1',        'default' => '0',               'hint' => '0|1 - Whether or not to allow thumbnail to show'),
      'title_linked' =>             array('match' => 'enum|0,1',        'default' => '1',               'hint' => '0|1'),
      'title_show' =>               array('match' => 'enum|0,1',        'default' => '1',               'hint' => '0|1'),
      'video_height' =>             array('match' => 'range|0,n',       'default' => '300',             'hint' => '0|x'),
      'video_width' =>              array('match' => 'range|0,n',       'default' => '400',             'hint' => '0|x'),
      'video_show' =>               array('match' => 'enum|0,1',        'default' => '1',               'hint' => '0|1 - Whether or not to allow video to show'),
    );
    $this->_cp_vars_listings = array(
      'audioplayer_width' =>        array('match' => 'range|0,n',       'default' => '180',             'hint' => '0..x'),
      'author_show' =>              array('match' => 'enum|0,1',        'default' => '0',               'hint' => '0|1'),
      ...
      'results_order' =>            array('match' => 'enum|date,date_a,title', 'default' => 'date',            'hint' => 'date|date_a|atitle'),
      ...
      'thumbnail_show' =>           array('match' => 'enum|0,1',        'default' => '',                'hint' => '0|1 - Whether or not to allow thumbnail to show'),
  ...
  protected function BL_audio_clip(){
    if ($this->record['enclosure_url']!=''){
      $width = (isset($this->_cp['audioplayer_width']) ?  $this->_cp['audioplayer_width'] :  $this->_cp_defaults['audioplayer_width']);
      return
        "[audio:"
  ...
  protected function BL_link_for_audioplayer(){
    ...
    $url =
       ($this->record['systemID']!=SYS_ID && substr($this->record['enclosure_url'],0,4)!="http" ? trim($this->record['systemURL'],"/")."/" : "")
      ."?command=podcast_player&amp;targetID=".$this->record['ID'];
    return
       "<a class='item_download' href=\"".$url."\" onclick=\"popWin(this.href,'".$this->record['ID']."','resizable=1',320,120);return false;\" "
      ." rel='external'"
  ...
  function draw_player(){
    ...
    $media_URL =
      ($record['systemID']!=SYS_ID && substr($record['enclosure_url'],0,4)!="http" ?
...
*/
    ...
    if (substr($url,0,4)!='http') {
      ...
      if ($record['date']=='' || $record['date']=='0000-00-00'){
        ...
        else if ($date = sanitize('date-stamp',substr($result['filename'],0,4).'-'.substr($result['filename'],4,2).'-'.substr($result['filename'],6,2))){
          // YYYYMMDD:
          $out['date'] = $date;
        }
        else {
      ...
      if (isset($result['fileformat']) && $result['fileformat']=='mp3'){
        ...
        if ($record['title']=='' || $record['title']=='Untitled'){
          $find = array('"');
          $replace = array("'");
          if (isset($result['tags']['id3v2']['title'][0]) && trim($result['tags']['id3v2']['title'][0])!='') {
            $out['title'] = trim(str_replace($find,$replace,$result['tags']['id3v2']['title'][0]));
          }
          else if (isset($result['id3v1']['comments']['title'][0]) && trim($result['id3v1']['comments']['title'][0])!='') {
            $out['title'] = trim(str_replace($find,$replace,$result['id3v1']['comments']['title'][0]));
          }
          else if (isset($result['id3v1']['title']) && trim($result['id3v1']['title'])) {
            $out['title'] = trim(str_replace($find,$replace,$result['id3v1']['title']));
          }
          else {
            $out['title'] = 'Untitled';
          }
        ...
        if ($record['author']=='') {
          $find = array('"');
          $replace = array("'");
          if (isset($result['tags']['id3v2']['artist'][0]) && $result['tags']['id3v2']['artist'][0]!='') {
            $out['author'] = str_replace($find,$replace,$result['tags']['id3v2']['artist'][0]);
          }
          else if (isset($result['id3v1']['comments']['artist'][0]) && $result['id3v1']['comments']['artist'][0]!='') {
            $out['author'] = str_replace($find,$replace,$result['id3v1']['comments']['artist'][0]);
          }
          else if (isset($result['id3v1']['artist']) && $result['id3v1']['artist']!='') {
            $out['author'] = str_replace($find,$replace,$result['id3v1']['artist']);
          }
        }
        ...
        if ($record['content']=='') {
          $find = array('"');
          $replace = array("'");
          if (isset($result['tags']['id3v2']['comments'][0]) && $result['tags']['id3v2']['comments'][0]) {
            $out['content'] = str_replace($find,$replace,$result['tags']['id3v2']['comments'][0]);
          }
          else if (isset($result['id3v1']['comments']['comments'][0]) && $result['id3v1']['comments']['comments'][0]) {
            $out['content'] = str_replace($find,$replace,$result['id3v1']['comments']['comments'][0]);
          }
          else if (isset($result['id3v1']['comments'][0]) && $result['id3v1']['comments'][0]) {
            $out['content'] = str_replace($find,$replace,$result['id3v1']['comments'][0]);
          }
          else {
            $out['content'] = '';
          }
  ...
  static function on_action_get_metadata(){
