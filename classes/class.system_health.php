<?php
define('VERSION_SYSTEM_HEALTH','1.0.41');
...
class System_Health extends System {
  ...
  private function _draw_button_code_documentation($config_arr,$ID){
    ...
    foreach($changed_files as $changed_file){
      switch ($changed_file){
        case 'codebase.php':
          $changed_files_list.=
             "  ".pad($changed_file,52).System::get_item_version('codebase')."\n"
        ...
        default:
          ...
          $history_arr =          explode("\n",$history);
          $history_version =      trim(substr(array_shift($history_arr),3));
          $history =              implode("\n",$history_arr);
          $changed_files_list.=
             "  "
            .pad(trim($changed_file),50)
            .$history_version
            .$history;
        break;
      }
  ...
  private function _draw_button_sql_build_info($config_arr,$ID){
    ...
    foreach ($config_arr as $config){
      if ($config['category']=='reports' && $config['title']!='reports_cs_actual'){
        $title =      $config['title'];
        if(substr($title,0,4)!='cus_' && substr($title,0,7)!='module.' && substr($title,0,12)!='custom_treb_'){
          $detail =     str_replace(",","",$config['content']);
  ...
  private function _draw_db_row_checksum_table($config, $url, $db_name, $final_cs_fail){
    ...
    return
       "<tr"
      .($customised && $correct ? " class='warning' title='This table has been customised,\nbut still meets the checksum criteria'" : "")
      .($customised && $incorrect ? " class='bad' title='This table has been customised\nand in addition it fails the checksum criteria - expected CS was ".$expected_cs."'" : "")
      .(!$customised && $correct ? " class='unchanged' title='This table has not changed'" : "")
      .(!$customised && $incorrect && $expected_cs===false ? " class='bad' title='This table was added resulting in changes to final checksum'" : "")
      .(!$customised && $incorrect && $expected_cs!==false ? " class='bad' title='This table was altered resulting in changes to final checksum - expected CS was ".$expected_cs."'" : "")
      .(!$customised && $custom ? " class='ignored' title='Custom tables do not affect the final checksum'" : "")
      .(!$customised && $module ? " class='module' title='Module tables do not affect the final checksum but do generate their own combined checksums'" : "")
      .">\n"
      ."  <th>"
  ...
  private function _draw_classes_table($config_arr){
    ...
    foreach ($config_arr as $config){
      ...
      // Regular entries
        ...
        else if ($expected_arr['expected_version']=='0'){
          $out.= "  <tr class='good' title='New addition for this build'>\n";
        }
        else if ($version!=$expected_arr['expected_version']){
          $out.=
  ...
  private function _draw_libraries_table_row($config_arr,$entry) {
    ...
    if ($libraries_detail){
      ...
      if (trim($entry[0],'/')=='codebase.php' && $version!=$expected_v){
        $tr = "  <tr class='good' title=\"Version has changed but the checksum cannot measured without affecting the result - it's quantum.\">\n";
        $checksum = "Quantum";
      }
      else if (trim($entry[0],'/')!='codebase.php' && $checksum==''){
        $tr = "  <tr class='warning' title='The file checksum is unmonitored in this build'>\n";
      }
      else if ($checksum == $expected_cs){
        $tr = "  <tr class='unchanged' title='No changes'>\n";
      }
      else if ($expected_cs=='-1'){
        $tr = "  <tr class='good' title='New addition for this build'>\n";
      }
      else if ($version!=$expected_v){
        $tr =
           "  <tr class='good' title='Changed, new version number supplied -\n"
  ...
  private function _draw_reports_table($config_arr){
    ...
    foreach ($config_arr as $config){
      ...
      // Regular entries
        ...
        else if (substr($title,0,7)=='module.'){
          $out.= "  <tr class='module' title='Module reports do not affect the final checksum'>\n";
        }
        else if ($expected_arr['expected_final']=='0'){
          $out.= "  <tr class='good' title='New addition for this build'>\n";
        }
        else if ($final == $expected_arr['expected_final']){
          $out.= "  <tr class='unchanged' title='No changes'>\n";
        }
        else {
          $out.=
        ...
        $out.=
           "    <th><span class='link' title=\"Click to export this report\" onclick=\"return export_sql('report',".$ID.")\">".$report."</span></th>\n"
          ...
          case $entry:
            switch ($entry) {
              ...
              case "classes_cs_target":
                $out.=
                  ($content==$classes_cs_actual ?
                    "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                    "<tr class='bad'><th>".$label."</th><td><span title='Actual value'>".$classes_cs_actual."</span> <span style='color:#ff8000' title='Expected value'>(Exp ".$content.")</span></td></tr>\n"
                   );
              break;
              case "classes_cs_status":
                $out.=
                  ($content=='Pass' ?
                     "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                     "<tr class='bad'><th><a href='#Classes'>".$label."</a></th><td>".$content."</td></tr>\n"
                  );
              break;
              case "db_cs_target":
                $out.=
                  ($content==$system_checksum_final ?
                    "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                    "<tr class='bad'><th>".$label."</th><td><span title='Actual value'>".$system_checksum_final."</span> <span style='color:#ff8000' title='Expected value'>(Exp ".$content.")</span></td></tr>\n"
                   );
              break;
              case "db_cs_status":
                $out.=
                  ($content=='Pass' ?
                     "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                     "<tr class='bad'><th><a href='#DB'>".$label."</a></th><td>".$content."</td></tr>\n"
                  );
              break;
              ...
              case "libraries_cs_status":
                $out.=
                  ($content=='Pass' ?
                     "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                     "<tr class='bad'><th><a href='#Libraries'>".$label."</a></th><td>".$content."</td></tr>\n"
                  );
              break;
              case "libraries_cs_target":
                $out.=
                  ($content==$libraries_cs_actual ?
                    "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                    "<tr class='bad'><th>".$label."</th><td><span title='Actual value'>".$libraries_cs_actual."</span> <span style='color:#ff8000' title='Expected value'>(Exp ".$content.")</span></td></tr>\n"
                   );
              break;
              case "reports_cs_status":
                $out.=
                  ($content=='Pass' ?
                     "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                     "<tr class='bad'><th><a href='#Reports'>".$label."</a></th><td>".$content."</td></tr>\n"
                  );
              break;
              case "reports_cs_target":
                $out.=
                  ($content==$reports_cs_actual ?
                    "<tr class='good'><th>".$label."</th><td>".$content."</td></tr>\n"
                   :
                    "<tr class='bad'><th>".$label."</th><td><span title='Actual value'>".$reports_cs_actual."</span> <span style='color:#ff8000' title='Expected value'>(Exp ".$content.")</span></td></tr>\n"
                   );
              break;
              case "akismet_key_status":
                $out.= "<tr class='".($content=='Pass' ? 'good' : 'bad')."'><th>".$label."</th><td>".$content."</td></tr>\n";
              break;
              ...
              case "build_version":
                if ($content) {
                  $out.=
                    "<tr><th>".$label."</th><td><a href=\"#\" onclick=\"version('".$content."');return false;\">"
                   .$content."</a></td></tr>\n";
                }
                else {
                  $out.= "<tr class='bad'><th>".$label."</th><td>ID not in database</td></tr>\n";
                }
              break;
              case "URL":
                $out.= "<tr><th>".$label."</th><td><a href=\"".$content."\" rel=\"external\">".$content."</a></td></tr>\n";
              break;
              default:
                $out.= "<tr><th>".$label."</th><td>".$content."</td></tr>\n";
              break;
            }
          break;
        }
  ...
  private function _get_config_reports(&$out){
    ...
    foreach ($reports as $report){
      ...
      if(substr($report['name'],0,4)!='cus_' && substr($report['name'],0,7)!='module.' && substr($report['name'],0,12)!='custom_treb_'){
  ...
  private function _get_config_system(&$out,$initial){
    ...
    System::$cache_version['classes_cs_status'] =   (System::$cache_version['classes_cs_actual']==System::get_item_version('classes_cs_target') ? "Pass" : "Fail");
    System::$cache_version['db_cs_status'] =        (System::$cache_version['db_cs_actual']==System::get_item_version('db_cs_target') ? 'Pass' : 'Fail');
    ...
    System::$cache_version['libraries_cs_status'] = (System::$cache_version['libraries_cs_actual']==System::get_item_version('libraries_cs_target') ? 'Pass' : 'Fail');
    System::$cache_version['reports_cs_status'] = (System::$cache_version['reports_cs_actual']==System::get_item_version('reports_cs_target') ? 'Pass' : 'Fail');
    $entries =
      array(
        ...
        array('last_loggedin_access',   ($access['history_datetime'] ? $access['PUsername']." (".$access['history_datetime'].")" : "")),
        array('codebase_version',       System::get_item_version('codebase')),
  ...
  protected function _get_config_tables(&$out){
    ...
    foreach ($out as &$entry) {
      if ($entry['category']=='tables'){
        ...
        else if (substr($entry['title'],0,4)=="cus_"){
          //
        }
        else {
