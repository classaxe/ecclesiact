...
class Community extends Displayable_Item {
  ...
  protected function _check_dropbox(){
    ...
    sscanf($this->_community_record['dropbox_last_checked'],"%4s-%2s-%2s %2s:%2s:%2s",$YYYY, $MM, $DD, $hh, $mm, $ss);
      ...
      if ($record['primary_communityID']==$this->_community_record['ID']){
        if ($record['dropbox_folder']){
          ...
          switch ($num_files){
            case 0:
              $status =   1;
              $tooltip =  "No items pending\n[Checked every ".format_seconds($check_period)." min, next check in ".format_seconds($check_period-$diff)." min]";
            break;
            case 1:
              $status =   2;
              $tooltip =  "One item pending:\n- ".implode("\n- ",$files)."\n[Checked every ".format_seconds($check_period)." min, next check in ".format_seconds($check_period-$diff)." min]";
            break;
            default:
              $status =   2;
              $tooltip =  $num_files." items pending:\n- ".implode("\n- ",$files)."\n[Checked every ".format_seconds($check_period)." min, next check in ".format_seconds($check_period-$diff)." min]";
            break;
          }
        }
      }
  ...
  public function get_communities(){
    $sql =
       "SELECT\n"
      ."  (SELECT COUNT(*) FROM `community_membership` WHERE `community_membership`.`communityID`=`".$this->_get_table_name()."`.`ID`) `members`,\n"
  ...
  public function get_members(){
    $sql =
       "SELECT\n"
      ."  (SELECT `system`.`textEnglish` FROM `system` WHERE `system`.`ID` = `community_member`.`systemID`) `systemTitle`,\n"
      ."   CONCAT(TRIM(TRAILING '/' FROM `community`.`URL`),'/',`community_member`.`name`) AS `member_URL`,\n"
      ."  (LENGTH(`partner_csv`) - LENGTH(replace(`partner_csv`,',','')) + if(LENGTH(`partner_csv`),1,0)>=`partner_min`) as `compliance`,\n"
      ."  LENGTH(`partner_csv`) - LENGTH(replace(`partner_csv`,',','')) + if(LENGTH(`partner_csv`),1,0) `partners`,\n"
  ...
  public function export_sql($targetID,$show_fields){
    ...
    $extra_delete =
       "DELETE FROM `community_member`       WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.");\n"
      ."DELETE FROM `community_membership`   WHERE `communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.");\n"
      ."DELETE FROM `category_assign`        WHERE `assignID` IN (SELECT `ID` FROM `postings` where `communityID` IN(".$targetID."));\n"
      ."DELETE FROM `category_assign`        WHERE `assignID` IN (SELECT `ID` FROM `postings` where `memberID` IN(SELECT `ID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")));\n"
      ."DELETE FROM `postings`               WHERE `communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.");\n"
      ."DELETE FROM `postings`               WHERE `memberID` IN (SELECT `ID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.");\n"
      ."DELETE FROM `person`                 WHERE `ID` IN (SELECT `contactID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.");\n"
      ;
    $extra_select =
       Backup::db_export_sql_query("`community_member`      ","SELECT * FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      .Backup::db_export_sql_query("`community_membership`  ","SELECT * FROM `community_membership` WHERE `communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      .Backup::db_export_sql_query("`category_assign`       ","SELECT * FROM `category_assign` WHERE `assignID` IN (SELECT `ID` FROM `postings` where `communityID` IN(".$targetID."));\n",$show_fields)
      .Backup::db_export_sql_query("`category_assign`       ","SELECT * FROM `category_assign` WHERE `assignID` IN (SELECT `ID` FROM `postings` where `memberID` IN(SELECT `ID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")));\n",$show_fields)
      .Backup::db_export_sql_query("`postings`              ","SELECT * FROM `postings` WHERE `communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      .Backup::db_export_sql_query("`postings`              ","SELECT * FROM `postings` WHERE `memberID` IN (SELECT `ID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      .Backup::db_export_sql_query("`person`                ","SELECT * FROM `person` WHERE `ID` IN (SELECT `contactID` FROM `community_member` WHERE `primary_communityID` IN (".$targetID.") AND `systemID` IN (1,".SYS_ID.")) AND `systemID` IN (1,".SYS_ID.")",$show_fields)
      ;
  ...
  protected function _on_action_community_setup_dropbox(){
    ...
    if (!$r['dropbox_app_key']){
      ...
      $msg.=
        ...
        ."Please provide these or <a target='new_window' href='https://www.dropbox.com/login?cont=https%3A//www.dropbox.com/developers/apps'>click here</a> to create them.";
      return;
    }
    ...
    if (isset($_SESSION['DropLib_RequestToken'])){
      ...
      $msg.=
         "Please <a target='new_window' href='".$url."'>click here</a> to sign in to dropbox and confirm the request to connect, then save this community member one more time.";
    }
  }
  ...
  protected function _on_action_community_setup_home_page(){
    ...
    $data = array(
      ...
      'layoutID' =>             1,
      'meta_description' =>     $this->record['URL_external']." - Churches and Christian organisations in ".$this->record['title'],
  ...
  public function _on_action_community_setup_website_button(){
    ...
    $RGB =              ImageColorAllocate($this->_img, HexDec(substr($color,0,2)), HexDec(substr($color,2,2)), HexDec(substr($color,4,2)));
