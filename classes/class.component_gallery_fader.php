<?php
define ("VERSION_COMPONENT_GALLERY_FADER","1.0.41");
/*
Version History:
  1.0.41 (2014-01-28)
...
class Component_Gallery_Fader extends Component_Base {
  ...
  public function __construct(){
    ...
    $this->_parameter_spec =    array(
      'caption_show' =>             array('match' => 'enum|0,1',        'default' =>'1',            'hint'=>'0|1'),
      'content_background' =>       array('match' => '',                'default' =>'404040',       'hint'=>'Background colour for caption'),
      'content_color' =>            array('match' => '',                'default' =>'ffffff',       'hint'=>'Text colour for caption'),
      'content_height' =>           array('match' => 'range|1,n',       'default' =>'40',           'hint'=>'Height in pixels to make caption area'),
      'content_opacity' =>          array('match' => 'range|0,100',     'default' =>'90',           'hint'=>'Opacity in % for caption area'),
      'controls_show' =>            array('match' => 'enum|0,1',        'default' =>'1',            'hint'=>'0|1'),
      'controls_size' =>            array('match' => 'enum|s,m,l',      'default' =>'s',            'hint'=>'s|m|l'),
      'count_show' =>               array('match' => 'enum|0,1',        'default' =>'1',            'hint'=>'0|1 - Showns (n of m) on title line'),
      'cover_image' =>              array('match' => '',                'default' =>'',             'hint'=>'Path to an image to use as the cover image - will show intially if given'),
      ...
      'maintain_aspect' =>          array('match' => 'enum|0,1',        'default' =>'1',            'hint'=>'If set to 0 the image will be resized to the exact width and height given'),
      'max_height' =>               array('match' => 'range|1,n',       'default' =>'200',          'hint'=>'Maximum height in pixels'),
      'max_width' =>                array('match' => 'range|1,n',       'default' =>'200',          'hint'=>'Maximum width in pixels'),
      'onchange' =>                 array('match' => '',                'default' =>'',             'hint'=>'javascript to execute when an image changes'),
      'random_start' =>             array('match' => 'enum|0,1',        'default' =>'0',            'hint'=>'0|1 - Whether to start with first image or play from a random position'),
      ...
      'title' =>                    array('match' => '',                'default' =>'',             'hint'=>'If given, set title of all images to this'),
      'title_prefix' =>             array('match' => '',                'default' =>'',             'hint'=>'Text to place before each title where shown'),
    ...
    $this->_html.=
      ...
      .($this->_first_image['url'] ?
        ...
        .($this->_first_image['url_popup'] ?
           "popWin('".$this->_first_image['url']."','url_".$instance."','location=1,status=1,scrollbars=1,resizable=1',720,400,1)"
         :
      ...
      ." style=\"opacity:0;filter:alpha(opacity = 0);\" height=\"".$this->_cp['max_height']."\" width=\"".$this->_cp['max_width']."\"/>\n"
  ...
  protected function _draw_controls(){
    ...
    $this->_html.= "  <div id=\"".$this->_safe_ID."_controls\" style=\"opacity:0;filter:alpha(opacity=0);\"></div>\n";
  }
  ...
  protected function _draw_css(){
    ...
    Page::push_content(
      'style',
       "#".$this->_safe_ID." {\n"
      ."  position:relative; margin:auto; width:100%; height:".($this->_max_height ? $this->_max_height : $this->_cp['max_height'])."px;\n"
      ."}\n"
      ."#".$this->_safe_ID."_background {\n"
      ."  background: #".$this->_cp['content_background'].";\n"
      ."}\n"
      ."#".$this->_safe_ID."_content {\n"
      ."  position:absolute; z-index:1; padding:5px; width:".($this->_cp['max_width']-10)."px; height:".$this->_cp['content_height']."px; top:".($this->_max_height-10-$this->_cp['content_height'])."px;\n"
      ...
      ."  position:absolute; z-index:2; margin:5px; padding:0; width:".(5*($btn_size+5))."px; height:".$btn_size."px; left:".($this->_cp['max_width']-(5*($btn_size+5)+5))."px;\n"
      ."}\n"
      ."#".$this->_safe_ID."_mask{\n"
      ."  position:relative; z-index:1; margin:auto; width:100%; height:".$this->_max_height."px;\n"
      ."}\n"
      ."#".$this->_safe_ID."_status{\n"
      ."  position:absolute; z-index:2; width:".$this->_cp['max_width']."px; height:".$this->_max_height."px; top:0;\n"
      ."}\n"
      ...
      ."#".$this->_safe_ID."_background{\n"
      ."  position:absolute; z-index:1; padding:5px; width:".($this->_cp['max_width']-10)."px; height:".$this->_cp['content_height']."px; top:".($this->_max_height-10-$this->_cp['content_height'])."px;\n"
      ."  opacity:".($this->_cp['content_opacity']/100).";filter: alpha(opacity=".$this->_cp['content_opacity'].");\n"
      ."}\n"
    ...
    foreach ($this->_records as $record){
      ...
      if (file_exists($_filename)){
        ...
        if ($img) {
          ...
          if ($height>$this->_max_height) { $this->_max_height = $height; }
        }
      }
    }
  }
  ...
  protected function _setup_load_records(){
    ...
    $args =     array(
      ...
      'filter_container_path' =>    (isset($this->_cp['filter_container_path']) ?   $this->_cp['filter_container_path'] : ''),
      'filter_container_subs' =>    (isset($this->_cp['filter_container_subs']) ?   $this->_cp['filter_container_subs'] : ''),
      'filter_memberID' =>          (isset($this->_cp['filter_memberID']) ?         $this->_cp['filter_memberID'] : ''),
      'filter_offset' =>            (isset($_REQUEST['offset']) ? $_REQUEST['offset'] : 0),
      'filter_personID' =>          (isset($this->_cp['filter_personID']) ?         $this->_cp['filter_personID'] : ''),
      'filter_limit' =>             $this->_cp['results_limit'],
      'filter_order_by' =>          (isset($this->_cp['results_order']) ?           $this->_cp['results_order'] : 'date')
    );
  ...
  protected function _setup_images(){
    foreach ($this->_records as $record){
      ...
      $_image =
        ...
        .(isset($record['thumbnail_cs_small']) && $record['thumbnail_cs_small'] ? "&cs=".$record['thumbnail_cs_small'] : "");
      ...
      $_title =     str_replace(array("'","\r","\n"),array("\'","\\r","\\n"),($this->_cp['title_prefix'] ? $this->_cp['title_prefix'] : "").$record['title']);
