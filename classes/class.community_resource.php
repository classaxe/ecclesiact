<?php
define('COMMUNITY_RESOURCE_VERSION','1.0.1');
/* Custom Fields used:
...
class Community_Resource extends Community_Display{
  ...
  public function draw($cp,$path_extension,$community_record){
    ...
    if (
      Portal::_parse_request_search_range(
        $this->_path_extension, $page, $search_date_start, $search_date_end, $search_type
    )){
  ...
  protected function _draw_rss(){
    ...
    $request =  (isset($path_arr[1]) ? $path_arr[1] : '');
    $Obj_RSS =  new RSS;
//    y($this);die;
    $args =
      array(
        ...
        'submode' =>        $request,
        'title' =>          $page_vars['title']." > RSS".($request ? " > ".title_case_string($request) : ""),
...
//      y($search_results);
  ...
  protected function _draw_sermons(){
    ...
    $args = array(
       'text_prompt_to_choose' =>       '<h2>Sermons</h2><p>Please choose a member or speaker to view their sermons.</p>',
