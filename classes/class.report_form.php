<?php
define ("VERSION_REPORT_FORM","1.0.59");
...
//    foreach($this->_columnList as $c){if ($c['required_feature']){y($c);}};die;
    ...
    $this->_html.=
      ...
      ."    <td class='va_t' style='height:".($this->_form_settings['height']-($this->_controls ? 58 : 30))."px;background-image: none;'>";
    ...
    $this->_html.=
      ...
      .($this->_controls ?
        ...
        ."<input type='button' value='Save' onclick=\"this.value='Please Wait...';this.disabled=1;geid('submode').value='save';geid('form').submit();\" class='formbutton' style='width: 60px;'/>\n"
        ."<input type='button' value='Save and Close' onclick=\"this.value='Please Wait...';this.disabled=1;geid('submode').value='save_and_close';geid('form').submit();\" class='formbutton' style='width: 120px;'/>\n"
        .(!$this->_bulk_update && $this->_can_add!==false ? "<input type='button' value='Save and New...' onclick=\"this.value='Please Wait...';this.disabled=1;geid('submode').value='save_and_new';geid('form').submit();\" class='formbutton' style='width: 120px;'/>\n" : "")
        ."</td>\n"
        ."  </tr>\n"
      : ""
      )
  ...
  protected function _draw_form_field($c){
    ...
    $reportTab =		$c['tab'];
    $field_form_safe =  (substr($c['formField'],0,4)=='xml:' ? str_replace('/',':',$c['formField']) : $c['formField']);
    $c['formFieldWidth'] =   ($c['formFieldWidth'] ? $c['formFieldWidth'] : $this->_form_settings['default_field_width']);
    $value =
      ($this->_recordID ?
        ...
        :
          (isset($_REQUEST[$field_form_safe]) ? $_REQUEST[$field_form_safe] : "")
        );
    $selectorSQL = $this->_Obj_Report_Column->get_selector_sql($c['fieldType'],$this->_current_user_rights['is_MASTERADMIN'],$c['formSelectorSQLMaster'],$c['formSelectorSQLMember']);
...
//      case "iframe":
    ...
    if (count($reportTab)) {
      if ($reportTab != $this->_tab_last_shown) {
        if ($reportTab !="0.") {
          ...
          $this->_html.= "<table cellpadding='2' cellspacing='0' style='width:100%;border:0;border-collapse:collapse;'>\n";
        }
    ...
    switch ($c['fieldType']) {
      case "combo_action_operation":
        $destinationOperation =   (isset($this->_record) ? $this->_record['destinationOperation'] : '');
        $destinationID =          (isset($this->_record) ? $this->_record['destinationID'] : get_var('destinationID'));
        $destinationValue =       (isset($this->_record) ? $this->_record['destinationValue'] : '');
        $Obj_Action =             new Action;
        $this->_html.=            $Obj_Action->draw_combo_action_operation($destinationOperation,$destinationID,$destinationValue,$c['formFieldSpecial'],$this->_form_settings['default_field_width'],$this->_report_name);
      break;
      case "combo_product_relationship":
        $related_object =         (isset($this->_record) ? $this->_record['related_object'] : get_var('related_objectID'));
        $related_objectID =       (isset($this->_record) ? $this->_record['related_objectID'] : get_var('related_objectID'));
        $Obj_PR =                 new Product_Relationship;
        $this->_html.=            $Obj_PR->draw_combo_product_relationship($related_object,$related_objectID,$c['formFieldSpecial'],$this->_form_settings['default_field_width'],$this->_report_name);
      break;
      default:
       $this->_html.= $this->_Obj_Report_Column->draw_form_field((isset($this->_record) ? $this->_record : array()),$field_form_safe,$value,$c['fieldType'],$fieldWidth,$selectorSQL,$this->_reportID,"",$c['readOnly'],$this->_bulk_update,$label,$c['formFieldSpecial'],$c['formFieldHeight']);
      break;
    }
  ...
  protected function _draw_form_title(){
    $this->_html.=
      ...
      .draw_form_header($this->_report_record['formTitle'].($this->_bulk_update ? " (Bulk update)" : ""),$this->_report_record['help'],0)
      ."</td>\n"
      ."  </tr>\n";
  }
  ...
  protected function _do_insert(){
    ...
    $this->_recordID = $this->_Obj_Primary->insert($field_set);
  }
  ...
  protected function _do_update(){
    $Obj_Record = new Record($this->_table,$this->_recordID);
    if ($this->_report_record['archiveChanges']=='1') {
      $Obj_Record->archive();
    }
    $field_set = $this->_prepare_fields();
    $Obj_Record->update($field_set);
  }
  ...
  protected function _execute_actions_post_save() {
    ...
    switch ($this->_operation){
      case "insert":
        $this->actions_execute('report_insert_post',$this->_table,$this->_report_record['primaryObject'],$this->_recordID,$_POST);
      break;
      case "update":
        $this->actions_execute('report_update_post',$this->_table,$this->_report_record['primaryObject'],$this->_recordID,$_POST);
      break;
    }
  }
  ...
  protected function _execute_actions_pre_save(){
    switch ($this->_operation) {
      case "insert":
        $this->actions_execute('report_insert_pre',$this->_table,$this->_report_record['primaryObject'],false,$_POST);
      break;
      case "update":
        $this->actions_execute('report_update_pre',$this->_table,$this->_report_record['primaryObject'], $this->_recordID, $_POST);
      break;
    }
  }
  ...
  static function _get_js_form_code($mode,$submode,$ID,$error_msg,$report_name,$selectID,$operation){
    ...
    if ($error_msg!='') {
      $out.=
        ...
        ."      \"<div style='padding:4px;max-height:290px;overflow:auto;'>Please note:<ul style='margin:0;padding:0 0 0 2em'>\"+popup_msg+\"</ul></div>\",\n"
  ...
  protected function _load_form_dimensions(){
    ...
    $this->_form_settings['width'] =		        ($this->_forced_width ? $this->_forced_width : (int)$this->_report_record['popupFormWidth'])-20;
    $this->_form_settings['label_width'] =	        165;
    $this->_form_settings['default_field_width'] =	$this->_form_settings['width'] - $this->_form_settings['label_width'] - 15;
  }
  ...
  protected function _prepare_field(&$field_set, $data){
    ...
    switch ($type) {
      ...
      case "file_upload_to_userfile_folder":
        if (isset($_REQUEST[$field_form_safe."_mark_delete"]) && $_REQUEST[$field_form_safe."_mark_delete"]=='1'){
          $old_data =
        ...
        else if (isset($_FILES[$field_form_safe]) && isset($_FILES[$field_form_safe]['tmp_name']) && is_uploaded_file($_FILES[$field_form_safe]['tmp_name'])){
      ...
      case "html_with_text":
        ...
        $value =    $Obj->prepare_field($field);
        $field_set[$field] =            addslashes($value);
        $field_set[$field.'_text'] =    addslashes(preg_replace("/\[ECL\][^\[]*\[\/ECL\]/","",strip_tags($raw_value)));
      break;
      case "listTypeID":
...
//    y($field_set);die;
  ...
  protected function _set_default_settings(){
    ...
    if ($this->_default_settings['all_user_perms']) {
      if (!isset($_REQUEST['permCOMMUNITYADMIN'])) {  $_REQUEST['permCOMMUNITYADMIN']=1; }
      if (!isset($_REQUEST['permGROUPEDITOR'])) {     $_REQUEST['permGROUPEDITOR']=1; }
      if (!isset($_REQUEST['permGROUPVIEWER'])) {     $_REQUEST['permGROUPVIEWER']=1; }
      if (!isset($_REQUEST['permMASTERADMIN'])) {     $_REQUEST['permMASTERADMIN']=1; }
      if (!isset($_REQUEST['permPUBLIC'])) {          $_REQUEST['permPUBLIC']=1; }
      if (!isset($_REQUEST['permSYSLOGON'])) {        $_REQUEST['permSYSLOGON']=1; }
      if (!isset($_REQUEST['permSYSMEMBER'])) {       $_REQUEST['permSYSMEMBER']=1; }
      if (!isset($_REQUEST['permSYSEDITOR'])) {       $_REQUEST['permSYSEDITOR']=1; }
      if (!isset($_REQUEST['permSYSAPPROVER'])) {     $_REQUEST['permSYSAPPROVER']=1; }
      if (!isset($_REQUEST['permSYSADMIN'])) {        $_REQUEST['permSYSADMIN']=1; }
      if (!isset($_REQUEST['permUSERADMIN'])) {       $_REQUEST['permUSERADMIN']=1; }
    }
    if (!isset($_REQUEST['comments_allow'])) {    $_REQUEST['comments_allow'] =   ($this->_default_settings['comments_public'] ? 'all' : 'none'); }
    if (!isset($_REQUEST['ratings_allow'])) {     $_REQUEST['ratings_allow']=     ($this->_default_settings['ratings_public'] ?  'all' : 'none'); }
    if (!isset($_REQUEST['enable'])) {            $_REQUEST['enable']=            ($this->_default_settings['enabled'] ?         '1' : '0'); }
  ...
  protected function _setup($report_name, $controls=1,$alt_controls='',$show_header=true, $forced_width=false){
    global $ID, $msg;
  ...
  function seq($primaryTable,$targetField,$targetValue,$offset,$debug=false) {
    ...
    switch($primaryTable){
      ...
      break;
      default:
        do_log(3,__CLASS__.'::'.__FUNCTION__.'()','seq','The SEQ operation is not supported for entries in the '.$primaryTable.' table.');
        die('The SEQ operation is not supported for entries in the '.$primaryTable.' table. Please report this error.');
      break;
    }
    $sql =
       "UPDATE\n"
