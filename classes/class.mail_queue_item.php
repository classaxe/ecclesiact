<?php
define('VERSION_MAIL_QUEUE_ITEM', '1.0.16');
/*
Version History:
  1.0.16 (2015-03-01)
...
class Mail_Queue_Item extends Displayable_Item
{
    ...
    public function view()
    {
        ...
        $this->with_list =  get_var('with_list')=='yes';
        $Obj =              new Person($record['personID']);
        $this->messages =   $Obj->get_email_message_list();
        ...
        if ($count>1) {
            $html.= ($this->with_list ?
                 "<p><b>All ".$count." messages sent to your account "
                ."(<a href=\"".BASE_PATH."email-view/".$this->_get_ID()."\">hide this list</a>)</b></p>\n"
                .$this->view_message_list()
             :
                 "<p><b><a href=\"".BASE_PATH."email-view/".$this->_get_ID()
                ."?with_list=yes\">Click here</a> to see all ".$count." messages sent to your account</b></p>\n"
            );
        }
        $html.=
             "</body>\n"
    ...
    public function view_message_list()
    {
        ...
        foreach ($this->messages as $m) {
            $url =    BASE_PATH."email-view/".$m['ID'].($this->with_list ? "?with_list=yes" : "");
            $out.=
                 "  <tr"
                .($m['ID']===$this->_get_ID() ?
                    " style='background:#e0ffe0' title='(Currently displayed message)'"
                 :
                    ""
                )
                .">\n"
                ...
                ."    <td>".$m['subject']."</td>\n"
                ."    <td><a href=\"".$url."\""
                ." onclick=\"popWin(this.href,'message_".$m['ID']."','location=1,status=1,scrollbars=1,resizable=1',"
                ."900,600,1);return false;\">View</a></td>\n"
                ."  </tr>\n";
        }
        $out.=
             "  </tbody>\n"
    ...
    public function viewMessagesForPerson($personID)
    {
        $Person =           new Person($personID);
        $this->messages =  $Person->get_email_message_list();
        return $this->view_message_list();
