<?php
define ("VERSION_REPORT_REPORT","1.0.28");
...
class Report_Report extends Report {
  ...
  function do_commands() {
    ...
    switch ($_REQUEST['command']){
      case 'report':
        ...
        $json_mode = (isset($_REQUEST['ajax_popup_url']) && $_REQUEST['ajax_popup_url'] ? $_REQUEST['ajax_popup_url'] : false);
  ...
  function draw($report_name,$toolbar,$ajax_mode=false) {
...
//    y($_POST);DIE;
//    print "<pre>\$targetReportID=$targetReportID, \$targetField=$targetField, \$targetFieldID=$targetFieldID, \$targetValue=$targetValue, \$filterField=$filterField, \$filterExact=$filterExact, \$filterValue=$filterValue, \$toolbar=$toolbar</pre>\n";
    mem(__FUNCTION__."()-1");
...
//    $this->_debug = 1;
    if ($this->_toolbar==0 || $this->_toolbar==2) {  // preserve vars if no toolbar - used for pages with multiple reports
      $old_limit =  $limit;   $limit = -1;
      $old_offset = $offset;  $offset = 0;
      $old_filterField =  $filterField;   $filterField = "";
      $old_filterValue =  $filterValue;   $filterValue = "";
      $old_filterExact =  $filterExact;   $filterExact = "";
    }
    $newID =          "";
    $msg =            "";
    $msg_tooltip =    "";
...
//    print "\$targetReportID=$targetReportID, \$targetValue=$targetValue, \$filterField=$filterField, \$filterExact=$filterExact, \$filterValue=$filterValue<br />\n";
    $filterField_sql = "";
    if ($filterField!='') {
    ...
    if (empty($limit))  { $limit = 10; } else { $limit = (int) $limit; }
...
//    print "\$targetReportID=$targetReportID, \$targetValue=$targetValue, \$filterFieldID=$filterFieldID, \$filterExact=$filterExact, \$filterValue=$filterValue<br />\n";
    mem(__FUNCTION__."()-2");
    if ($this->_debug==1) {
      print
         "<pre>\n"
...
//      die;
    ...
    if (!$isMASTERADMIN && $report_row['required_feature'] && !SYSTEM::has_feature($report_row['required_feature'])){
      if ($ajax_mode) {
        return array('html'=>'','js'=>'');
      }
      return "<p><b>Error:</b> Required feature <b>".$report_row['required_feature']."</b> must be enabled in order to use report <b>".$report_row['name']."</b></p>";
    }
    mem(__FUNCTION__."()-3");
//    y($report_row);
    ...
    if ($thisReportID==$targetReportID) {
      switch ($submode) {
        case "copy_report":
          ...
          if ($newID) {
            $ID = $newID;
            $msg = status_message(0,true,'Report','',
              "been copied to "
        ...
        case "filter_delete":
          ...
          else if (($isMASTERADMIN || $isSYSADMIN) && $filter['destinationType']=='system' && $filter['destinationID']==SYS_ID){
            $canDo = true;
          }
          else if ($filter['destinationType']=='person' && $filter['destinationID']==get_userID()){
            $canDo = true;
          }
          if ($canDo) {
            $Obj_Filter->delete();
  ...
  //  print "$thisReportID==$targetReportID ";
    if ($thisReportID==$targetReportID && $isEDITOR) {
      ...
      switch ($submode) {
        case 'delete':
          $this->actions_execute('report_delete_pre',$reportPrimaryTable,$reportPrimaryObjectName,$targetID);
        break;
        case "set":
          $data = array($targetField => $targetValue);
          $this->actions_execute('report_update_pre',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
        break;
      }
      $done = false;
      ...
      if (!$done) {
        switch ($submode) {
          case "copy":
            if ($this->handle_copy($report_name,$args,$newID,$msg,$msg_tooltip)) {
              $this->actions_execute('report_copy_post',$reportPrimaryTable,$reportPrimaryObjectName,$newID);
            }
            else {
              $targetID = false;
            }
          ...
          case "empty":
            switch ($report_name) {
              case "groups":
                ...
                $Obj->delete_members();
                $msg =    status_message(0,true,$record_type,'had all '.$members.' members','deleted.',$targetID);
              break;
            }
          break;
          case "process_order":
            ...
            if (strlen($targetID)>0) {
              $msg =    status_message(0,true,$record_type,'',"been processed.",$targetID);
            }
            else {
              $msg =    status_message(1,true,$record_type,'',"been processed as none were previously unprocessed.",$targetID);
            }
          break;
          case "set":
            if ($targetField!="") {
              ...
              $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
            }
          break;
          case "set_as_approved":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked as approved.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as approved.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_as_hidden":
            $ObjRecord =  new Record($reportPrimaryTable,$targetID);
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been hidden.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been hidden.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_as_attended":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been shown as attended.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been shown as attended.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_as_member":
            $ObjRecord =  new Record($reportPrimaryTable,$targetID);
            ...
            $ObjRecord->update($data);
            $msg =         status_message(0,true,$record_type,'','been set to full member.',$targetID);
            $msg_tooltip = status_message(0,false,$record_type,'','been set to full member.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_as_spam":
            $ObjRecord =  new Record($reportPrimaryTable,$targetID);
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked and reported as spam.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked and reported as spam.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_as_unapproved":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked as pending approval.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as pending approval.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_email_opt_in":
            ...
            foreach ($targetID_arr as $ID){
              ...
              $new_log =
                 ...
                 .$targetValue
                 .($old_log ? "\r\n".$old_log : "");
              $data =       array('permEMAILOPTIN'=>'1', 'permEMAILOPTOUT'=>'0', 'email_subscription_log'=>$new_log);
              $ObjRecord->update($data);
            }
            $msg =            status_message(0,true,$record_type,'','been marked as opting in for emails to this group.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as opting in for emails to this group.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_email_opt_out":
            ...
            foreach ($targetID_arr as $ID){
              ...
              $new_log =
                 ...
                 .$targetValue
                 .($old_log ? "\r\n".$old_log : "");
              $data =       array('permEMAILOPTIN'=>'0', 'permEMAILOPTOUT'=>'1', 'email_subscription_log'=>$new_log);
              $ObjRecord->update($data);
            }
            $msg =            status_message(0,true,$record_type,'','been marked as opting out of emails to this group.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as opting out of emails to this group.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_important_off":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked as of normal importance.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as of normal importance.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_important_on":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked as of high importance.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked as of high importance.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
          case "set_password":
            if ($targetValue!="") {
              ...
              $msg =         status_message(0,true,'Password','','been updated for '.$PUsername.' (Passwords are <b>not</b> case sensitive.)',$targetID);
              $msg_tooltip = status_message(0,false,'Password','','been updated for '.$PUsername,$targetID);
            }
          break;
          case "set_process_maps":
            ...
            $ObjRecord->update($data);
            $msg =            status_message(0,true,$record_type,'','been marked for map processing.',$targetID);
            $msg_tooltip =    status_message(0,false,$record_type,'','been marked for map processing.',$targetID);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
          break;
        }
        // ***************************
        ...
        switch ($submode) {
          case 'delete':
            $this->actions_execute('report_delete_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID);
          break;
          case "set":
            $data = array($targetField => $targetValue);
            $this->actions_execute('report_update_post',$reportPrimaryTable,$reportPrimaryObjectName,$targetID,$data);
    ...
    if ($record_count>1000 && $limit==-1) {
      $limit = 1000;
      $msg = "<b>Information:</b> In order to preserve memory, the number of records shown has been limited to 1000.";
    }
    ...
    if ($this->_toolbar==1 || $this->_toolbar==2){
      ...
      $this->_html.=
        ...
        .$Obj_HTML->draw_toolbar('component')
...
//
      ;
      $this->_html.=
         $this->draw_toolbar_paging($record_count,$limit,$offset,1,1,$this->_get_ID(),$report_name,$this->_toolbar,'',$ajax_mode)
    ...
    if ($this->_toolbar==1) {
      ...
      $this->_html.=
         $this->draw_toolbar_paging($record_count,$limit,$offset,1,1,$this->_get_ID(),$report_name,$this->_toolbar,'',$ajax_mode)
        ."<br class='clr_b' />"
        .$this->draw_toolbar_filter($columnList,$filterExact,$filterField,$filterValue,$filterValueDefault,$this->_get_ID(),$report_name,$this->_toolbar,$ajax_mode)
        ."<br class='clr_b' />"
    ...
    if ($submode=='show_addresses' && $thisReportID==$targetReportID) {
      ...
      else if (count($email_addresses)==1 && substr($email_addresses[0],0,6)=='Error:'){
        $email_title = "Error:";
        $email_addresses[0] = substr($email_addresses[0],6);
      }
      else if (count($email_addresses)==1){
        $email_title = "Viewing the only email address:";
      }
      else {
        $email_title = "Viewing ".($targetID ? "selected" : "all")." email addresses (".count($email_addresses).")";
      }
      $this->_html.=
    ...
    $this->_html.=
       draw_form_field('sortBy_'.$this->_get_ID(),($thisReportID==$targetReportID ? $sortBy : ""),'hidden')
      ."\n<table cellpadding=\"2\" cellspacing=\"1\" border=\"0\" class=\"report\" summary=\"".$reportTitle."\">\n"
      ."  <thead>\n"
      ."    <tr>\n"
      .$this->get_report_heads($columnList,false,$report_name,$this->_toolbar,$ajax_mode)
      ."    </tr>\n"
      ."  </thead>\n"
...
//        y($row);
          ...
          if ($column['access']==1 && $column['visible']==1) {
            $this->_html.=
              $ObjReportColumn->draw_report_field(
                ...
                $isEDITOR,$report_name,$_targetID,$mayCancel,
                $reportMembersGlobalEditors,$ajax_mode,$ObjPrimary
              );
          }
        }
        $this->_html.= "      </tr>\n";
      ...
      $cols = 0;
      foreach ($columnList as $column) {
        if (($column['reportLabel']!="" || $column['fieldType']=='checkbox') && $ObjReportColumn->is_visible($column)) {
          $cols++;
        }
      }
      $this->_html.=
    ...
    if ($this->_toolbar==1 && $limit!=-1 && $record_count>$limit) {
      $this->_html.=
         $this->draw_toolbar_paging($record_count,$limit,$offset,true,true,$this->_get_ID(),$report_name,$this->_toolbar,true,$ajax_mode)
        ."<br class='clr_b' />"
        ;
    }
  ...
  public function draw_toolbar_filter($columnList,$filterExact,$filterField,$filterValue,$filterValueDefault,$reportID=false,$report_name=false,$toolbar=false,$ajax_mode=false) {
    global $page_vars;
    ...
    $out =
      ...
      ."  <tr>\n"
      ."    <td>"
      ."<img class='b' src='".BASE_PATH."img/sysimg/icon_toolbar_end_left.gif' style='height:16px;width:6px;padding-top:2px;padding-bottom:2px;' alt='|' />"
      ."</td>\n"
   ...
   // FilterExact
    ...
    $out.=
      ...
      ."<img class='b' src=\"".BASE_PATH."img/sysimg/icon_info.gif\" style='border:none' alt='' /></a></td>\n"
      ."    <td class='va_t'><input type='button' id='btn_go_".$reportID."' value='Go' class='formButton' style='width: 30px; height: 20px;'"
      ." onclick=\"filterbar_go_onclick('".$reportID."','".$report_name."','".$toolbar."','".$ajax_mode."','".$page_url."')\" /></td>"
      ."    <td class='va_t'><input type='button' id='btn_clear_".$reportID."' value='Clear' class='formButton' style='width: 40px; height: 20px;'"
      ." onclick=\"filterbar_clear_onclick('".$reportID."','".$report_name."','".$toolbar."','".$ajax_mode."','".$page_url."')\" />"
      ."</td>\n"
      .($ajax_mode ?
         ""
       :
         "    <td class='va_t'><input type='button' id='btn_save_".$reportID."' value='Save...' class='formButton' style='width: 50px; height: 20px'"
  ...
  public function draw_toolbar_paging($record_count,$limit,$offset,$show_prev_next,$show_page_select,$reportID=false,$report_name=false,$toolbar=false,$secondary=false,$ajax_mode='') {
    global $filterField, $page_vars;
    $page_url =         BASE_PATH.trim($page_vars['path'],'/');
    if ($limit>$record_count) {
      if ($record_count>20) {   $limit = 10; }
      if ($record_count>50) {   $limit = 25; }
      if ($record_count>100) {  $limit = 50; }
      if ($record_count>200) {  $limit = 100; }
      if ($record_count>500) {  $limit = 200; }
      if ($record_count>1000) { $limit = 500; }
    }
    $out =
    ...
    if ($record_count>10) {
      $out.=
        ...
        .($record_count>20 ?    "  <option value=\"20\"".($limit==20 ? " selected='selected'":"").">20 Results</option>\n" : "")
        .($record_count>50 ?    "  <option value=\"50\"".($limit==50 ? " selected='selected'":"").">50 Results</option>\n" : "")
        .($record_count>100 ?   "  <option value=\"100\"".($limit==100 ? " selected='selected'":"").">100 Results</option>\n" : "")
        .($record_count>200 ?   "  <option value=\"200\"".($limit==200 ? " selected='selected'":"").">200 Results</option>\n" : "")
        .($record_count>500 ?   "  <option value=\"500\"".($limit==500 ? " selected='selected'":"").">500 Results</option>\n" : "")
        .($record_count==1000 ? "  <option value=\"1000\"".($limit==1000 ? " selected='selected'":"").">1000 Results</option>\n" : "")
      ...
      if ($limit!=-1 && $show_page_select) {
        $out.=
           "<td><select id=\"offset_".$reportID.($secondary ? "_s" : "")."\" name=\"offset_".$reportID.($secondary ? "_s" : "")."\""
        ...
        for ($i=0; $i<$record_count; $i = $i+$limit) {
          $out.="  <option value=\"".$i."\"".($offset==$i ? " selected='selected'":"").">Show ".($i+1)."-".($i+$limit>$record_count ? $record_count : $i+$limit)."</option>\n";
        }
        $out.=
           "</select>&nbsp;</td>\n"
  ...
  function get_report_heads($columns,$ajax=false,$report_name,$toolbar,$ajax_mode) {
    $isMASTERADMIN = get_person_permission("MASTERADMIN");
    $out = '';
    $Obj_Report = new Report();
    foreach ($columns as $column){
      if ($column['access']==1 && $column['visible']==1 && ($column['reportLabel']!="" || $column['fieldType']=='checkbox' || $column['fieldType']=='delete')){
        switch ($column['fieldType']) {
          case "quantity":
          break;
          default:
            $labelIsImage =	(substr($column['reportLabel'],0,4)=="<img" || substr($column['reportLabel'],0,7)=="[LABEL]");
            $cm_code =
               ($isMASTERADMIN && !$ajax ?
                 " onmouseover=\"if(!_contextActive) {_CM.type='report_column';_CM.ID='".$column['report_columnID']."'};\""
                ." onmouseout=\"_CM.type='';\""
               : "");
            switch (strToLower($column['fieldType'])){
              case 'checkbox':
                $out.=
                  ...
                  .($ajax ? "\\n" : "\n")
                  ."  <input type='checkbox' value='1' onclick=\"column_select(".$column['reportID'].",this.checked)\"/>"
                  .($ajax ? "\\n" : "\n")
                  ."</th>".($ajax ? "\\n" : "\n")
                  .($ajax? "\"" : "");
              break;
  ...
  function get_report_head_sortable($hint,$label,$test,$AZ,$lblImage,$ID,$report_name,$toolbar,$ajax_popup_url){
    ...
    return
      ...
      ."');ajax_report('".$this->_get_ID()."','".$report_name."','".$toolbar."','".$ajax_popup_url."','".$page_url."');\""
	  ." title=\"".$hint."\">".$label
	  .($sortBy==$test ?        "<img src='".BASE_PATH."img/spacer' class='icons' style='height:8px;width:9px;background-position:-2919px 0px;' alt='A-Z' />" : "")
	  .($sortBy==$test."_d" ?   "<img src='".BASE_PATH."img/spacer' class='icons' style='height:8px;width:9px;background-position:-2928px 0px;' alt='Z-A' />" : "")
	  ."</th>";
  }
