...
class Listtype extends Record
{
    ...
    public function deleteListData()
    {
        $list_data_arr = $this->get_listdata();
        $Obj_ListData = new ListData;
        foreach ($list_data_arr as $list_data) {
    ...
    {
        $header =
            "Selected ".$this->_get_object_name().$this->plural($targetID)." with List Data";
        $extra_delete =
             "DELETE FROM `listdata`               WHERE `systemID` IN (1,".SYS_ID.")"
            ." AND `listtypeID` IN (".$targetID.");\n";
        $Obj = new Backup;
        $extra_select = $Obj->db_export_sql_query(
            "`listdata`              ",
            "SELECT * FROM `listdata` WHERE `systemID` IN (1,".SYS_ID.") AND `listtypeID` IN (".$targetID.")"
            ." ORDER BY `listtypeID`;",
            $show_fields
        )."\n";
    ...
    public function get_record_by_name($value, $systemID = SYS_ID)
    {
        if ($this->_get_ID()=='') {
            return false;
        }
        $sql =
            ...
            ."  `listdata`.`systemID` IN (1,".$systemID.")\n";
    //    z($sql);
        return $this->get_record_for_sql($sql);
    }

    public function get_sql_options($name, $sortBy = '', $filter = '')
    {
        return
             "SELECT\n"
    ...
    public static function getListData($listTypeName, $filter = false, $sortBy = false)
    {
