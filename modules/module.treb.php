<?php
define('MODULE_TREB_VERSION','1.0.24');
define ('TREB_PROXY',0);
define ('TREB_DEBUG',0);
...
class TREB extends Gallery_Image {
  static $data_url =    "http://3pv.torontomls.net/data3pv/DownLoad3PVAction.asp";
  static $entry_url =   "http://trebdata.trebnet.com/";
  ...
  protected function _get_context_menu_ID()            { return get_js_safe_ID($this->_get_subtype()); }
  ...
  protected function _get_records_sort_records_using_filter_order_by(){
    switch($this->_get_records_args['order_by']){
      ...
      case "date_a":
      case "title":
        parent::_get_records_sort_records_using_filter_order_by();
        return;
      break;
      case "price_a":
        $order_arr =
  ...
  protected function _manage_insert_available(){
    ...
    foreach ($this->_available_records as $r){
      ...
      if (!$ID){
        ...
        else if ($result =   $Obj_Map->get_geocode($address_2)){
          $data['map_location'] =   $address_2;
          $data['map_lat'] =        $result['lat'];
          $data['map_lon'] =        $result['lon'];
        }
        else if ($result =   $Obj_Map->get_geocode($address_3)){
