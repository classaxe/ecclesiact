...
Version History:
  1.0.26 (2017-01-02)
    1) do_submode() renamed to forum_do_submode() to prevent confusion with parent 
*/
...
class Forum extends Posting
{
    ...
    public function __construct($ID = "", $systemID = SYS_ID)
    {
        ...
        $this->set_module_version(static::VERSION);
    }

    private function forum_do_submode($args, &$ID, &$category, &$msg)
    {
        switch ($args['submode']) {
            case 'ajax_cancel':
                die;
            ...
            case 'ajax_edit_forum':
                print ($args['isAdmin'] ?
                    ...
                    ." onclick=\"forum_action("
                    ."'cancel','edit_".$args['current_forumID']."','".$args['instance']."'"
                    .");\" />"
                    ."<input class='btn' type='button' value='Save'"
                    ." onclick=\"geid_set('targetID','".$args['current_forumID']."');"
                    ."geid_set('submode','save_forum');"
                    ."geid('form').submit();\" />"
                    ."</td>\n"
                    ."  </tr>\n"
                    ."</table>\n"
            ...
            case 'ajax_edit_topic':
                print ($args['isAdmin'] ?
                    ...
                    ."<input class='btn' type='button' value='Save'"
                    ." onclick=\"geid_set('targetID','".$args['current_topicID']."');"
                    ."geid_set('submode','save_topic');"
                    ."geid('form').submit();\" />"
                    ."</td>\n"
                    ."  </tr>\n"
                    ."</table>"
            ...
            case 'ajax_new_forum':
                print ($args['isAdmin'] ?
                    ...
                    ." onclick=\"forum_action("
                    ."'cancel','forum_form_".$args['instance']."','".$args['instance']."'"
                    .");\" />"
                ...
                break;
            case 'ajax_new_post':
                print ($args['isContributor'] && $args['current_topicID'] ?
                    ...
                    .draw_form_field(
                        'new_content',
                        ($args['signature'] ? $args['signature_block'] : ""),
                        'textarea',
                        400,
                        '',
                        '',
            ...
            case 'ajax_your_settings':
                ...
                print ($args['isContributor'] ?
                    ...
                    ."    <td>"
                    .($args['avatar'] ?
                        "<img style='float:left; margin:0 10px 0 0;' src=\"".$avatar."\" alt=\"Your current Avatar\" />"
                    :
                        ""
                    )
                    .Report_Column::draw_form_field(
                        ...
                        'xml:forum:avatar',
                        $args['avatar'],
                        'file_upload_to_userfile_folder',
                        300,
                        '',
                        $reportID
                    )
            ...
            case 'save_your_settings':
                ...
                } elseif (is_uploaded_file($_FILES['xml:forum:avatar']['tmp_name'])) {
    ...
    public function draw_path($_category, $forums, $ID, $topID, $topics)
    {
        global $page;
        $out =
    ...
    public function draw_posts($forum, $instance, $isAdmin, $isContributor, $posts, $topic, $topID, $view_posts, $view_replies)
    {
        global $page;
        if (!$view_posts) {
            return
    ...
    public function get_avatar($personID)
    {
    ...
    public function get_structure($category_csv)
    {
    ...
    public function get_topics_for_forum($ID)
    {
        $sql =
         "SELECT\n"
        ."  COUNT(`post`.`ID`) `posts`,\n"
    ...
    public function get_posts_for_topic($ID)
    {
        $sql =
         "SELECT\n"
        ."  `postings`.`ID`,\n"
...
INSTALL_SQL;
    ...
    public function render($instance)
    {
        ...
        // Handle submodes:
        $msg = array();
        $this->forum_do_submode($args, $ID, $category, $msg);
        $msg = implode("", $msg);
