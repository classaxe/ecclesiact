...
switch ($request_arr[0]) {
    ...
    case "css":
        ...
        if (isset($request_arr[2])) {
            $_REQUEST['ID'] =         $request_arr[2];
        }
        if (isset($request_arr[3])) {
            $_REQUEST['map'] =        $request_arr[3];
        }
        css();
        break;
    case "facebook":
    ...
    case "img":
        switch($request_arr[1]){
            case "barcode":
                if (isset($request_arr[2])) {
                    $_REQUEST['number'] = $request_arr[2];
                }
                if (isset($request_arr[3])) {
                    $_REQUEST['scale'] =  $request_arr[3];
                }
                if (isset($request_arr[4])) {
                    $_REQUEST['color'] =  $request_arr[4];
                }
                if (isset($request_arr[5])) {
                    $_REQUEST['bgcolor'] =$request_arr[5];
                }
                if (isset($request_arr[6])) {
                    $_REQUEST['height'] = $request_arr[6];
                }
                barcode();
                break;
            case "beacon":
            ...
            case "encoded":
                ...
                if (isset($request_arr[3])) {
                    $_REQUEST['size'] =       $request_arr[3];
                }
                if (isset($request_arr[4])) {
                    $_REQUEST['code'] =       $request_arr[4];
                }
                encoded();
                break;
            case "height":
            ...
            case "icon":
                if (isset($request_arr[2])) {
                    $_REQUEST['offset'] =         $request_arr[2];
                }
                if (isset($request_arr[3])) {
                    $_REQUEST['width'] =          $request_arr[3];
                }
                if (isset($request_arr[4])) {
                    $_REQUEST['width_total'] =    $request_arr[4];
                }
                icon();
                break;
            case "max":
            ...
            case "text":
                ...
                if (isset($request_arr[3])) {
                    $_REQUEST['bgcolor'] =$request_arr[3];
                }
                if (isset($request_arr[4])) {
                    $_REQUEST['size'] =   $request_arr[4];
                }
                if (isset($request_arr[5])) {
                    $_REQUEST['font'] =   $request_arr[5];
                }
                if (isset($request_arr[6])) {
                    $_REQUEST['bold'] =   $request_arr[6];
                }
                if (isset($request_arr[7])) {
                    $_REQUEST['text'] =   $request_arr[7];
                }
                text();
                break;
            case "ticket":
            ...
            case "treeview":
                if (isset($request_arr[2])) {
                    $_REQUEST['img']="treeview/".$request_arr[2];
                }
                sysimg();
                break;
            case "user":
...
{
    $system =       get_system();
    $heartbeat =    strtotime($system['cron_job_heartbeat_last_run']);
    if ((time()-$heartbeat) > 50) {
        use_codebase();
        CRON::heartbeat();
    }
    ...
    die(
         "<pre>"
        ."----------------------------------\n"
        ." Ecclesiact Job Runner\n"
        ."----------------------------------\n"
        ." Last run:    ".$system['cron_job_heartbeat_last_run']."\n"
        ." Time now:    ".date('Y-m-d H:i:s', time())."\n"
        ." Memory Used: ".str_repeat(' ', 13 - strlen($mem)).$mem." bytes\n"
        ."----------------------------------</pre>"
    );
}
...
function css()
{
    ...
    switch ($_REQUEST['submode']){
        ...
        case "system":
            ...
            $imgPath =      BASE_PATH."img/sysimg/";
            print
                 "/* [Shared Icons and Labels] */\r\n"
                ."img.icon              { background-image:url(".$imgPath."icons.gif/".$cs_icons.");}\r\n"
                ."img.icon-big          { background-image:url(".$imgPath."icons-big.gif/".$cs_icons_big.");}\r\n"
                ."img.label             { background-image:url(".$imgPath."labels.gif/".$cs_labels.");}\r\n"
                ."ul.breadcrumbs li.sub { background-image:url(".$imgPath."icon_path_separator.gif);}\r\n"
                ."div.rating .img       { background-image:url(".$imgPath."icon_ratings_13x13.gif);}\r\n"
                .".icons                { background-image:url(".$imgPath."icons.gif/".$cs_icons.");}\r\n"
                .".icons-big            { background-image:url(".$imgPath."icons-big.gif/".$cs_icons_big.");}\r\n"
                .".toolbar_icon         { background-image:url(".$imgPath."icons.gif/".$cs_icons.");}\r\n"
                .".admin_toolbartable, a.ti { background:url(".$imgPath."icon_toolbar_background.gif) top; }\r\n"
                ."#popupMask            { "
                .($isIE_lt7 ? "opacity:.40;filter:alpha(opacity=40);" : "")
                ."background-image: url(".$imgPath."maskbg.png) !important; } /* For browsers Moz, Opera, etc. */\r\n"
                ."\r\n"
                ."/* [System Colour Scheme settings] */\r\n"
                ."h1,h2,h3          { color: #".$record['text_heading'].";}\r\n"
                .".highlight        { font-weight: bold; background-color: #".$record['table_header'].";"
                ." color: #".$record['text_heading'].";}\r\n"
                .".cal_table        { border: solid 1px #".$record['cal_border']."; }\r\n"
                .".cal_enlarge      { background: url(".$imgPath."calendar_enlarge.gif) no-repeat;}\r\n"
                .".cal_help         { background: url(".$imgPath."calendar_help.gif) no-repeat;}\r\n"
                .".cal_head         { background-color: #".$record['cal_head']."; height: 2em;}\r\n"
                .".cal_days         { border-top: solid 1px #".$record['cal_border'].";"
                ." border-bottom: solid 1px #".$record['cal_border'].";"
                ." border-right: solid 1px #".$record['cal_border'].";"
                ." background-color: #".$record['cal_days'].";}\r\n"
                ...
                .".cal_has_event    { background-image: url(".$imgPath."calendar_event_indicator.gif,"
                .$record['cal_event'].") !important; }\r\n"
                .".cal_has_events   { background-image: url(".$imgPath."calendar_events_indicator.gif,"
                .$record['cal_event'].") !important; }\r\n"
                .".calendar_mini .cal_has_event,\n"
                .".calendar_mini .cal_has_events { color: #".$record['cal_event']." !important; }\r\n"
                .".cal_nav          { background: url(".$imgPath."calendar_menu_arrow.gif) no-repeat 100% 100%;}\r\n"
                .".table_border     { border-style: solid; border-width: 1px;"
                ." border-color: #".$record['table_border']."; border-collapse: collapse;}\r\n"
...
function icon()
{
    ...
    $width_total =    (
        isset($_REQUEST['width_total']) && (int)$_REQUEST['width_total'] ?
            (int)$_REQUEST['width_total']
         :
            $width
        );
...
function img_ext_mime_header($ext)
{
    switch (strToLower($ext)){
        case "css":     $m = "text/css";
            break;
        case "gif":     $m = "image/gif";
            break;
        case "htm":     $m = "text/html";
            break;
        case "html":    $m = "text/html";
            break;
        case "ico":     $m = "image/x-icon";
            break;
        case "jpg":     $m = "image/jpeg";
            break;
        case "jpeg":    $m = "image/jpeg";
            break;
        case "js":      $m = "text/javascript";
            break;
        case "mp3":     $m = "audio/mpeg";
            break;
        case "pdf":     $m = "application/pdf";
            break;
        case "php":     $m = "text/html";
            break;
        case "png":     $m = "image/png";
            break;
        case "svg":     $m = "image/svg+xml";
            break;
        case "xml":     $m = "application/xml";
            break;
