...
Version History:
  ...
  4.3.c (2010-05-10)
    1) Now includes us.js
  4.3.b (2009-05-30)
    1) Work by Martin to remove redundant 'Go to Help' tips
  4.3.a (2003-03-01)
    1) Initial release for Ecclesiact - includes ua.js code
*/
...
function drawFolder(insertAtObj) {
  ...
  if (this.level>0){
    if (this.isLastNode){ //the last child in the children array
      leftSide = leftSide + "<td valign=top title=\"\">" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
    }
    else{
      leftSide = leftSide + "<td valign=top title=\"\" background=" + ICONPATH + "ftv2vertline.gif>" + auxEv + "<img name='nodeIcon" + this.id + "' id='nodeIcon" + this.id + "' src='" + nodeName + "' width=16 height=22 border=0></a></td>"
    }
  }
  this.isRendered = 1
  if (browserVersion == 2) {
  ...
  var bc_path = (writeBreadcrumbsPath(this.getID())!='Help ' ? true : false);
  docW = docW + "<tr " + (bc_path ? "onmouseover='return omo(\"Go to "+writeBreadcrumbsPath(this.getID())+"\")' onmouseout='return omo(0)' title=\"Go to "+writeBreadcrumbsPath(this.getID())+"\"" : "")+">" + leftSide + "<td valign=top>";
  if (USEICONS) {
...
function propagateChangesInState(folder){
  ...
  }  //otherwise the one given at render stays
  //Change node
  if (USEICONS){
...
function linkFolderHTML(isTextLink) {
  var docW = "";
  if (this.hreference && this.hreference!='javascript:void(0)') { 
    if (USEFRAMES){
...
function drawItem(insertAtObj){
  ...
  var bc_path = (writeBreadcrumbsPath(this.getID())!='Help ' ? true : false);
  docW = docW + "<tr"+(bc_path ? " title=\"Go to "+writeBreadcrumbsPath(this.getID())+"\" onmouseover='return omo(\"Go to "+writeBreadcrumbsPath(this.getID())+"\")' onmouseout='return omo(0)'" : "") + ">" + leftSide + "<td valign=top>"
  if (USEICONS){
...
//alert (id);
  var i=0;
  var nodeObj;
  if (typeof foldersTree != "undefined" && typeof foldersTree.xID != "undefined") {
...
function writeBreadcrumbsPath(thisPage) {
  if (thisPage==null) {
    thisPage =	location.pathname.substring(1+location.pathname.lastIndexOf('/'));
  }
  if (writeBreadcrumbPath(thisPage)==" &gt; Help") {
    return "Help (Root Folder)";
  }
  return "Help "+writeBreadcrumbPath(thisPage);
}

function writeBreadcrumbPath(thePage) {
  if (typeof(parent.treeframe)=='undefined') {
    return "";
  }
  var thisPage =	location.pathname.substring(1+location.pathname.lastIndexOf('/'));
